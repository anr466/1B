...
try:
    from backend.utils.connection_manager import get_connection_manager, ConnectionConfig
...
class BackgroundTradingManager:
    ...
    def start(self):
        ...
        try:
            ...
            self.heartbeat_thread = Thread(target=self._run_heartbeat_loop, daemon=False)
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if _consecutive_errors > 0:
                    logger.info(f"✅ Group B recovered after {_consecutive_errors} consecutive errors")
                    _consecutive_errors = 0
                ...
                logger.error(f"❌ خطأ في حلقة Group B (consecutive: {_consecutive_errors}): {e}")
                ...
                if _consecutive_errors >= _max_consecutive_for_error_status:
                    self._update_system_status("error", f"Group B: {_consecutive_errors} consecutive errors: {e}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"⚠️ فشل تحديث الحالة (محاولة {attempt+1}/{max_retries}): {e}")
    ...
    def get_status(self):
        ...
        return {
            ...
            'last_group_b_run': self.last_group_b_run.isoformat() if self.last_group_b_run else None,
                ...
                while manager.is_running:
                    ...
                    if not manager.group_b_thread.is_alive():
                        ...
                        if _recovery_attempts <= _max_recovery_attempts:
                            logger.warning(f"⚠️ Group B thread died — auto-recovering (attempt {_recovery_attempts}/{_max_recovery_attempts})")
                            recovered = manager._recover_group_b()
                            if recovered:
                                logger.info("✅ Group B thread recovered successfully")
                                continue
                        logger.error(f"❌ Group B failed after {_recovery_attempts} recovery attempts — stopping system")
