...
class DbTradingMixin:
    ...
    def get_unprocessed_signals(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            ...
            rows = conn.execute("""
                SELECT id, symbol, signal_type, strategy, timeframe,
                       price, confidence, is_processed, generated_at
            ...
            with self.get_connection() as conn:
                query = """
                    SELECT id, user_id, symbol, strategy, timeframe,
                           position_type, entry_date, entry_price, quantity,
                           stop_loss, take_profit, order_id,
                           entry_commission, exit_commission,
                           is_active, is_demo, ml_status, ml_confidence,
                           signal_metadata, created_at, updated_at,
                           trailing_sl_price, highest_price, exit_reason,
                           exit_price, profit_loss, profit_pct,
                           closed_at, entry_indicators, exit_order_id
                    FROM active_positions 
                    WHERE user_id = %s AND is_active = TRUE
                """
                params = [user_id]
    ...
    def close_position_on_conn(
        ...
        position = conn.execute(
            ...
            SELECT
                ...
                tp_levels_hit,
                brain_decision_id
            FROM active_positions WHERE id = %s
            """,
            (position_id,),
        ).fetchone()
