...
class PeakValleyScalpingStrategy(StrategyBase):
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['bb_upper'] = bb[bb_upper_col] if bb_upper_col in bb.columns else bb.iloc[:, 0]
            df['bb_middle'] = bb[bb_middle_col] if bb_middle_col in bb.columns else bb.iloc[:, 1]
            df['bb_lower'] = bb[bb_lower_col] if bb_lower_col in bb.columns else bb.iloc[:, 2]
    ...
    def generate_signals(self, dataframe: pd.DataFrame, timeframe: str = '1h', candles_count: int = 100, 
        ...
        """
        try:
            logger.info(f"بدء توليد إشارات التداول لاستراتيجية سكالبنج مبسطة - الإطار الزمني: {timeframe}")
            ...
            for i in range(20, len(df)):
                try:
                    ...
                    if all(col in df.columns for col in ['bb_lower', 'close']):
                        buy_conditions.append(df['close'].iloc[i] <= df['bb_lower'].iloc[i] * 1.01)
                    
                    # حجم جيد
                    if 'volume_ratio' in df.columns:
                        buy_conditions.append(df['volume_ratio'].iloc[i] > volume_threshold)
                    ...
                    if 'rsi' in df.columns:
                        sell_conditions.append(df['rsi'].iloc[i] > rsi_overbought)
                    
                    # السعر قريب من الحد العلوي لـ Bollinger Bands
                    if all(col in df.columns for col in ['bb_upper', 'close']):
                        sell_conditions.append(df['close'].iloc[i] >= df['bb_upper'].iloc[i] * 0.99)
                    
                    # حجم جيد
                    if 'volume_ratio' in df.columns:
                        sell_conditions.append(df['volume_ratio'].iloc[i] > volume_threshold)
                    ...
                    if len(buy_conditions) >= 2 and sum(buy_conditions) >= 2:
                        ...
                        if 'atr' in df.columns:
                            ...
                            df.loc[df.index[i], 'stop_loss'] = df['close'].iloc[i] - (1.5 * atr_value)
                            df.loc[df.index[i], 'take_profit'] = df['close'].iloc[i] + (2.5 * atr_value)
                    
                    # توليد إشارة البيع (يجب تحقق 2 من 3 شروط على الأقل)
                    elif len(sell_conditions) >= 2 and sum(sell_conditions) >= 2:
                    ...
                    if 'rsi' in df.columns:
                        ...
                        if df['rsi'].iloc[i] > 50 and i > 0 and df['rsi'].iloc[i-1] <= 50:
                            exit_conditions.append(True)
                        # خروج من البيع إذا وصل RSI إلى 50 أو أقل
                        elif df['rsi'].iloc[i] < 50 and i > 0 and df['rsi'].iloc[i-1] >= 50:
                            exit_conditions.append(True)
                    
                    # السعر عاد إلى المتوسط
                    if all(col in df.columns for col in ['bb_middle', 'close']):
                        price_near_middle = abs(df['close'].iloc[i] - df['bb_middle'].iloc[i]) / df['bb_middle'].iloc[i] < 0.005
