...
class IntelligentExitSystem:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "tp_levels": {
                "tp1": {
                    "pct": 0.015,
                    "sell_if_bullish": 0.30,
                    "sell_if_bearish": 0.70,
                },
                "tp2": {
                    "pct": 0.030,
                    "sell_if_bullish": 0.40,
                    "sell_if_bearish": 1.0,
                },
                "tp3": {
                    "pct": 0.045,
    ...
    def register_position(
        ...
        self.logger.info(
            f"   TP1: +{self.config['tp_levels']['tp1']['pct'] * 100:.1f}%"
        )
        self.logger.info(
            f"   TP2: +{self.config['tp_levels']['tp2']['pct'] * 100:.1f}%"
        )
        self.logger.info(
            f"   TP3: +{self.config['tp_levels']['tp3']['pct'] * 100:.1f}%"
        )
    ...
    def check_intelligent_exit(
        ...
        trailing_check = self._check_trailing_stop(
            tracker, current_price, pnl_pct, trend
        )
        if trailing_check:
            return trailing_check

        # ===== 4. فحص TP Levels مع منطق ذكي =====
        tp_check = self._check_tp_levels_smart(
            tracker, current_price, pnl_pct, trend, df, idx
        )
        if tp_check:
            return tp_check
        ...
        return ExitSignal(
            ...
            pnl_pct=pnl_pct,
            trailing_stop=(
                tracker.trailing_stop if tracker.trailing_active else None
            ),
    ...
    def _check_stop_loss(
        self, tracker: PositionTracker, price: float, pnl_pct: float
    ) -> Optional[ExitSignal]:
        """فحص Stop Loss"""
    ...
    def _check_trailing_stop(
        ...
        if not tracker.trailing_active and pnl_pct >= activation_pct:
            ...
            self.logger.info(
                f"🎯 [{tracker.symbol}] Trailing مُفعّل @ +{pnl_pct:.2f}%"
            )
            ...
            if price <= tracker.trailing_stop:
                ...
                if trend in [TrendStatus.STRONG_BULLISH, TrendStatus.BULLISH]:
                    ...
                    if drawdown < self.config["pullback"]["max_pullback_pct"]:
                        ...
                        tracker.trailing_stop = tracker.highest_price * (
                            1 - distance_pct * 1.5
                        )
                        return None

                protected_pnl = (
                    (tracker.trailing_stop - tracker.entry_price)
                    / tracker.entry_price
                ) * 100
                return ExitSignal(
                    decision=ExitDecision.FULL_EXIT,
        ...
        for level in tracker.tp_levels:
            if not level["triggered"] and price >= level["price"]:
                ...
                if trend in [TrendStatus.STRONG_BULLISH, TrendStatus.BULLISH]:
                    ...
                    decision = (
                        ExitDecision.PARTIAL_EXIT
                        if sell_pct < 1.0
                        else ExitDecision.FULL_EXIT
                    )
    ...
    def _check_time_exit(
        ...
        if "timestamp" in df.columns:
            ...
            current_time = (
                datetime.now(tracker.entry_time.tzinfo)
                if tracker.entry_time.tzinfo
                else datetime.now()
            )
        ...
        if isinstance(current_time, datetime):
            if tracker.entry_time.tzinfo and current_time.tzinfo is None:
                current_time = current_time.replace(
                    tzinfo=tracker.entry_time.tzinfo
                )
            elif (
                tracker.entry_time.tzinfo is None
    ...
    def _check_reversal(
        ...
        drawdown = (
            tracker.highest_price - df["close"].iloc[idx]
        ) / tracker.highest_price
        ...
        if drawdown >= cfg["reversal_threshold"]:
            ...
            if idx >= cfg["confirm_candles"]:
                ...
                bearish_count = sum(
                    1 for _, r in recent.iterrows() if r["close"] < r["open"]
                )
    ...
    def _analyze_smaller_tf(self, df: pd.DataFrame) -> float:
        ...
        # اتجاه آخر 5 شموع
        bullish = sum(
            1 for _, r in last_5.iterrows() if r["close"] > r["open"]
        )
        score += (bullish - 2.5) * 10

        # اتجاه السعر
        price_change = (
            (last_5["close"].iloc[-1] - last_5["close"].iloc[0])
            / last_5["close"].iloc[0]
            * 100
        )
...
def get_intelligent_exit_system(
    config: Optional[Dict] = None,
) -> IntelligentExitSystem:
