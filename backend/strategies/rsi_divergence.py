...
class RSIDivergenceStrategy(StrategyBase):
    ...
    def find_peaks_valleys(self, data: pd.Series, distance: int = 5) -> Tuple[List[int], List[int]]:
        ...
        try:
            ...
            for i in range(distance, len(data) - distance):
                ...
                for j in range(1, distance + 1):
                    if (data.iloc[i-j] >= current_val or 
    ...
    def detect_divergence(self, price_data: pd.Series, rsi_data: pd.Series, 
                         peaks: List[int], valleys: List[int]) -> pd.Series:
        """اكتشاف التباعد"""
        try:
            ...
            for i in range(1, len(valleys)):
                if valleys[i] < len(price_data) and valleys[i-1] < len(price_data):
                    current_valley = valleys[i]
                    prev_valley = valleys[i-1]
                    
                    # السعر ينخفض و RSI يرتفع
                    price_lower = price_data.iloc[current_valley] < price_data.iloc[prev_valley]
                    rsi_higher = rsi_data.iloc[current_valley] > rsi_data.iloc[prev_valley]
            ...
            for i in range(1, len(peaks)):
                if peaks[i] < len(price_data) and peaks[i-1] < len(price_data):
                    current_peak = peaks[i]
                    prev_peak = peaks[i-1]
                    
                    # السعر يرتفع و RSI ينخفض
                    price_higher = price_data.iloc[current_peak] > price_data.iloc[prev_peak]
                    rsi_lower = rsi_data.iloc[current_peak] < rsi_data.iloc[prev_peak]
    ...
    def generate_signals(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            for i in range(self.params['lookback_period'], len(df) - self.params['confirmation_candles']):
                # شروط الشراء (تباعد إيجابي)
                bullish_divergence = df['divergence'].iloc[i] == 1
                rsi_oversold = df['rsi'].iloc[i] < self.params['rsi_oversold'] + 10  # مرونة في المستوى
                ...
                if bullish_divergence and rsi_oversold and volume_confirm and price_confirmation:
                ...
                rsi_overbought = df['rsi'].iloc[i] > self.params['rsi_overbought'] - 10  # مرونة في المستوى
                ...
                if bearish_divergence and rsi_overbought and volume_confirm and price_confirmation:
