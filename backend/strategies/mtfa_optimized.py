...
class MTFAOptimizedStrategy(StrategyBase):
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # 2. Bollinger Bands
            bb = ta.bbands(df['close'], length=self.params['bb_period'], std=self.params['bb_std'])
            if bb is not None:
            ...
            # 7. MACD للتأكيد
            macd = ta.macd(df['close'], fast=self.params['macd_fast'], 
                          slow=self.params['macd_slow'], signal=self.params['macd_signal'])
            if macd is not None:
                cols = macd.columns.tolist()
                df['macd'] = macd[cols[0]] if len(cols) > 0 else None
                df['macd_signal_line'] = macd[cols[1]] if len(cols) > 1 else None
    ...
    def calculate_m15_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['is_hammer'] = df['is_bullish'] & (df['lower_shadow'] > 2 * df['candle_body'])
            df['is_shooting_star'] = df['is_bearish'] & (df['upper_shadow'] > 2 * df['candle_body'])
    ...
    def check_h1_buy_signal(self, df: pd.DataFrame, idx: int) -> Tuple[bool, int, list]:
        """
        ...
        # 3. Volume > 1.3x
        if pd.notna(vol_ratio) and vol_ratio > self.params['volume_multiplier']:
            score += 1
    ...
    def check_h1_sell_signal(self, df: pd.DataFrame, idx: int) -> Tuple[bool, int, list]:
        """
    ...
    def check_m15_buy_confirmation(self, df: pd.DataFrame, idx: int) -> Tuple[bool, list]:
        """
        ...
        for offset in range(max_bars):
            ...
            macd_positive = pd.notna(macd) and pd.notna(macd_signal) and macd > macd_signal
            
            # RSI يتحسن
            rsi = row.get('rsi')
            prev_rsi = prev.get('rsi')
            rsi_improving = pd.notna(rsi) and pd.notna(prev_rsi) and rsi > prev_rsi
    ...
    def check_m15_sell_confirmation(self, df: pd.DataFrame, idx: int) -> Tuple[bool, list]:
        """
        ...
        for offset in range(max_bars):
            ...
            macd = row.get('macd')
            macd_signal = row.get('macd_signal_line')
            macd_negative = pd.notna(macd) and pd.notna(macd_signal) and macd < macd_signal
            
            # RSI يهبط
            rsi = row.get('rsi')
            prev_rsi = prev.get('rsi')
            rsi_falling = pd.notna(rsi) and pd.notna(prev_rsi) and rsi < prev_rsi
    ...
    def calculate_stop_loss(self, entry_price: float, atr: float, 
                           highest_price: float = None, profit_pct: float = 0) -> Tuple[float, float, bool]:
        """
        ...
        if trailing_active and highest_price:
            trailing_sl = highest_price - (self.params['trailing_sl_atr'] * atr)
        else:
            trailing_sl = fixed_sl
        
        return fixed_sl, trailing_sl, trailing_active
    
    def generate_signals(self, df: pd.DataFrame, timeframe: str = '1h', 
            ...
            for i in range(50, len(df)):
                ...
                if h1_buy:
                    ...
                    if df_m15 is not None:
                        m15_idx = min(i * 4, len(df_m15) - 5)  # نسبة H1:M15 = 1:4
                        m15_confirmed, m15_reasons = self.check_m15_buy_confirmation(df_m15, m15_idx)
                    ...
                    if m15_confirmed:
                        ...
                        fixed_sl, _, _ = self.calculate_stop_loss(current_price, atr)
                        
                        # تسجيل الإشارة
                        df.loc[df.index[i], 'buy_signal'] = 1
                        df.loc[df.index[i], 'stop_loss'] = fixed_sl
                        df.loc[df.index[i], 'signal_confidence'] = min(h1_score / 6.0, 1.0)
                        df.loc[df.index[i], 'signal_reasons'] = ' | '.join(h1_reasons + m15_reasons)
                
                # فحص إشارة البيع/الخروج على H1
                h1_sell, sell_score, sell_reasons = self.check_h1_sell_signal(df, i)
                ...
                if h1_sell:
                    ...
                    if df_m15 is not None:
                        ...
                        m15_confirmed, m15_reasons = self.check_m15_sell_confirmation(df_m15, m15_idx)
                    
                    if m15_confirmed:
                        df.loc[df.index[i], 'sell_signal'] = 1
                        df.loc[df.index[i], 'signal_reasons'] = ' | '.join(sell_reasons + m15_reasons)
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'risk_management': {
                'fixed_sl': f"{self.params['fixed_sl_atr']} ATR",
                'trailing_sl': f"{self.params['trailing_sl_atr']} ATR",
                'trailing_activation': f"{self.params['trailing_activation_pct']}%"
            },
            'entry_conditions': [
                'RSI < 30 (تشبع بيعي)',
                'Price <= BB Lower',
