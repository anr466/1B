...
class StrategyBase(ABC):
    ...
    def __init__(self, **params):
        ...
        self.params = params
        self.required_candles = 200  # الحد الأدنى لعدد الشموع المطلوبة للاستراتيجية
        
        # إعداد محلل الأطر الزمنية المتعددة
        self.mtf_analyzer = MultiTimeframeAnalyzer()
        
        # المعلمات الافتراضية للتحليل متعدد الأطر الزمنية
        self.use_mtf_analysis = params.get('use_mtf_analysis', True)  # تفعيل/تعطيل التحليل متعدد الأطر الزمنية
        self.higher_timeframes = params.get('higher_timeframes', [])  # الأطر الزمنية الأعلى للتحليل
    ...
    def run(self, dataframe: pd.DataFrame, timeframe: str = '1h', symbol: str = None) -> pd.DataFrame:
        """
    ...
    def run_strategy(self, dataframe: pd.DataFrame, timeframe: str = '1h', symbol: str = None) -> pd.DataFrame:
        """
        ...
        if dataframe is None or len(dataframe) < self.required_candles:
            logger.error(f"البيانات غير كافية، مطلوب {self.required_candles} شمعة على الأقل")
            return None
        ...
        try:
            ...
            if self.use_mtf_analysis and self.higher_timeframes and symbol:
                ...
                logger.info(f"جلب بيانات متعددة الأطر الزمنية لـ {symbol}: {timeframe} (أساسي) + {self.higher_timeframes}")
                
                try:
                    # جلب البيانات وحساب المؤشرات لكل إطار زمني
                    mtf_data = self.mtf_analyzer.get_multi_timeframe_data(
                        symbol, timeframe, self.higher_timeframes, len(dataframe)
                    )
                    
                    if mtf_data and timeframe in mtf_data:
                        # محاذاة الأطر الزمنية
                        aligned_mtf = self.mtf_analyzer.align_timeframes(mtf_data, timeframe)
                        
                        # حساب المؤشرات لكل إطار زمني
                        mtf_with_indicators = self.mtf_analyzer.calculate_mtf_indicators(aligned_mtf)
                        
                        # دمج بيانات الأطر الزمنية المختلفة في إطار زمني واحد
                        aligned_df = self.mtf_analyzer.resample_to_base_timeframe(mtf_with_indicators, timeframe)
                        logger.info(f"تم دمج بيانات {len(self.higher_timeframes) + 1} إطار زمني بنجاح")
                    else:
                        logger.warning("لم يتم العثور على بيانات متعددة الأطر الزمنية، سيتم استخدام البيانات الأساسية فقط")
                        aligned_df = dataframe.copy()
                        mtf_data = None
                except Exception as e:
                    logger.error(f"خطأ في جلب بيانات متعددة الأطر الزمنية: {str(e)}")
                    aligned_df = dataframe.copy()
                    mtf_data = None
            ...
            # توليد الإشارات - تمرير بيانات الأطر الزمنية المتعددة إذا كانت متوفرة
            result = self.generate_signals(df_with_indicators, timeframe, len(dataframe), mtf_data)
            ...
            signal_count = {
                'buy_signal': result['buy_signal'].sum() if 'buy_signal' in result.columns else 0,
                'sell_signal': result['sell_signal'].sum() if 'sell_signal' in result.columns else 0,
                'stop_loss_signal': result['stop_loss_signal'].sum() if 'stop_loss_signal' in result.columns else 0,
                'take_profit_signal': result['take_profit_signal'].sum() if 'take_profit_signal' in result.columns else 0,
                'partial_exit_signal': result['partial_exit_signal'].sum() if 'partial_exit_signal' in result.columns else 0
            }
    ...
    def _standardize_column_names(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        essential_signal_columns = ['buy_signal', 'sell_signal', 'stop_loss_signal', 'take_profit_signal', 'partial_exit_signal']
        ...
        for old_col, new_col in signal_map.items():
            if old_col in df.columns:
                if new_col not in df.columns:
                    ...
                    # نستخدم عملية + بدلاً من | ونحول النتيجة لقيمة منطقية بحيث إذا كان العمود الأول أو الثاني يحتوي على 1 تكون النتيجة 1
                    df[new_col] = ((df[new_col] > 0) | (df[old_col] > 0)).astype(int)
        ...
        risk_columns = ['stop_loss_signal', 'take_profit_signal', 'partial_exit_signal']
    ...
    def backtest(self, dataframe: pd.DataFrame, initial_capital: float = 1000.0, 
                commission: float = 0.001, risk_per_trade: float = 0.1) -> Dict[str, Any]:
        """
        ...
        buy_signals_count = df['buy_signal'].sum() if 'buy_signal' in df.columns else 0
        sell_signals_count = df['sell_signal'].sum() if 'sell_signal' in df.columns else 0
        logger.debug(f"[تشخيص backtest] عدد إشارات الشراء: {buy_signals_count}, عدد إشارات البيع: {sell_signals_count}")
        logger.debug(f"[تشخيص backtest] أسماء الأعمدة المتاحة: {', '.join(df.columns.tolist()[:10])} ...")
        ...
        for i in range(1, len(df)):
            ...
            buy_signal = df['buy_signal'].iloc[i-1] == 1 if 'buy_signal' in df.columns else False
            if buy_signal and position == 0:
                ...
                if cost <= capital:
                    ...
                    capital -= cost
                    logger.debug(f"[تشخيص backtest] فتح صفقة شراء: التاريخ={entry_date}, السعر={entry_price:.2f}, الحجم={position_size:.6f}")
                    
            # إشارة بيع ولدينا مركز - استخدام المسميات الموحدة
            sell_signal = df['sell_signal'].iloc[i-1] == 1 if 'sell_signal' in df.columns else False
            stop_loss = df['stop_loss_signal'].iloc[i-1] == 1 if 'stop_loss_signal' in df.columns else False
            take_profit = df['take_profit_signal'].iloc[i-1] == 1 if 'take_profit_signal' in df.columns else False
            ...
            if (sell_signal or stop_loss or take_profit) and position > 0:
                ...
                exit_reason = "بيع" if sell_signal else "وقف خسارة" if stop_loss else "جني أرباح"
                logger.debug(f"[تشخيص backtest] إغلاق صفقة [{exit_reason}]: التاريخ={df.index[i]}, دخول={entry_price:.2f}, خروج={current_price:.2f}, ربح={profit_pct:.2f}%")
        ...
        if position > 0:
            ...
            logger.debug(f"[تشخيص backtest] إغلاق صفقة نهائية: الربح={profit_pct:.2f}%")
        ...
        logger.info(f"[نتائج backtest] عدد الصفقات: {total_trades}, إجمالي الربح: {profit_pct:.2f}%, نسبة الفوز: {win_rate*100:.1f}%")
        if total_trades > 0:
            ...
            logger.info(f"[نتائج backtest] متوسط الربح: {avg_profit:.2f}%, أعلى ربح: {max_profit:.2f}%, أعلى خسارة: {max_loss:.2f}%")
        else:
            logger.warning(f"[نتائج backtest] لم يتم تنفيذ أي صفقات! التحقق من إشارات التداول ومعايير الدخول/الخروج.")
    ...
    def _add_stop_loss(self, dataframe: pd.DataFrame, stop_loss_pct: float = 0.02) -> pd.DataFrame:
        """
        ...
        for i in range(1, len(df)):
            ...
            if df[buy_col].iloc[i-1] == 1 and not in_position:
                in_position = True
                entry_price = df['Close'].iloc[i] if 'Close' in df.columns else df['close'].iloc[i]
                
            # تحقق من وقف الخسارة
            if in_position:
                current_price = df['Close'].iloc[i] if 'Close' in df.columns else df['close'].iloc[i]
    ...
    def _identify_swing_points(self, dataframe: pd.DataFrame, window: int = 3, min_size: float = 0.003) -> pd.DataFrame:
        """
        ...
        for i in range(window, len(df) - window):
            ...
            if all(df['high'].iloc[i] > df['high'].iloc[i-j] for j in range(1, window+1)) and \
               all(df['high'].iloc[i] > df['high'].iloc[i+j] for j in range(1, window+1)):
                ...
                price_range = (df['high'].iloc[i] - min(df['low'].iloc[i-window:i+window+1])) / df['close'].iloc[i]
            ...
            if all(df['low'].iloc[i] < df['low'].iloc[i-j] for j in range(1, window+1)) and \
               all(df['low'].iloc[i] < df['low'].iloc[i+j] for j in range(1, window+1)):
                ...
                # التحقق من حجم التأرجح
                price_range = (max(df['high'].iloc[i-window:i+window+1]) - df['low'].iloc[i]) / df['close'].iloc[i]
                if price_range >= min_size:
                    df.loc[df.index[i], 'Swing_Low'] = 1
        
        return df
    
    def _calculate_bollinger_bands(self, dataframe: pd.DataFrame, window: int = 20, num_std: float = 2.0) -> pd.DataFrame:
        """
    ...
    def _calculate_stochastic(self, dataframe: pd.DataFrame, k_period: int = 14, d_period: int = 3) -> pd.DataFrame:
        """
        حساب مؤشر ستوكاستك
