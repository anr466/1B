...
class TrendFollowingStrategy(StrategyBase):
    ...
    def _identify_market_structure(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            natr = ta.natr(df_copy['high'], df_copy['low'], df_copy['close'], length=14)
            ...
            # استخدام pdta.minmax يحل محل الدوال اليدوية is_local_maxima/is_local_minima
            ...
            def is_local_maxima(data, i, window):
                if i - window < 0 or i + window >= len(data):
                    return False
                # القيمة الحالية يجب أن تكون أكبر من جميع القيم في النافذة المحيطة
                return all(data.iloc[i] >= data.iloc[i-j] for j in range(1, window+1)) and \
            ...
            def is_local_minima(data, i, window):
                if i - window < 0 or i + window >= len(data):
                    return False
                # القيمة الحالية يجب أن تكون أصغر من جميع القيم في النافذة المحيطة
                return all(data.iloc[i] <= data.iloc[i-j] for j in range(1, window+1)) and \
                       all(data.iloc[i] <= data.iloc[i+j] for j in range(1, window+1))
            ...
            for i in range(swing_length, len(df_copy) - swing_length):
                ...
                if is_local_maxima(df_copy['high'], i, swing_length):
                    ...
                    if i < len(natr) and not np.isnan(natr.iloc[i]) and natr.iloc[i] > volatility_threshold:
                        df_copy.loc[df_copy.index[i], 'swing_high'] = df_copy.iloc[i]['high']
                        df_copy.loc[df_copy.index[i], 'swing_high_confirmed'] = True
                
                # التحقق من القيعان (النقاط المنخفضة)
                if is_local_minima(df_copy['low'], i, swing_length):
                    # تحقق من أن التقلب كافٍ لاعتبار القاع مهمًا
                    if i < len(natr) and not np.isnan(natr.iloc[i]) and natr.iloc[i] > volatility_threshold:
                        df_copy.loc[df_copy.index[i], 'swing_low'] = df_copy.iloc[i]['low']
                        df_copy.loc[df_copy.index[i], 'swing_low_confirmed'] = True
                    
            # تحسين دقة القمم والقيعان باستخدام مؤشرات إضافية من pandas_ta
            # استخدام RSI لتأكيد نقاط التحول (تشبع شرائي عند القمم، تشبع بيعي عند القيعان)
            df_copy['rsi'] = ta.rsi(df_copy['close'], length=14)
            
            # تأكيد القمم باستخدام تشبع شرائي RSI
            high_idx = df_copy.index[df_copy['swing_high_confirmed'] == True]
            for idx in high_idx:
                i = df_copy.index.get_loc(idx)
                if df_copy['rsi'].iloc[i] < 50:  # إذا كان RSI لا يؤكد قمة (أقل من 50)، نقلل الثقة
                    df_copy.loc[idx, 'swing_high_confirmed'] = False
            
            # تأكيد القيعان باستخدام تشبع بيعي RSI
            low_idx = df_copy.index[df_copy['swing_low_confirmed'] == True]
            for idx in low_idx:
                i = df_copy.index.get_loc(idx)
                if df_copy['rsi'].iloc[i] > 50:  # إذا كان RSI لا يؤكد قاع (أعلى من 50)، نقلل الثقة
            ...
            logger.info(f"تم تحديد {df_copy['swing_high_confirmed'].sum()} قمة و {df_copy['swing_low_confirmed'].sum()} قاع")
            return df_copy
    ...
    def __init__(self, **params):
        ...
        # سجل التتبع
        logger.info(f"تم تهيئة استراتيجية متابعة الاتجاه مع المعلمات: {params}")
    ...
    def _get_default_higher_timeframes(self) -> List[str]:
        ...
        all_timeframes = ['1m', '3m', '5m', '15m', '30m', '1h', '2h', '4h', '6h', '8h', '12h', '1d', '3d', '1w']
        ...
        # الحصول على الإطار الزمني إما من معلمات الاستراتيجية أو استخدام قيمة افتراضية
    ...
    def calculate_indicators(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            timeframe = getattr(df, 'timeframe', '4h') if hasattr(df, 'attrs') else '4h'
            ...
            adapted_params = self._adapt_parameters(timeframe, avg_adx, volatility_ratio)
            ...
            adx = ta.adx(df['high'], df['low'], df['close'], length=adapted_params['adx_period'])
            ...
            for col in macd_cols:
                if 'MACD_' in col and '_Signal' not in col and 'hist' not in col:
                    df['macd'] = macd[col]
            ...
            if 'macd' not in df.columns:
                # إذا لم نجد الأعمدة المتوقعة، نستخدم الأعمدة الثلاثة الأولى من ناتج MACD
                if len(macd_cols) >= 3:
                    df['macd'] = macd[macd_cols[0]]
                    df['macd_signal'] = macd[macd_cols[1]]
                    df['macd_histogram'] = macd[macd_cols[2]]
                    logger.warning(f"استخدام أسماء الأعمدة البديلة للMACD: {macd_cols[:3]}")
                else:
                    logger.error(f"لم يتم العثور على أعمدة MACD كافية: {macd_cols}")
            ...
            supertrend_col = f"SUPERT_{adapted_params['supertrend_atr_period']}_{adapted_params['supertrend_factor']:.1f}"
            ...
            df['atr'] = ta.atr(df['high'], df['low'], df['close'], length=adapted_params['atr_period'])
            
            # 6. حساب مؤشر Chaikin Money Flow للتدفق المالي
            df['cmf'] = ta.cmf(df['high'], df['low'], df['close'], df['volume'], length=adapted_params['cmf_period'])
            
            # 7. حساب مؤشر RSI
            df['rsi'] = ta.rsi(df['close'], length=adapted_params.get('rsi_period', 14))
            ...
            for i in range(20, len(df)):
                ...
                # الاتجاه متذبذب/محايد
                    ...
                    # استخدام الاتجاه السابق كافتراضي للحفاظ على استقرار الإشارات
            ...
            self._adapted_params = self._adapt_parameters(timeframe, avg_adx, volatility_ratio)
            
            # تسجيل معلومات حول المؤشرات المحسوبة
            logger.info(f"تم حساب المؤشرات الفنية للإطار الزمني {timeframe} - متوسط ADX: {avg_adx:.2f}, نسبة التقلب: {volatility_ratio:.2f}%")
        ...
        if timeframe in mtf_relationships:
            ...
            higher_tfs = [tf for tf in mtf_relationships[timeframe] if tf is not None]
            logger.info(f"تم تحديد الأطر الزمنية الأعلى تلقائياً: {higher_tfs} للإطار الأساسي {timeframe}")
            return higher_tfs
        else:
            # إطار زمني غير معروف، استخدام قيم افتراضية آمنة
            default_higher_tfs = ['4h', '1d']
            logger.warning(f"إطار زمني غير معروف: {timeframe}، استخدام أطر زمنية افتراضية: {default_higher_tfs}")
            return default_higher_tfs
    
    def _adapt_parameters(self, timeframe: str, avg_adx: float, volatility_ratio: float) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # تحليل السوق لتحديد ما إذا كانت حالة السوق مترابطة، متجهة، أو متذبذبة
            ...
            # تحسين المعاملات للتداول السريع (Scalping) والتداول اليومي (Intraday)
            tf_multipliers = {
                # معاملات مخصصة للتداول السريع (Scalping)
                '1m': {'ema': 0.15, 'adx': 0.7, 'signal_quality': 0.6, 'stop_loss': 0.8, 'take_profit': 1.2, 'momentum': 1.5, 'noise_filter': 1.8},
                '3m': {'ema': 0.2, 'adx': 0.8, 'signal_quality': 0.62, 'stop_loss': 0.9, 'take_profit': 1.3, 'momentum': 1.4, 'noise_filter': 1.6},
                '5m': {'ema': 0.25, 'adx': 0.9, 'signal_quality': 0.65, 'stop_loss': 1.0, 'take_profit': 1.5, 'momentum': 1.3, 'noise_filter': 1.5},
                
                # معاملات مخصصة للتداول اليومي (Intraday)
                '15m': {'ema': 0.4, 'adx': 1.0, 'signal_quality': 0.7, 'stop_loss': 1.1, 'take_profit': 1.7, 'momentum': 1.2, 'noise_filter': 1.3},
                '30m': {'ema': 0.6, 'adx': 1.1, 'signal_quality': 0.72, 'stop_loss': 1.2, 'take_profit': 1.9, 'momentum': 1.1, 'noise_filter': 1.2},
                '1h': {'ema': 0.8, 'adx': 1.0, 'signal_quality': 0.7, 'stop_loss': 1.0, 'take_profit': 2.0, 'momentum': 1.0, 'noise_filter': 1.0},
                
                # معاملات الأطر الزمنية الأطول
                '2h': {'ema': 1.2, 'adx': 0.9, 'signal_quality': 0.68, 'stop_loss': 0.9, 'take_profit': 2.2, 'momentum': 0.9, 'noise_filter': 0.9},
                '4h': {'ema': 1.5, 'adx': 0.8, 'signal_quality': 0.65, 'stop_loss': 0.8, 'take_profit': 2.5, 'momentum': 0.8, 'noise_filter': 0.8},
                '6h': {'ema': 1.8, 'adx': 0.75, 'signal_quality': 0.63, 'stop_loss': 0.75, 'take_profit': 2.8, 'momentum': 0.7, 'noise_filter': 0.7},
                '12h': {'ema': 2.0, 'adx': 0.7, 'signal_quality': 0.6, 'stop_loss': 0.7, 'take_profit': 3.0, 'momentum': 0.6, 'noise_filter': 0.6},
                '1d': {'ema': 2.5, 'adx': 0.6, 'signal_quality': 0.55, 'stop_loss': 0.65, 'take_profit': 3.5, 'momentum': 0.5, 'noise_filter': 0.5},
                '3d': {'ema': 3.0, 'adx': 0.5, 'signal_quality': 0.5, 'stop_loss': 0.6, 'take_profit': 4.0, 'momentum': 0.4, 'noise_filter': 0.4},
                '1w': {'ema': 4.0, 'adx': 0.4, 'signal_quality': 0.45, 'stop_loss': 0.5, 'take_profit': 5.0, 'momentum': 0.3, 'noise_filter': 0.3}
            }
                
            # إذا لم يكن الإطار الزمني موجودًا، استخدم المعاملات الافتراضية
            multipliers = tf_multipliers.get(timeframe, {'ema': 1.0, 'adx': 1.0, 'signal_quality': 0.7, 'stop_loss': 1.0, 'take_profit': 2.0, 'momentum': 1.0, 'noise_filter': 1.0})
            
            # تعديل عتبة ADX حسب الإطار الزمني - تخفيض العتبة للإطار 4h لتحسين عدد الإشارات
            if timeframe == '4h':
                adapted_params['adx_threshold'] = max(15, min(35, self.params['adx_threshold'] * 0.75))
            else:
                adapted_params['adx_threshold'] = max(15, min(40, self.params['adx_threshold'] * multipliers['adx']))
            
            # تعديل معلمات MACD حسب الإطار الزمني
            adapted_params['macd_fast'] = int(max(6, round(self.params['macd_fast'] * multipliers['ema'])))
            adapted_params['macd_slow'] = int(max(13, round(self.params['macd_slow'] * multipliers['ema'])))
            adapted_params['macd_signal'] = int(max(4, round(self.params['macd_signal'] * multipliers['ema'])))
            
            # تعديل معلمات EMA حسب الإطار الزمني
            adapted_params['ema_short_period'] = int(max(5, round(self.params.get('ema_short_period', 8) * multipliers['ema'])))
            adapted_params['ema_medium_period'] = int(max(8, round(self.params.get('ema_medium_period', 13) * multipliers['ema'])))
            adapted_params['ema_long_period'] = int(max(13, round(self.params.get('ema_long_period', 21) * multipliers['ema'])))
            
            # تعديل الحد الأدنى لجودة الإشارة حسب الإطار الزمني
            if timeframe == '4h':
                # تعديل مخصص للإطار الزمني 4h لتحسين نسبة الفوز
                adapted_params['min_signal_quality'] = max(0.55, min(0.85, self.params['min_signal_quality'] * 0.95))
            else:
                adapted_params['min_signal_quality'] = max(0.5, min(0.9, self.params['min_signal_quality'] * multipliers['signal_quality']))
            
            # تعديل معلمات وقف الخسارة وجني الأرباح حسب الإطار الزمني
            if timeframe == '4h':
                # تحسين نسبة المخاطرة/المكافأة للإطار 4h
                adapted_params['stop_loss_atr'] = max(1.2, self.params['stop_loss_atr'] * 0.85)  # وقف خسارة أقل قليلاً
                adapted_params['take_profit_atr'] = max(2.5, self.params['take_profit_atr'] * 1.1)  # جني ربح أعلى لتحسين نسبة الربح/الخسارة
            else:
                adapted_params['stop_loss_atr'] = max(1.0, self.params['stop_loss_atr'] * multipliers['stop_loss'])
                adapted_params['take_profit_atr'] = max(2.0, self.params['take_profit_atr'] * multipliers['take_profit'])
            ...
            if volatility_ratio > 2.0:  # تقلب مرتفع
                adapted_params['stop_loss_atr'] = min(adapted_params['stop_loss_atr'] * 1.2, 3.5)
                adapted_params['take_profit_atr'] = min(adapted_params['take_profit_atr'] * 1.2, 7.0)
                adapted_params['supertrend_factor'] = min(adapted_params['supertrend_factor'] * 1.2, 4.5)
                adapted_params['min_signal_quality'] = min(adapted_params['min_signal_quality'] * 1.1, 0.9)
            elif volatility_ratio < 0.5:  # تقلب منخفض
                adapted_params['stop_loss_atr'] = max(adapted_params['stop_loss_atr'] * 0.8, 1.2)
                adapted_params['take_profit_atr'] = max(adapted_params['take_profit_atr'] * 0.8, 2.5)
                adapted_params['supertrend_factor'] = max(adapted_params['supertrend_factor'] * 0.8, 1.8)
                adapted_params['min_signal_quality'] = max(adapted_params['min_signal_quality'] * 0.9, 0.5)
                
            # 2. تكييف عتبة ADX حسب قوة الاتجاه المرصودة
            if avg_adx > 30:  # اتجاه قوي
                adapted_params['adx_threshold'] = max(adapted_params['adx_threshold'] * 0.9, 18.0)
            elif avg_adx < 15:  # اتجاه ضعيف
                adapted_params['adx_threshold'] = min(adapted_params['adx_threshold'] * 1.1, 35.0)
            
            # تعديلات خاصة للأطر الزمنية الصغيرة (سكالبينج)
            if timeframe in ['1m', '3m', '5m']:
                # تسريع الاستجابة للتغيرات السريعة
                adapted_params['rsi_period'] = max(7, int(self.params.get('rsi_period', 14) * 0.6))
                # زيادة حساسية التقاطعات
                adapted_params['ema_short_period'] = max(3, int(adapted_params['ema_short_period'] * 0.7))
                # تقليل عتبة ADX للسماح بإشارات أكثر
                adapted_params['adx_threshold'] = max(12, min(25, adapted_params['adx_threshold'] * 0.8))
                # تعديل نسبة المخاطرة/المكافأة للتداولات قصيرة المدى
                adapted_params['risk_reward_ratio'] = max(1.2, min(1.5, self.params.get('risk_reward_ratio', 2.0) * 0.75))
            
            logger.debug(f"تكييف المعلمات للإطار الزمني {timeframe}: " + 
                       f"نسبة التقلب={volatility_ratio:.2f}%, متوسط ADX={avg_adx:.2f}, " + 
                       f"EMA قصير={adapted_params['ema_short_period']}, " + 
                       f"عتبة ADX={adapted_params['adx_threshold']:.1f}, " + 
                       f"الحد الأدنى لجودة الإشارة={adapted_params['min_signal_quality']:.2f}")
    ...
    def _calculate_signal_quality(self, df: pd.DataFrame, row_idx: int, direction: int, timeframe: str = '4h') -> float:
        """
        ...
        try:
            ...
            # الأطر الزمنية الأكبر: وزن أكبر للاتجاه العام والحجم والمؤشرات طويلة المدى
            tf_weights = {
                # أوزان معدلة للسكالبينج - زيادة وزن حركة السعر والزخم السريع
                '1m': {'trend': 0.15, 'momentum': 0.35, 'volume': 0.15, 'price_action': 0.35},
                '3m': {'trend': 0.20, 'momentum': 0.35, 'volume': 0.15, 'price_action': 0.30},
                '5m': {'trend': 0.25, 'momentum': 0.30, 'volume': 0.15, 'price_action': 0.30},
                
                # أوزان معدلة للتداول اليومي - توازن بين الاتجاه وحركة السعر
                '15m': {'trend': 0.30, 'momentum': 0.30, 'volume': 0.15, 'price_action': 0.25},
                '30m': {'trend': 0.35, 'momentum': 0.25, 'volume': 0.15, 'price_action': 0.25},
                '1h': {'trend': 0.40, 'momentum': 0.25, 'volume': 0.15, 'price_action': 0.20},
                '2h': {'trend': 0.45, 'momentum': 0.25, 'volume': 0.15, 'price_action': 0.15},
                
                # أوزان معدلة للتداول المتوسط والطويل
                '4h': {'trend': 0.50, 'momentum': 0.20, 'volume': 0.15, 'price_action': 0.15},
                '6h': {'trend': 0.50, 'momentum': 0.20, 'volume': 0.20, 'price_action': 0.10},
                '8h': {'trend': 0.55, 'momentum': 0.20, 'volume': 0.20, 'price_action': 0.05},
                '12h': {'trend': 0.55, 'momentum': 0.20, 'volume': 0.20, 'price_action': 0.05},
                '1d': {'trend': 0.60, 'momentum': 0.15, 'volume': 0.20, 'price_action': 0.05},
                '3d': {'trend': 0.65, 'momentum': 0.10, 'volume': 0.25, 'price_action': 0.00},
                '1w': {'trend': 0.70, 'momentum': 0.10, 'volume': 0.20, 'price_action': 0.00},
            }
            ...
            if direction == 1:  # إشارة شراء
                ...
                # قياس قوة الاتجاه
                if 'ADX' in row and row['ADX'] > self._adapted_params['adx_threshold']:
                    trend_score += 0.4
                ...
                if 'macd' in row and 'macd_signal' in row:
                    if row['macd'] > row['macd_signal'] and row['macd'] > 0:
                        ...
                        if row['macd'] > row['macd_signal'] * 1.5 and row['macd'] > 0:
                            momentum_score += 0.1
                    elif row['macd'] > row['macd_signal']:
                        momentum_score += 0.5
                
                # توافق مؤشر Supertrend
                if 'supertrend_direction' in row and row['supertrend_direction'] == 1:
                    momentum_score += 0.3
                ...
                if row['volume'] > df['volume'].iloc[row_idx-5:row_idx].mean():
                    volume_score += 0.2
                
                # === قياس جودة حركة السعر ===
                # السعر فوق المتوسطات المتحركة
                if row['close'] > row['ema_short'] and row['close'] > row['ema_medium']:
                ...
                if row['close'] > row['open']:
                    candle_strength = (row['close'] - row['open']) / (row['high'] - row['low'] + 0.001)
                ...
                # قوة الشمعة الهبوطية
                if row['close'] < row['open']:
                    candle_strength = (row['open'] - row['close']) / (row['high'] - row['low'] + 0.001)
            ...
            # حساب جودة الإشارة النهائية باستخدام الأوزان المعدلة حسب الإطار الزمني
            quality = (
            ...
            timeframe_reliability = {
                '1m': 0.88, '3m': 0.90, '5m': 0.92, '15m': 0.94, '30m': 0.95, '1h': 0.97,
                '2h': 0.98, '4h': 1.0, '6h': 0.99, '8h': 1.0, '12h': 1.0,  # زيادة موثوقية الإطار الزمني 4h
                '1d': 1.0, '3d': 1.0, '1w': 1.0
            }
            reliability_factor = timeframe_reliability.get(timeframe, 0.98)  # 4h كقيمة افتراضية
            ...
            if timeframe in ['1m', '3m', '5m', '15m']:
                ...
                if direction == 1:  # إشارة شراء
                    ...
                    if row_idx > 5 and row['close'] > df['close'].iloc[row_idx-1] > df['close'].iloc[row_idx-2]:
                        # تعزيز للموجة الصاعدة
                        momentum_boost = 0.05
                        final_quality = min(final_quality + momentum_boost, 1.0)
                        
                        # تعزيز إضافي إذا كان هناك زيادة في الحجم
                        if 'volume' in row and row_idx > 1 and row['volume'] > df['volume'].iloc[row_idx-1] * 1.2:
                            volume_boost = 0.03
                            final_quality = min(final_quality + volume_boost, 1.0)
                else:  # إشارة بيع
            ...
            if 'atr' in df.columns:
                volatility_ratio = df.iloc[row_idx]['atr'] / df.iloc[row_idx]['close'] * 100
                ...
                if volatility_ratio > volatility_threshold:  # تقلب عالٍ
                    volatility_penalty = min(volatility_ratio / 20.0, max_penalty)  # عقوبة بحد أقصى متغير
                    final_quality = max(final_quality * (1.0 - volatility_penalty), 0.0)
    ...
    def _calculate_exit_points(self, df: pd.DataFrame, row_idx: int, direction: int) -> Tuple[float, float]:
        """
        ...
        try:
            ...
            for i in range(row_idx-1, max(0, row_idx-lookback), -1):
                if 'swing_high' in df.iloc[i] and not np.isnan(df.iloc[i]['swing_high']):
                    last_swing_high = df.iloc[i]['swing_high']
                    break
                    
            for i in range(row_idx-1, max(0, row_idx-lookback), -1):
                if 'swing_low' in df.iloc[i] and not np.isnan(df.iloc[i]['swing_low']):
                    last_swing_low = df.iloc[i]['swing_low']
                    break
            
            # تحديد وقف الخسارة بناء على القمم/القيعان
            if direction == 1:  # للشراء
                if not np.isnan(last_swing_low) and last_swing_low < close_price:
                    # تحقق من أن المسافة معقولة
                    sl_distance_pct = (close_price - last_swing_low) / close_price
                ...
                if not np.isnan(last_swing_high) and last_swing_high > close_price:
                    # تحقق من أن المسافة معقولة
                    sl_distance_pct = (last_swing_high - close_price) / close_price
            ...
            has_atr = atr_column in df.columns and not np.isnan(row[atr_column])
            ...
            if not has_atr:
                ...
                df_temp = df.copy()
                atr = ta.atr(df_temp['high'], df_temp['low'], df_temp['close'], length=atr_length)
                current_atr = atr.iloc[row_idx] if row_idx < len(atr) else atr.iloc[-1]
            else:
            ...
            if timeframe in ['1d', '3d', '1w']:
                ...
                sl_multiplier = 1.2
                tp_multiplier = 2.5
            elif timeframe in ['1m', '3m']:  # الأطر الزمنية السريعة جدًا للسكالبينج
                sl_multiplier = 0.6  # وقف خسارة أقل من سعر الدخول بنسبة من ATR
                tp_multiplier = 1.2  # نسبة مخاطرة/مكافأة متوازنة للتداول السريع
    ...
    def generate_signals(self, dataframe: pd.DataFrame, timeframe: str = '1h', candles_count: int = 100, 
                       mtf_data: Optional[Dict[str, pd.DataFrame]] = None) -> pd.DataFrame:
        """
        توليد إشارات الدخول والخروج بناءً على المؤشرات الفنية المحسوبة من pandas_ta
        """
        try:
            logger.info(f"بدء توليد إشارات التداول لاستراتيجية تتبع الاتجاه - الإطار الزمني: {timeframe}")
            ...
            for i in range(20, len(df)):
                try:
                    ...
                    if 'supertrend_direction' in df.columns:
                        buy_conditions.append(df['supertrend_direction'].iloc[i] == 1)
                    
                    # MACD إيجابي
                    if all(col in df.columns for col in ['macd', 'macd_signal']):
                        buy_conditions.append(df['macd'].iloc[i] > df['macd_signal'].iloc[i])
                    
                    # RSI ليس في تشبع شرائي
                    if 'rsi' in df.columns:
                        buy_conditions.append(df['rsi'].iloc[i] < rsi_overbought)
                    
                    # حجم جيد
                    if 'relative_volume' in df.columns:
                        buy_conditions.append(df['relative_volume'].iloc[i] > volume_threshold)
                    ...
                    # Supertrend هابط
                    if 'supertrend_direction' in df.columns:
                        sell_conditions.append(df['supertrend_direction'].iloc[i] == -1)
                    
                    # MACD سلبي
                    if all(col in df.columns for col in ['macd', 'macd_signal']):
                        sell_conditions.append(df['macd'].iloc[i] < df['macd_signal'].iloc[i])
                    
                    # RSI ليس في تشبع بيعي
                    if 'rsi' in df.columns:
                        sell_conditions.append(df['rsi'].iloc[i] > rsi_oversold)
                    
                    # حجم جيد
                    if 'relative_volume' in df.columns:
                        sell_conditions.append(df['relative_volume'].iloc[i] > volume_threshold)
                    
                    # تحديد عدد الشروط المطلوبة
                    required_conditions = max(3, len(buy_conditions) // 2)
                    
                    # توليد إشارة الشراء
                    if len(buy_conditions) >= required_conditions and sum(buy_conditions) >= required_conditions:
                        df.loc[df.index[i], 'buy_signal'] = 1
                        
                        # حساب وقف الخسارة وجني الأرباح
                        if 'atr' in df.columns:
                            atr_value = df['atr'].iloc[i]
                            df.loc[df.index[i], 'stop_loss'] = df['close'].iloc[i] - (2 * atr_value)
                            df.loc[df.index[i], 'take_profit'] = df['close'].iloc[i] + (3 * atr_value)
                    
                    # توليد إشارة البيع
                    elif len(sell_conditions) >= required_conditions and sum(sell_conditions) >= required_conditions:
                    ...
                    if 'trend' in df.columns and i > 0:
                        exit_conditions.append(df['trend'].iloc[i] != df['trend'].iloc[i-1])
                    
                    # انعكاس Supertrend
                    if 'supertrend_direction' in df.columns and i > 0:
                        exit_conditions.append(df['supertrend_direction'].iloc[i] != df['supertrend_direction'].iloc[i-1])
                    
                    # تقاطع MACD
                    if all(col in df.columns for col in ['macd', 'macd_signal']) and i > 0:
                        current_cross = df['macd'].iloc[i] - df['macd_signal'].iloc[i]
                        previous_cross = df['macd'].iloc[i-1] - df['macd_signal'].iloc[i-1]
                        exit_conditions.append(current_cross * previous_cross < 0)
            ...
            logger.info(f"تم توليد إشارات التداول بنجاح: {buy_count} إشارة شراء، {sell_count} إشارة بيع، {exit_count} إشارة خروج")
