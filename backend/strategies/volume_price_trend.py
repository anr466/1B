...
class VolumePriceTrendStrategy(StrategyBase):
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df["vpt_ma"] = (
                df["vpt"].rolling(self.params["vpt_ma_period"]).mean()
            )

            # 2. المتوسطات المتحركة للسعر
            df["price_ma_fast"] = ta.sma(
                df["close"], length=self.params["price_ma_fast"]
            )
            df["price_ma_slow"] = ta.sma(
                df["close"], length=self.params["price_ma_slow"]
            )

            # 3. متوسط الحجم
            df["volume_ma"] = (
                df["volume"].rolling(self.params["volume_ma_period"]).mean()
            )
            df["volume_ratio"] = df["volume"] / df["volume_ma"]

            # 4. مؤشر الزخم
            df["momentum"] = ta.mom(
                df["close"], length=self.params["momentum_period"]
            )
            ...
            # 6. قوة الاتجاه
            df["trend_strength"] = (
                np.abs(df["price_ma_fast"] - df["price_ma_slow"])
                / df["price_ma_slow"]
            )
            ...
            for i in range(self.params["vpt_ma_period"], len(df)):
                if (
                    df["vpt"].iloc[i] > df["vpt_ma"].iloc[i]
                    and df["vpt"].iloc[i - 1] <= df["vpt_ma"].iloc[i - 1]
                ):
    ...
    def generate_signals(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            for i in range(
                max(
                    self.params["price_ma_slow"], self.params["vpt_ma_period"]
                ),
                len(df),
            ):
                # شروط الشراء
                ...
                ma_cross_up = (
                    df["price_ma_fast"].iloc[i] > df["price_ma_slow"].iloc[i]
                    and df["price_ma_fast"].iloc[i - 1]
                    <= df["price_ma_slow"].iloc[i - 1]
                )
                ...
                buy_conditions.append(
                    ...
                    >= self.params["trend_strength_min"]
                )
