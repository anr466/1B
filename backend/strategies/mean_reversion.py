    ...
    def generate_signals(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            for i in range(self.params['sma_period'], len(df)):
                ...
                oversold_condition = (
                    df['close'].iloc[i] < df['bb_lower'].iloc[i] and  # تحت الحد الأدنى لـ BB
                    df['rsi'].iloc[i] < self.params['rsi_oversold'] and  # RSI في منطقة التشبع البيعي
                    df['price_deviation'].iloc[i] < -self.params['deviation_threshold']  # انحراف سلبي كبير
                )
                ...
                if oversold_condition and volume_confirm:
                    ...
                    df.loc[df.index[i], 'take_profit'] = df['sma'].iloc[i] * self.params['take_profit_ratio'] + current_price * (1 - self.params['take_profit_ratio'])
                
                # شروط البيع (انحراف إيجابي شديد)
                overbought_condition = (
                    df['close'].iloc[i] > df['bb_upper'].iloc[i] and  # فوق الحد الأعلى لـ BB
                    df['rsi'].iloc[i] > self.params['rsi_overbought'] and  # RSI في منطقة التشبع الشرائي
                    df['price_deviation'].iloc[i] > self.params['deviation_threshold']  # انحراف إيجابي كبير
                )
