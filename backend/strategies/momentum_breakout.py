...
class MomentumBreakoutStrategy(StrategyBase):
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            # 1. مستويات الكسر
            df['resistance'] = df['high'].rolling(self.params['breakout_period']).max()
            df['support'] = df['low'].rolling(self.params['breakout_period']).min()
            ...
            df['adx'] = adx['ADX_14'] if 'ADX_14' in adx.columns else adx.iloc[:, 0]
            ...
            for i in range(1, len(df)):
                if df['close'].iloc[i] > df['resistance'].iloc[i-1]:
                    strength = ((df['close'].iloc[i] - df['resistance'].iloc[i-1]) / df['resistance'].iloc[i-1]) * 100
                    df.loc[df.index[i], 'breakout_strength'] = strength
                elif df['close'].iloc[i] < df['support'].iloc[i-1]:
                    strength = ((df['support'].iloc[i-1] - df['close'].iloc[i]) / df['support'].iloc[i-1]) * 100
                    df.loc[df.index[i], 'breakout_strength'] = strength
    ...
    def generate_signals(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            for i in range(self.params['breakout_period'], len(df)):
                ...
                adx_ok = df['adx'].iloc[i] >= self.params.get('adx_min', 25) if 'adx' in df.columns else True
                bullish_breakout = (
                    df['close'].iloc[i] > df['resistance'].iloc[i-1] and  # كسر المقاومة
                    df['volume_ratio'].iloc[i] >= self.params['volume_multiplier'] and  # حجم مرتفع
                    df['rsi'].iloc[i] >= self.params['rsi_breakout_min'] and  # زخم إيجابي
                    df['breakout_strength'].iloc[i] >= self.params['min_breakout_strength'] and  # قوة كسر كافية
                    adx_ok  # اتجاه قوي
                )
                ...
                bearish_breakout = (
                    df['close'].iloc[i] < df['support'].iloc[i-1] and  # كسر الدعم
                    df['volume_ratio'].iloc[i] >= self.params['volume_multiplier'] and  # حجم مرتفع
                    df['rsi'].iloc[i] <= self.params['rsi_breakout_max'] and  # زخم سلبي
                    df['breakout_strength'].iloc[i] >= self.params['min_breakout_strength'] and  # قوة كسر كافية
                    adx_ok  # اتجاه قوي
                )
