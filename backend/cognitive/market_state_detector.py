...
class MarketStateDetector:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.thresholds = {
            ...
            'adx_very_strong_trend': self.config.get('adx_very_strong_trend', 40),
            
            # RSI Extremes
            'rsi_overbought': self.config.get('rsi_overbought', 70),
            'rsi_oversold': self.config.get('rsi_oversold', 30),
            'rsi_extreme_overbought': self.config.get('rsi_extreme_overbought', 80),
            'rsi_extreme_oversold': self.config.get('rsi_extreme_oversold', 20),
            
            # Volatility
            'volatility_high': self.config.get('volatility_high', 0.05),  # 5%
            'volatility_low': self.config.get('volatility_low', 0.015),   # 1.5%
    ...
    def detect_state(self, df: pd.DataFrame, symbol: str = "") -> MarketStateResult:
        """
        ...
        try:
            ...
            self.logger.info(
                f"🔍 [{symbol}] Market State: {state.value} | "
                f"Confidence: {confidence:.1f}% | ADX: {indicators.get('adx', 0):.1f}"
            )
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if ta:
                ...
                indicators['rsi'] = rsi.iloc[-1] if rsi is not None and len(rsi) > 0 else 50
            else:
            ...
            if ta:
                ...
                if adx_df is not None and 'ADX_14' in adx_df.columns:
                    ...
                    indicators['plus_di'] = adx_df['DMP_14'].iloc[-1] if 'DMP_14' in adx_df.columns else 0
                    indicators['minus_di'] = adx_df['DMN_14'].iloc[-1] if 'DMN_14' in adx_df.columns else 0
                else:
            ...
            if ta:
                ...
                if bb is not None:
                    indicators['bb_upper'] = bb['BBU_20_2.0'].iloc[-1] if 'BBU_20_2.0' in bb.columns else current_price * 1.02
                    indicators['bb_lower'] = bb['BBL_20_2.0'].iloc[-1] if 'BBL_20_2.0' in bb.columns else current_price * 0.98
                    indicators['bb_mid'] = bb['BBM_20_2.0'].iloc[-1] if 'BBM_20_2.0' in bb.columns else indicators['sma_20']
                    indicators['bb_width'] = (indicators['bb_upper'] - indicators['bb_lower']) / indicators['bb_mid']
                else:
            ...
            if ta:
                ...
                indicators['atr'] = atr.iloc[-1] if atr is not None and len(atr) > 0 else 0
            else:
                indicators['atr'] = self._calculate_atr_manual(df)
            
            indicators['volatility'] = indicators['atr'] / current_price if current_price > 0 else 0
            
            # MACD
            if ta:
                macd_df = ta.macd(df['close'])
                if macd_df is not None:
                    indicators['macd'] = macd_df['MACD_12_26_9'].iloc[-1] if 'MACD_12_26_9' in macd_df.columns else 0
                    indicators['macd_signal'] = macd_df['MACDs_12_26_9'].iloc[-1] if 'MACDs_12_26_9' in macd_df.columns else 0
                    indicators['macd_hist'] = macd_df['MACDh_12_26_9'].iloc[-1] if 'MACDh_12_26_9' in macd_df.columns else 0
                else:
            ...
            # Price change
            indicators['price_change_1d'] = (df['close'].iloc[-1] - df['close'].iloc[-2]) / df['close'].iloc[-2] * 100 if len(df) > 1 else 0
            indicators['price_change_5d'] = (df['close'].iloc[-1] - df['close'].iloc[-5]) / df['close'].iloc[-5] * 100 if len(df) > 5 else 0
        ...
        try:
            ...
            if len(structure['swing_highs']) >= 2:
                for i in range(1, len(structure['swing_highs'])):
                    if structure['swing_highs'][i]['price'] > structure['swing_highs'][i-1]['price']:
                        structure['hh_count'] += 1
                    else:
                        structure['lh_count'] += 1
                structure['last_swing_high'] = structure['swing_highs'][-1]['price']
            
            if len(structure['swing_lows']) >= 2:
                for i in range(1, len(structure['swing_lows'])):
                    if structure['swing_lows'][i]['price'] > structure['swing_lows'][i-1]['price']:
                        structure['hl_count'] += 1
                    else:
                        structure['ll_count'] += 1
                structure['last_swing_low'] = structure['swing_lows'][-1]['price']
    ...
    def _cluster_prices(self, prices: np.ndarray, threshold: float = 0.02) -> List[List[float]]:
    ...
    def _classify_state(
        ...
        if adx >= self.thresholds['adx_strong_trend']:
            if structure['trend_structure'] == 'bullish' or (plus_di > minus_di and price_vs_sma20 > 0):
                scores[MarketState.UPTREND] += 35
                reasons.append(f"ADX قوي ({adx:.1f}) + هيكل صاعد")
            elif structure['trend_structure'] == 'bearish' or (minus_di > plus_di and price_vs_sma20 < 0):
    ...
    def _calculate_adx_manual(self, df: pd.DataFrame, period: int = 14) -> Dict:
    ...
    def _calculate_rsi_manual(self, close: pd.Series, period: int = 14) -> float:
        """حساب RSI يدوياً"""
        delta = close.diff()
...
def get_market_state_detector(config: Optional[Dict] = None) -> MarketStateDetector:
