...
class CognitiveOrchestrator:
    ...
    def analyze_entry(self, symbol: str, df_4h: pd.DataFrame,
                      df_1h: Optional[pd.DataFrame] = None,
                      df_15m: Optional[pd.DataFrame] = None) -> CognitiveDecision:
        """
        ...
        try:
            ...
            if market_state.state in [MarketState.DOWNTREND, MarketState.NEAR_TOP]:
                return self._stay_out(
                    symbol,
            ...
            if surveillance.opportunity_score < self.min_opportunity_score:
                return self._stay_out(
                    symbol,
                    f"Opportunity too low: {surveillance.opportunity_score:.0f}% < {self.min_opportunity_score}%",
                    market_state=market_state.state.value,
                    market_quality=surveillance.market_quality.value,
                    market_phase=surveillance.market_phase.value,
            ...
            if surveillance.risk_score > self.max_risk_score:
                return self._stay_out(
                    symbol,
                    f"Risk too high: {surveillance.risk_score:.0f}% > {self.max_risk_score}%",
                    market_state=market_state.state.value,
                    market_quality=surveillance.market_quality.value,
                    market_phase=surveillance.market_phase.value,
                    opportunity_score=surveillance.opportunity_score,
            ...
            entry_logic = (
                ...
                f"Market: {market_state.state.value} (conf {market_state.confidence:.0f}%) | "
    ...
    def analyze_exit(self, symbol: str, df: pd.DataFrame,
                     position: Dict) -> CognitiveDecision:
        """
        ...
        try:
            ...
            if exit_decision.should_exit:
                ...
                return CognitiveDecision(
                    ...
                    exit_logic=f"Exit {exit_pct*100:.0f}%: {exit_decision.primary_reason.value}",
                    warnings=surveillance.warnings
                )
            ...
            return CognitiveDecision(
                ...
                market_state="error", market_quality="error",
                market_phase="error", entry_strategy=EntryStrategy.NONE,
                reasoning=f"Error: {e}"
            )
    ...
    def _select_best_entry(self, symbol: str, df_4h: pd.DataFrame,
    ...
    def _check_trend_continuation(self, df_4h: pd.DataFrame,
                                   df_1h: Optional[pd.DataFrame],
                                   market_state: MarketStateResult) -> Optional[Dict]:
        """
        ...
        try:
            ...
            ema_aligned = (
                ema_8.iloc[-1] > ema_21.iloc[-1] > ema_55.iloc[-1] and
                current > ema_8.iloc[-1]
            )
            ...
            # Volume confirmation
            vol = df_4h['volume']
            vol_ratio = vol.iloc[-1] / vol.rolling(20).mean().iloc[-1] if len(vol) >= 20 else 1
            vol_ok = vol_ratio > 0.8
            
            if ema_aligned and strong_trend and momentum_ok and vol_ok:
                confidence = min(88, 48 + market_state.trend_strength * 0.8 + market_state.momentum * 0.2)
                if near_ema:
                    confidence += 8
                if vol_ratio > 1.2:
                    confidence += 5
    ...
    def _check_pullback_entry(self, df_4h: pd.DataFrame,
                               df_1h: Optional[pd.DataFrame],
                               market_state: MarketStateResult) -> Optional[Dict]:
        """
    ...
    def _check_breakout_entry(self, df_4h: pd.DataFrame,
                               market_state: MarketStateResult,
                               surveillance: SurveillanceReport) -> Optional[Dict]:
        """
        ...
        try:
            ...
            prev_bb_width = (2 * std_20 / sma_20).iloc[-5] if len(close) > 5 else bb_width
            squeeze_release = bb_width > prev_bb_width * 1.3
            
            # Must have both breakout + volume, AND either squeeze release or very strong volume
            ...
            if breakout and vol_confirmed and (squeeze_release or strong_vol):
                ...
                return {
                    ...
                    'reasons': [
                        ...
                        "BB squeeze release" if squeeze_release else "Normal breakout"
                    ]
                }
            return None
        except Exception as e:
    ...
    def _check_volatility_expansion(self, df_4h: pd.DataFrame,
                                     market_state: MarketStateResult) -> Optional[Dict]:
        """
        ...
        try:
            ...
            price_rising = close.iloc[-1] > close.iloc[-3]  # Price rising over last 3 bars
            ...
            if expanding and bullish_expansion and price_rising and vol_ratio > 1.1 and rsi_ok:
    ...
    def _check_reversal_entry(self, df_4h: pd.DataFrame,
                               df_1h: Optional[pd.DataFrame],
                               market_state: MarketStateResult) -> Optional[Dict]:
        """
        ...
        try:
            ...
            lower_wick = min(close.iloc[-1], df_4h['open'].iloc[-1]) - df_4h['low'].iloc[-1]
            hammer = lower_wick > body * 2  # Long lower wick
            
            confirmations = sum([rsi_oversold, divergence, vol_spike, bullish_candle, hammer])
            ...
            if confirmations >= 3:
                ...
                reasons = []
                if rsi_oversold: reasons.append(f"RSI oversold ({rsi.iloc[-1]:.0f})")
                if divergence: reasons.append("Bullish divergence")
                if vol_spike: reasons.append(f"Volume spike ({vol_ratio:.1f}x)")
                if hammer: reasons.append("Hammer pattern")
    ...
    def _adjust_confidence(self, base_confidence: float,
                           market_state: MarketStateResult,
                           surveillance: SurveillanceReport) -> float:
    ...
    def _calculate_position_size(self, confidence: float, risk_score: float,
                                  market_state: MarketStateResult) -> float:
    ...
    def _define_invalidation(self, strategy: EntryStrategy, entry_price: float,
                              stop_loss: float, market_state: MarketStateResult) -> str:
        """تحديد نقطة الإبطال"""
        sl_pct = abs(entry_price - stop_loss) / entry_price * 100
        ...
        return invalidation_map.get(strategy, f"SL at ${stop_loss:.2f} (-{sl_pct:.1f}%)")
    ...
    def record_trade_result(self, symbol: str, strategy: str,
                            ...
                            market_state: str, exit_reason: str):
...
def get_cognitive_orchestrator(config: Optional[Dict] = None) -> CognitiveOrchestrator:
