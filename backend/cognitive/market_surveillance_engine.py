...
class MarketSurveillanceEngine:
    ...
    def survey(
        ...
        try:
            ...
            self.logger.info(f"👁️ [{symbol}] Surveillance: {
                market_quality.value} | " f"Phase: {
                market_phase.value} | Opp: {
                opportunity_score:.0f}% | " f"Risk: {
                risk_score:.0f}% | Trade: {
    ...
    def _check_structure_integrity(self, df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            bullish = (
                swing_high_1 >= swing_high_2 and swing_low_1 >= swing_low_2
            )
    ...
    def _detect_market_phase(self, df: pd.DataFrame) -> MarketPhase:
        ...
        try:
            ...
            price_change = (
                (close.iloc[-1] - close.iloc[-20]) / close.iloc[-20] * 100
            )
            ...
            vol_change = (
                (vol_late - vol_early) / vol_early if vol_early > 0 else 0
            )
            ...
            # Accumulation: Price flat/down, volume increasing, volatility
            # contracting
            ...
            # Distribution: Price flat/up slightly, volume decreasing,
    ...
    def _detect_behavior_changes(
        ...
        try:
            ...
            if len(close) > 10:
                ...
                macd = ema_12 - ema_26
                macd_falling = (
                    macd.iloc[-1] < macd.iloc[-5] if len(macd) > 5 else False
                )
            ...
            if (
                trend_health > 60
                and momentum_health > 60
                and volatility_state in ["normal", "low"]
            ):
    ...
    def _empty_report(self, symbol: str, reason: str) -> SurveillanceReport:
        return SurveillanceReport(
            symbol=symbol,
            timestamp=datetime.now(),
            market_quality=MarketQuality.POOR,
            market_phase=MarketPhase.UNKNOWN,
            behavior_signals=[BehaviorSignal.NONE],
            is_tradeable=False,
            opportunity_score=0,
            risk_score=100,
            trend_health=0,
