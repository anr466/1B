...
class WeaknessExitEngine:
    ...
    def evaluate(self, df: pd.DataFrame, position: Dict) -> ExitSignal:
        try:
            ...
            ema_diff_prev = (ema_8.iloc[-3] - ema_21.iloc[-3]) / ema_21.iloc[-3] if len(close) > 3 else ema_diff
            ...
            rs = gain / loss
            rsi = 100 - (100 / (1 + rs))
            rsi_declining = rsi.iloc[-1] < rsi.iloc[-3] if len(rsi) > 3 else False
            ...
            hist_shrinking = (
                len(hist) > 3 and 
                hist.iloc[-1] < hist.iloc[-2] < hist.iloc[-3] and
                hist.iloc[-1] > 0
            )
            ...
            if pnl_pct > 0.01 and weakness_score >= 50:
                ...
                return ExitSignal(
                    ...
                    urgency=ExitUrgency.MEDIUM if weakness_score > 70 else ExitUrgency.LOW,
                    confidence=weakness_score,
                    exit_pct=exit_pct,
                    reasoning=f"Trend weakening: {', '.join(reasons)}"
                )
            
            return ExitSignal("WeaknessExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, "Trend healthy")
            
        except Exception as e:
            logger.debug(f"WeaknessExit error: {e}")
            return ExitSignal("WeaknessExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, f"Error: {e}")
...
class StructureBreakExitEngine:
    ...
    def evaluate(self, df: pd.DataFrame, position: Dict) -> ExitSignal:
        try:
            ...
            structure_broken = current < swing_low and swing_low < prev_swing_low
            
            # Also check if price broke below key EMA
            ema_55 = close.ewm(span=55).mean().iloc[-1] if len(close) >= 55 else close.mean()
            ...
            if structure_broken:
                return ExitSignal(
                    ...
                    reasoning=f"Structure broken: price {current:.2f} < swing low {swing_low:.2f}"
                )
            ...
            return ExitSignal("StructureBreakExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, "Structure intact")
            
        except Exception as e:
            logger.debug(f"StructureBreakExit error: {e}")
            return ExitSignal("StructureBreakExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, f"Error: {e}")
...
class VolatilityShiftExitEngine:
    ...
    def evaluate(self, df: pd.DataFrame, position: Dict) -> ExitSignal:
        try:
            ...
            if atr_baseline == 0:
                return ExitSignal("VolatilityShiftExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, "No data")
            ...
            if volatility_ratio > 2.5:
                return ExitSignal(
                    ...
                    confidence=80,
                    exit_pct=0.5,
                    reasoning=f"Extreme volatility expansion: {volatility_ratio:.1f}x normal"
                )
                ...
                return ExitSignal(
                    ...
                    confidence=60,
                    exit_pct=0.3,
                    reasoning=f"High volatility expansion: {volatility_ratio:.1f}x normal"
                )
            
            return ExitSignal("VolatilityShiftExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, 
                            f"Volatility normal ({volatility_ratio:.1f}x)")
            
        except Exception as e:
            logger.debug(f"VolatilityShiftExit error: {e}")
            return ExitSignal("VolatilityShiftExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, f"Error: {e}")
    ...
    def evaluate(self, df: pd.DataFrame, position: Dict) -> ExitSignal:
        try:
            ...
            if rsi.iloc[-1] < 45 and rsi.iloc[-3] > 55:
                confirmations += 1
                reasons.append(f"RSI reversal ({rsi.iloc[-3]:.0f}→{rsi.iloc[-1]:.0f})")
            
            # 3. Price crossed below EMA21
            ema_21 = close.ewm(span=21).mean()
            if close.iloc[-1] < ema_21.iloc[-1] and close.iloc[-2] >= ema_21.iloc[-2]:
                confirmations += 1
            ...
            return ExitSignal("ReversalExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, "No reversal signals")
            
        except Exception as e:
            logger.debug(f"ReversalExit error: {e}")
            return ExitSignal("ReversalExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, f"Error: {e}")
...
class EmergencyExitEngine:
    ...
    def evaluate(self, df: pd.DataFrame, position: Dict) -> ExitSignal:
        try:
            ...
            if len(df) > 3:
                recent_drop = (df['close'].iloc[-1] - df['close'].iloc[-3]) / df['close'].iloc[-3]
                if recent_drop < -0.05:
                    return ExitSignal(
                        engine_name="EmergencyExit",
                        reason=ExitReason.EMERGENCY,
                        ...
                        confidence=95,
                        exit_pct=1.0,
                        reasoning=f"Flash crash detected: {recent_drop*100:.1f}% in 3 candles"
                    )
            ...
            if hold_hours > self.max_hold_hours:
                if abs(pnl_pct) < 0.005:  # Less than 0.5% movement
                    return ExitSignal(
                        ...
                        confidence=70,
                        exit_pct=1.0,
                        reasoning=f"Stagnant position: {hold_hours:.0f}h with {pnl_pct*100:.2f}% PnL"
                    )
            ...
            return ExitSignal("EmergencyExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, "No emergency")
            
        except Exception as e:
            logger.debug(f"EmergencyExit error: {e}")
            return ExitSignal("EmergencyExit", ExitReason.HOLD, ExitUrgency.NONE, 0, 0, f"Error: {e}")
...
class MultiExitEngine:
    ...
    def evaluate_exit(self, df: pd.DataFrame, position: Dict) -> MultiExitDecision:
        """
        ...
        try:
            ...
            active_signals = [s for s in signals if s.reason != ExitReason.HOLD]
            
            if not active_signals:
                return MultiExitDecision(
                    should_exit=False, exit_pct=0,
                    primary_reason=ExitReason.HOLD,
                    urgency=ExitUrgency.NONE, confidence=0,
                    signals=signals,
            ...
            # بناء التحليل
            reasoning_parts = [f"{s.engine_name}: {s.reasoning}" for s in active_signals[:3]]
