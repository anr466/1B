...
class MTFReversalConfirmation:
    ...
    def confirm_bullish_reversal(
        self, df_1h: pd.DataFrame, df_15m: pd.DataFrame, current_price: float
    ) -> ReversalSignal:
        """
        ...
        try:
            ...
            indicators_aligned = (
                reversal_1h["indicators"] + reversal_15m["indicators"]
            )
            ...
            return ReversalSignal(
                type=(
                    ...
                    if len(confirmed_timeframes) > 0
                    else ReversalType.NONE
                ),
                strength=strength,
    ...
    def _detect_bullish_reversal_signals(
        ...
        try:
            ...
            if candle_patterns["detected"]:
                ...
                signals["candles_since_reversal"] = candle_patterns[
                    "candles_since"
                ]
            ...
            for i in range(1, min(11, len(df))):
                ...
                if body > 0:
                    if (
                        ...
                        result["candles_since"] = min(
                            result["candles_since"], i
                        )
                ...
                if idx > -len(df) + 1:
                    prev_body = abs(
                        df["close"].iloc[idx - 1] - df["open"].iloc[idx - 1]
                    )
    ...
    def _check_bullish_indicators(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if "macd" in df.columns and "macd_signal" in df.columns:
                ...
                prev_signal = (
                    df["macd_signal"].iloc[-2] if len(df) > 1 else signal
                )
