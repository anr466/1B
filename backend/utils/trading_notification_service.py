...
try:
    from backend.utils.firebase_notification_service import (
...
class TradingNotificationService:
                        ...
                        default_settings.update(
                            {
                                ...
                                "notify_new_deal": bool(
                                    parsed.get(
                                        "notifyNewDeal",
                                        default_settings["notify_new_deal"],
                                    )
                                ),
                                "notify_deal_profit": bool(
                                    parsed.get(
                                        "notifyDealProfit",
                                        default_settings["notify_deal_profit"],
                                    )
                                ),
                                "notify_deal_loss": bool(
                                    parsed.get(
                                        "notifyDealLoss",
                                        default_settings["notify_deal_loss"],
                                    )
                                ),
                                ...
                                "notify_daily_loss": bool(
                                    parsed.get(
                                        "notifyDailyLoss",
                                        default_settings["notify_daily_loss"],
                                    )
                                ),
                                "notify_low_balance": bool(
                                    parsed.get(
                                        "notifyLowBalance",
                                        default_settings["notify_low_balance"],
                                    )
                                ),
                                "daily_summary": bool(
                                    parsed.get(
                                        "dailySummary",
                                        default_settings["daily_summary"],
                                    )
                                ),
                                "cumulative_loss_alert_enabled": bool(
                                    parsed.get(
                                        "cumulativeLossAlertEnabled",
                                        default_settings[
                                            "cumulative_loss_alert_enabled"
                                        ],
                                    )
                                ),
                                "cumulative_loss_threshold_usd": float(
                                    parsed.get(
                                        "cumulativeLossThresholdUsd",
                                        default_settings[
                                            "cumulative_loss_threshold_usd"
                                        ],
                                    )
                                ),
    ...
    def is_quiet_hours(self, settings: Dict) -> bool:
        ...
        try:
            ...
            start = datetime.strptime(
                settings.get("quiet_hours_start", "22:00"), "%H:%M"
            ).time()
            end = datetime.strptime(
                settings.get("quiet_hours_end", "08:00"), "%H:%M"
            ).time()
    ...
    def should_send_notification(self, user_id: int, notification_type: str) -> bool:
        ...
        setting_key = notification_settings_map.get(
            notification_type, "system_notifications"
        )
    ...
    def save_notification_to_history(
        self,
        user_id: int,
        notification_type: str,
        title: str,
    ...
    def notify_trade_opened(
        ...
        if self.is_duplicate_notification(
            user_id, "trade_opened", unique_key, cooldown_minutes=60
        ):
            return False
    ...
    def notify_trailing_stop_activated(
        self,
        user_id: int,
        symbol: str,
        ...
        self.save_notification_to_history(
            user_id, "trailing_stop_activated", title, body, data
        )
    ...
    def notify_daily_loss_limit(
        self, user_id: int, daily_loss: float, daily_limit: float
    ) -> bool:
        ...
        # حفظ في السجل فقط (بدون إشعار Push)
        self.save_notification_to_history(
            user_id, "daily_loss_limit", title, body, data
        )

        # لا يتم إرسال إشعار Push - النظام يتعامل تلقائياً
        return True

    def notify_balance_low(
        self, user_id: int, current_balance: float, min_required: float
    ) -> bool:
    ...
    def notify_low_free_balance_simple(
        self, user_id: int, free_balance: float, open_positions: int = 0
    ) -> bool:
        """
    ...
    def notify_system_alert(
        self, user_id: int, message: str, alert_type: str = "info"
    ) -> bool:
        """إشعار تنبيه النظام"""
