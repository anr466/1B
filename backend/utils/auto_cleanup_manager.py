...
class AutoCleanupManager:
                ...
                for file_path in path.rglob(pattern):
                    ...
                    for exc_pattern in patterns:
                        if exc_pattern.startswith('!'):
                            if glob.fnmatch.fnmatch(file_path.name, exc_pattern[1:]):
                                excluded = True
                                break
    ...
    def cleanup_logs(self, max_age_days: int = 7, keep_latest: int = 1) -> Dict:
        """
        تنظيف ملفات السجلات
    ...
    def get_cleanup_report(self) -> str:
        ...
        report.append(f"   - الملفات المحذوفة: {self.stats.get('files_deleted', 0)}")
        report.append(f"   - المجلدات المحذوفة: {self.stats.get('directories_deleted', 0)}")
        report.append(f"   - المساحة المحررة: {self.stats.get('space_freed', 0) / 1024 / 1024:.2f} MB")
        report.append("")
            ...
            for category, details in self.stats['details'].items():
                if details.get('deleted'):
                    ...
                    if len(details['deleted']) > 5:
                        report.append(f"      ... و {len(details['deleted']) - 5} ملفات أخرى")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dry-run', action='store_true', help='معاينة بدون حذف')
    parser.add_argument('--report', action='store_true', help='عرض التقرير فقط')
