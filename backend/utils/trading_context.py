...
def get_effective_is_demo(db, user_id: int, requested_mode: Optional[str] = None) -> int:
    if not is_admin_user(db, user_id):
        return 0

    if requested_mode in {'demo', 'real'}:
        return 1 if requested_mode == 'demo' else 0

    return 0 if get_effective_trading_mode(db, user_id) == 'real' else 1


def get_trading_context(db, user_id: int, requested_mode: Optional[str] = None) -> Dict[str, Any]:
    is_admin = is_admin_user(db, user_id)
    effective_is_demo = get_effective_is_demo(db, user_id, requested_mode=requested_mode)
    trading_mode = requested_mode if requested_mode in {'demo', 'real'} else get_effective_trading_mode(db, user_id)
