...
class RateLimiterHelper:
    ...
    def is_rate_limited(
        self, ip: str, endpoint: str, limit: int, window_seconds: int
    ) -> bool:
        """
        ...
        with self.lock:
            ...
            if key in self.requests:
                ...
                self.requests[key] = [
                    (ts, ep)
                    for ts, ep in self.requests[key]
                    if ts > cutoff_time
                ]
            else:
                self.requests[key] = []
    ...
    def get_remaining(
        ...
        with self.lock:
            ...
            cutoff_time = now - timedelta(seconds=window_seconds)
            valid_requests = [
                (ts, ep) for ts, ep in self.requests[key] if ts > cutoff_time
            ]
...
def rate_limit(
    limit: int = 100, window_seconds: int = 60, endpoint_name: str = None
):
    """
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if rate_limiter.is_rate_limited(
                ip, endpoint, limit, window_seconds
            ):
...
def rate_limit_data(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        return rate_limit(
            limit=300, window_seconds=60, endpoint_name=endpoint
        )(f)(*args, **kwargs)
