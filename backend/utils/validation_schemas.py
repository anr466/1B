...
class ValidationResult:
    ...
    def __init__(
        self, is_valid: bool = True, errors: List[ValidationError] = None
    ):
...
class Validator:
    ...
    def validate_string(
        ...
        if len(value) < min_length:
            return (
                False,
                f"{field_name} يجب أن يكون على الأقل {min_length} أحرف",
            )
    ...
    @staticmethod
    def validate_email(
        value: Any, field_name: str = "email"
    ) -> Tuple[bool, str]:
    ...
    @staticmethod
    def validate_symbol(
        value: Any, field_name: str = "symbol"
    ) -> Tuple[bool, str]:
    ...
    def validate_percentage(
        value: Any, field_name: str = "percentage"
    ) -> Tuple[bool, str]:
    ...
    def validate_list(
        ...
        if len(value) < min_items:
            return (
                False,
                f"{field_name} يجب أن تحتوي على الأقل {min_items} عناصر",
            )

        if max_items and len(value) > max_items:
            return (
                False,
                f"{field_name} يجب ألا تحتوي على أكثر من {max_items} عناصر",
            )
...
class TradeValidator:
    ...
    def validate_buy_order(
        symbol: str, quantity: float, price: float, user_id: int = None
    ) -> ValidationResult:
        ...
        if user_id is not None:
            if not isinstance(user_id, int) or user_id <= 0:
                result.add_error(
                    "user_id",
                    "معرف المستخدم يجب أن يكون رقماً موجباً",
                    user_id,
                )
        ...
        if total_value < 1:
            result.add_error(
                "total_value",
                "قيمة الصفقة يجب أن تكون على الأقل 1 USDT",
                total_value,
            )
    ...
    def validate_sell_order(
        ...
        return TradeValidator.validate_buy_order(
            symbol, quantity, price, user_id
        )
...
class APIValidator:
    ...
    def validate_api_key(api_key: str) -> Tuple[bool, str]:
        ...
        is_valid, error = Validator.validate_string(
            api_key, "api_key", min_length=20, max_length=200
        )
        if not is_valid:
            return False, error
        return True, ""
    ...
    def validate_api_secret(api_secret: str) -> Tuple[bool, str]:
        ...
        is_valid, error = Validator.validate_string(
            api_secret, "api_secret", min_length=20, max_length=200
        )
        if not is_valid:
            return False, error
        return True, ""
...
class StrategyValidator:
    ...
    def validate_strategy_name(strategy_name: str) -> Tuple[bool, str]:
        ...
        is_valid, error = Validator.validate_choice(
            strategy_name, StrategyValidator.VALID_STRATEGIES, "strategy_name"
        )
        if not is_valid:
            return False, error
        return True, ""
    ...
    def validate_timeframe(timeframe: str) -> Tuple[bool, str]:
        ...
        is_valid, error = Validator.validate_choice(
            timeframe, StrategyValidator.VALID_TIMEFRAMES, "timeframe"
        )
        if not is_valid:
            return False, error
        return True, ""
    ...
    def validate_strategy_params(
        strategy_name: str, timeframe: str, params: Dict = None
    ) -> ValidationResult:
        """التحقق من صحة معاملات الاستراتيجية"""
        result = ValidationResult()

        is_valid, error = StrategyValidator.validate_strategy_name(
            strategy_name
        )
        if not is_valid:
        ...
        if params:
            if not isinstance(params, dict):
                result.add_error(
                    "params", "المعاملات يجب أن تكون قاموساً", type(params)
                )
...
class UserValidator:
    ...
    def validate_password(password: str) -> Tuple[bool, str]:
        ...
        is_valid, error = Validator.validate_string(
            password, "password", min_length=8, max_length=128
        )
        if not is_valid:
            return False, error
...
def validate_input(data: Dict, schema: Dict[str, Dict]) -> ValidationResult:
    """
