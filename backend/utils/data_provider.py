...
try:
    from backend.utils.retry_utils import retry_on_network_error
...
class DataProvider:
    ...
    def _manage_rate_limit(self) -> None:
        ...
        if self.request_count >= self.rate_limit_per_minute:
            ...
            for i in range(sleep_intervals):
                ...
                if i % 3 == 0:  # كل 3 فترات
                    logger.debug(
                        f"انتظار API: {(i + 1) * sleep_duration:.1f}/{wait_time:.1f} ثانية"
                    )
    ...
    @circuit_breaker(
        name="data_provider_klines", failure_threshold=3, recovery_timeout=30
    )
    ...
    @retry_on_network_error
    @circuit_breaker(
        name="data_provider_tickers", failure_threshold=3, recovery_timeout=30
    )
    ...
    def get_top_volume_pairs(
        self, quote_asset: str = "USDT", limit: int = 20
    ) -> List[str]:
        """
    ...
    @retry_on_network_error
    @circuit_breaker(
        name="data_provider_exchange_info",
        failure_threshold=3,
        recovery_timeout=60,
    )
    ...
    @retry_on_network_error
    @circuit_breaker(
        name="data_provider_ticker", failure_threshold=3, recovery_timeout=30
    )
        ...
        try:
            ...
            if RETRY_AVAILABLE:
                ...
                ticker_data = (
                    ...
                    if symbol
                    else self.client.get_symbol_ticker()
                )
    ...
    def check_connection(self) -> Dict:
        ...
        try:
            ...
            if "resolve" in error_msg or "nodename" in error_msg:
                ...
                result["error_detail"] = (
                    "فشل في حل اسم النطاق api.binance.com - تحقق من الاتصال بالإنترنت أو إعدادات DNS"
                )
            elif "connection" in error_msg:
                result["error"] = "CONNECTION_ERROR"
                result["error_detail"] = (
                    "فشل في الاتصال بـ Binance - تحقق من الاتصال بالإنترنت"
                )
    ...
    def _attempt_auto_reconnect(self) -> bool:
        ...
        logger.info(
            f"🔄 Attempting auto-reconnect {self._reconnect_attempts}/{self._max_reconnect_attempts} in {delay}s..."
        )
        time.sleep(delay)
        ...
        try:
            ...
            for cache_file in self.cache_dir.glob("*.pkl"):
                ...
                if older_than is None or (
                    current_time - cache_file.stat().st_mtime > older_than
                ):
                    cache_file.unlink()
                    count += 1
