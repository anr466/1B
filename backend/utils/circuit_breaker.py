        ...
        with self.lock:
            if self.state == CircuitState.OPEN:
                if self._should_attempt_reset():
                    ...
                    logger.info(f"🔄 {
                        self.config.name}: تغيير الحالة إلى HALF_OPEN")
                else:
                    raise CircuitBreakerOpenError(f"Circuit breaker '{
                        self.config.name}' مفتوح. " f"سيتم إعادة المحاولة بعد {
                        self._get_remaining_timeout():.0f} ثانية")
    ...
    def _on_success(self):
        ...
        with self.lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                logger.info(f"✅ {
                    self.config.name}: تم إغلاق الـ circuit (نجح الاتصال)")
    ...
    def _on_failure(self):
        ...
        with self.lock:
            ...
            if self.failure_count >= self.config.failure_threshold:
                ...
                logger.error(f"🔴 {
                    self.config.name}: فتح الـ circuit بعد {
                    self.failure_count} أخطاء")
