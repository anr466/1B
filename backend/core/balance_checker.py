...
class BalanceChecker:
    ...
    def can_open_new_trade(self, required_size: float = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if free_balance < self.MIN_TRADE_SIZE_BINANCE:
                return False, f"الرصيد الحر ({free_balance:.2f} USDT) أقل من الحد الأدنى ({self.MIN_TRADE_SIZE_BINANCE} USDT)"
            
            if free_balance < required_with_margin:
                return False, f"الرصيد الحر ({free_balance:.2f} USDT) لا يكفي للصفقة المطلوبة ({required_size:.2f} USDT + هامش)"
...
if __name__ == "__main__":
    ...
    print()
    print(f"   هل يمكن التداول: {'✅ نعم' if status['can_trade'] else '❌ لا'}")
