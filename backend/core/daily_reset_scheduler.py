...
class DailyResetScheduler:
    ...
    def reset_all_daily_limits(self):
        ...
        try:
            now = datetime.now()
            self.logger.info(
                f"🔄 بدء إعادة تعيين الحدود اليومية - {now.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            try:
                ...
                self.logger.info(
                    f"📸 تم تسجيل {snap_count} snapshot يومي للمحافظ"
                )
            except Exception as snap_err:
                self.logger.warning(
                    f"⚠️ تعذّر تسجيل snapshots المحافظ: {snap_err}"
                )
            ...
            for user in users:
                ...
                try:
                    ...
                    self.logger.error(
                        f"❌ خطأ في إعادة التعيين للمستخدم {username}: {user_error}")

            self.logger.info(
                f"✅ تم إعادة تعيين الحدود لـ {reset_count} مستخدم من أصل {
                    len(users)}"
            )
    ...
    def _reactivate_trading_if_needed(self, user_id: int):
        ...
        try:
                ...
                if recent_logs:
                    ...
                    for portfolio_is_demo in (False, True):
                        self.db.update_trading_settings(
                            ...
                            is_demo=portfolio_is_demo,
                        )
                    self.logger.info(
                        f"✅ تم إعادة تفعيل التداول للمستخدم {user_id} (يوم جديد)")
            ...
            self.logger.error(
                f"خطأ في إعادة تفعيل التداول للمستخدم {user_id}: {e}"
            )
    ...
    def start(self, reset_time: str = "00:00"):
        ...
        self.logger.info(
            f"✅ تم بدء Daily Reset Scheduler (إعادة تعيين يومية عند {reset_time})")
    ...
    def _run_scheduler(self):
        ...
        while self.is_running and not self._stop_event.is_set():
            try:
                ...
                self.logger.info(f"⏰ الانتظار {
                    wait_seconds /
                    3600:.1f} ساعة حتى إعادة التعيين التالية")
