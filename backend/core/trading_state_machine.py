...
class TradingStateMachine:
    ...
    def start(self, initiated_by: str = "admin", mode: str = "PAPER") -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_transition(
                STOPPED, STARTING, initiated_by, session_id, "طلب تشغيل النظام"
            )
    ...
    def emergency_stop(self, initiated_by: str = "admin") -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_transition(
                "ANY", STOPPED, initiated_by, session_id, "إيقاف طوارئ فوري"
            )
            ...
            if heartbeat.get("last_beat"):
                try:
                    hb_seconds = int(
                        (
                            datetime.now()
                            - datetime.fromisoformat(heartbeat["last_beat"])
                        ).total_seconds()
                    )
                    result["heartbeat"] = {
