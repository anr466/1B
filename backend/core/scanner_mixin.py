    ...
    def _check_market_regime(self) -> bool:
        ...
        try:
            ...
            if len(df) >= 24:
                ...
                if change_24h < -0.05:
                    self.logger.warning(
                        f"   🚨 BTC crashed {change_24h * 100:.1f}% in 24h - blocking entries"
                    )
            ...
            if len(df) >= 14:
                ...
                if atr_pct > 0.03:
                    self.logger.warning(
                        f"   🌊 BTC volatility too high (ATR={atr_pct * 100:.1f}%) - blocking entries"
                    )
                    self._market_caution_factor = 0.0  # لن يُستخدم (نرجع False)
                    return False
    ...
    def _analyze_smart_money_confluence(
        ...
        try:
            ...
            if analysis.get("error"):
                self.logger.debug(
                    f"Smart Money analysis error for {symbol}: {analysis.get('error_message')}"
                )
                return None
