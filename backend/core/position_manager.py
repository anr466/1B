...
class PositionManagerMixin:
    ...
    def _get_demo_symbol_rules(self, symbol: str) -> Dict:
        ...
        try:
            ...
            for f in symbol_info.get("filters", []):
                ...
                if f_type == "LOT_SIZE":
                    rules["step"] = float(
                        f.get("stepSize", rules["step"]) or rules["step"]
                    )
                    rules["min_qty"] = float(
                        f.get("minQty", rules["min_qty"]) or rules["min_qty"]
                    )
                elif f_type == "MIN_NOTIONAL":
                    rules["min_notional"] = float(
                        f.get("minNotional", rules["min_notional"])
                        or rules["min_notional"]
                    )
                elif f_type == "PRICE_FILTER":
                    rules["tick_size"] = float(
                        f.get("tickSize", rules["tick_size"]) or rules["tick_size"]
                    )
    ...
    def _get_demo_execution_price(
        ...
        try:
            if getattr(self, "data_provider", None) and getattr(
                self.data_provider, "client", None
            ):
                book_ticker = self.data_provider.client.get_orderbook_ticker(
                    symbol=symbol
                )
    ...
    def _simulate_demo_fill(
        ...
        if rules.get("status") != "TRADING":
            return {
                "success": False,
                "message": f"symbol_not_trading ({rules.get('status')})",
            }
    ...
    def _execute_real_order_with_retry(
        self, action: str, symbol: str, quantity: float, purpose: str
    ) -> Dict:
        ...
        for attempt in range(1, max_attempts + 1):
            if action == "buy":
                result = self.binance_manager.execute_buy_order(
                    self.user_id, symbol, quantity
                )
            else:
                result = self.binance_manager.execute_sell_order(
                    self.user_id, symbol, quantity
                )

            last_result = result if isinstance(result, dict) else {}
            success_with_order = last_result.get("success") and last_result.get(
                "order_id"
            )
    ...
    def _get_open_positions(self) -> List[Dict]:
        ...
        try:
            positions = self.db.get_user_active_positions(
    ...
    def _manage_position(self, position):
        ...
        if sl and sl > 0:
            ...
            try:
                ...
                self.logger.warning(
                    f"⚠️ Failed to fetch historical data for downtime SL check: {e}"
                )
            ...
            if sl_breached:
                ...
                return self._close_position(
                    position, current_price, "SL_BREACH_ON_RESTART", 1.0
                )
        ...
        self.logger.info(
            f"   📈 {symbol} ({position_type}): Entry ${entry_price:.4f} | "
        ...
        if self.strategy:
            try:
                ...
                if df is None or len(df) < 70:
                    self.logger.warning(
                        f"⚠️ {symbol}: Insufficient data for exit analysis"
                    )
                    return None
                ...
                if exit_result["should_exit"]:
                    ...
                    if use_exit_confirmation:
                        try:
                            conf = self.liquidity_filter.evaluate_exit_confirmation(
                                symbol, df, position, exit_result
                            )
                    ...
                    return self._close_position(
                        position, exit_price, reason, requested_close_pct
                    )
                else:
                    ...
                    if (
                        ...
                        try:
                            early = self.liquidity_filter.evaluate_early_exit(
                                symbol, df, position
                            )
                            if early.get("should_exit"):
                                early_price = float(
                                    early.get("exit_price", current_price)
                                    or current_price
                                )
                                ...
                                return self._close_position(
                                    position, early_price, early_reason, 1.0
                                )
                        except Exception as e:
                            self.logger.warning(
                                f"⚠️ LiquidityFilter early-exit error for {symbol}: {e}"
                            )
                ...
                if position_type == "SHORT":
                    if sl > 0 and current_price >= sl:
                        return self._close_position(
                            position, current_price, "STOP_LOSS_FALLBACK", 1.0
                        )
    ...
    def _close_position(
        ...
        Args:
            ...
            reason: سبب الخروج
            close_pct: نسبة الإغلاق (0.0-1.0)
        """
        position_id = position.get("id")
        symbol = position.get("symbol")
        entry_price = position.get("entry_price", 0)
        ...
        # ✅ FIX: حساب الكمية الفعلية للإغلاق بناءً على close_pct
        if close_pct <= 0 or close_pct > 1.0:
            close_pct = 1.0  # افتراضي للإغلاق الكامل

        closing_quantity = total_quantity * close_pct
        position_size_entry = entry_price * total_quantity  # الحجم الكامل للصفقة
        position_type = position.get("position_type", "long").upper()
        ...
        if position_type == "SHORT":
            ...
            pnl_raw = (exit_price - entry_price) * closing_quantity
        ...
        if position_is_demo:
            ...
            demo_close_fill = self._simulate_demo_fill(
                symbol, demo_close_side, closing_quantity, exit_price
            )
            ...
            if not self.binance_manager:
                try:
                    ...
                    self.logger.error(
                        f"⛔ BinanceManager unavailable for REAL close of {symbol}: {e}"
                    )
            ...
            if close_result.get("success"):
                ...
                # ✅ استخدام closing_quantity بدلاً من quantity
                if position_type == "SHORT":
                    pnl_raw = (entry_price - exit_price) * closing_quantity
                else:
                    pnl_raw = (exit_price - entry_price) * closing_quantity
                self.logger.info(
                    ...
                    f"qty={closing_quantity:.6f}/{total_quantity:.6f} ({close_pct:.1%}) "
        ...
        self.logger.info(
            f"   💰 CLOSING {position_type} {symbol}: ${exit_price:.4f} | "
            f"Qty: {closing_quantity:.6f}/{total_quantity:.6f} ({close_pct:.1%}) | "
        ...
        try:
            ...
            self.logger.error(
                f"❌ CRITICAL: Atomic transaction failed for closing {symbol}: {e}"
            )
            self.logger.error(
                f"   ⛔ Position NOT closed, balance NOT updated - transaction rolled back"
            )
            self._report_trading_error(
            ...
            try:
                ...
                try:
                    meta = (
                        ...
                        or position.get("signal_metadata")
                        or "{}"
                    )
    ...
    def _open_position(self, symbol: str, signal: Dict) -> Optional[Dict]:
        ...
        if self.is_demo_trading:
            ...
            if side == "LONG":
                result = self._execute_real_order_with_retry(
                    "buy", symbol, quantity, "open"
                )
            else:
                result = self._execute_real_order_with_retry(
                    "sell", symbol, quantity, "open"
                )

            if not result.get("success"):
                self.logger.error(
                    f"⛔ Binance order FAILED: {result.get('message', 'unknown error')}"
                )
                self._report_trading_error(
            ...
            if not result.get("order_id"):
                self.logger.error(
                    "⛔ Binance order missing order_id — trade considered NOT executed"
                )
                return None
        ...
        self.logger.info(
            f"   📍 OPENING {side} {symbol}: ${entry_price:.4f} | "
        ...
        try:
            with self.db.get_write_connection() as conn:
                ...
                if position_id is None:
                    self.logger.warning(
                        f"⚠️ Position {symbol} not saved — duplicate or constraint violation (balance NOT deducted)"
                    )
                    return None
            ...
            self.logger.error(
                f"❌ CRITICAL: Atomic transaction failed for {symbol}: {e}"
            )
            self.logger.error(
                f"   ⛔ Position NOT saved, balance NOT deducted - transaction rolled back"
            )
            self._report_trading_error(
    ...
    def _get_current_price(self, symbol: str) -> Optional[float]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    self.logger.debug(
                        f"⚠️ Price fetch retry {attempt + 1} for {symbol}: {e}"
                    )
                    _time.sleep(1)
                    continue
                self.logger.warning(
                    f"❌ Failed to get price for {symbol} after {max_retries} attempts: {e}"
                )
                return None
