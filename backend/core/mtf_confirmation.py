...
class MTFConfirmationEngine:
    ...
    def confirm_entry(
        ...
        reversal_15m = self._check_reversal_15m(df_15m)
        momentum_5m = self._check_momentum_5m(df_5m)
        trend_alignment = (
    ...
    def _check_reversal_15m(self, df: pd.DataFrame) -> Dict:
        rsi = df["rsi"].iloc[-1]
        ...
        return {"confirmed": False, "reason": f"15m RSI={rsi:.0f} no reversal"}

    def _check_momentum_5m(self, df: pd.DataFrame) -> Dict:
        ...
        return {"confirmed": False, "reason": f"5m momentum weak ({score}/4)"}
    ...
    def _check_trend_alignment(
        ...
        trend_15m_up = df_15m["close"].iloc[-1] > df_15m["ema21"].iloc[-1]
        trend_5m_up = df_5m["close"].iloc[-1] > df_5m["ema8"].iloc[-1]

        return trend_1h_up and (trend_15m_up or trend_5m_up)
