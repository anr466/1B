...
class TradingDecisionMatrix:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.entry_thresholds = {
            ...
            'minimum_market_quality': self.config.get('min_market_quality', 60),
            'minimum_confluence': self.config.get('min_confluence', 65),
            'maximum_portfolio_risk': self.config.get('max_portfolio_risk', 0.15),
        ...
        # معايير الخروج
        self.exit_criteria = {
            'stop_loss_hit': {'priority': ExitPriority.CRITICAL, 'exit_pct': 1.0},
            'take_profit_hit': {'priority': ExitPriority.HIGH, 'exit_pct': 0.7},
            'strong_exit_signal': {'priority': ExitPriority.HIGH, 'exit_pct': 0.8},
            'market_deterioration': {'priority': ExitPriority.MEDIUM, 'exit_pct': 0.5},
            'time_limit_reached': {'priority': ExitPriority.LOW, 'exit_pct': 1.0},
            'profit_protection': {'priority': ExitPriority.MEDIUM, 'exit_pct': 0.3}
        }
        
        logger.info("Decision Matrix initialized with entry threshold: {}".format(
    ...
    def evaluate_entry(self, 
                      ...
                      risk_assessment: RiskAssessment) -> DecisionResult:
        """
        ...
        try:
            ...
            if entry_score >= self.entry_thresholds['minimum_entry_score']:
                decision_type = (DecisionType.ENTER_LONG if market_condition.regime in ['UP', 'SIDEWAYS'] 
                               else DecisionType.ENTER_SHORT)
                
                position_size = self._calculate_position_size(entry_score, risk_assessment)
                ...
                return DecisionResult(
                    decision=DecisionType.WAIT,
                    confidence=int(entry_score),
                    reasoning=f"Entry score too low: {entry_score:.1f} < {self.entry_thresholds['minimum_entry_score']}",
                    entry_score=int(entry_score)
                )
    ...
    def evaluate_exit(self, 
        ...
        try:
            ...
            # 2. فحص جني الأرباح
            if take_profit and self._is_take_profit_hit(current_price, take_profit, position_type):
                return DecisionResult(
            ...
            exit_signals = self._analyze_exit_signals(position, current_market, current_price)
            
            if exit_signals:
                strongest_signal = max(exit_signals, key=lambda x: x['strength'])
    ...
    def _check_entry_rejections(self, market: MarketCondition, 
                               signal: SignalStrength,
                               risk: RiskAssessment, 
                               entry_score: float) -> List[str]:
        ...
        if signal.confluence_score < self.entry_thresholds['minimum_confluence']:
            rejections.append(f"Confluence too low: {signal.confluence_score}")
            
        if risk.portfolio_risk > self.entry_thresholds['maximum_portfolio_risk']:
            rejections.append(f"Portfolio risk too high: {risk.portfolio_risk:.2f}")
            
        if risk.risk_reward_ratio < self.entry_thresholds['minimum_rr_ratio']:
            rejections.append(f"R:R ratio too low: {risk.risk_reward_ratio:.2f}")
            
        return rejections
    
    def _calculate_position_size(self, entry_score: float, risk: RiskAssessment) -> float:
    ...
    def _is_stop_loss_hit(self, current_price: float, stop_loss: float, position_type: str) -> bool:
    ...
    def _is_take_profit_hit(self, current_price: float, take_profit: float, position_type: str) -> bool:
    ...
    def _analyze_exit_signals(self, position: Dict, market: MarketCondition, current_price: float) -> List[Dict]:
