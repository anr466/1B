...
class IntelligentCoinFilter:
    ...
    def filter_coin(
        ...
        try:
            ...
            perf_score, perf_passed, perf_reason = self._analyze_performance(
                df
            )
            ...
            trend_score, trend_passed, trend_reason = (
                self._analyze_trend_quality(df)
            )
            ...
            # 3. تحليل السيولة
            liquidity_score, liquidity_passed, liquidity_reason = (
                self._analyze_liquidity(df)
            )
            ...
            # 4. تحليل التقلب
            volatility_score, volatility_passed, volatility_reason = (
            ...
            # 5. تحليل الاستقرار
            stability_score, stability_passed, stability_reason = (
                self._analyze_stability(df)
            )
    ...
    def _analyze_performance(
        ...
        score = 0
        score += (
            positive_periods / self.criteria["min_profitable_periods"]
        ) * 0.4
        ...
        passed = (
            positive_periods >= self.criteria["min_profitable_periods"]
            and avg_return >= self.criteria["min_avg_return"]
            and max_drawdown <= self.criteria["max_drawdown"]
        )
    ...
    def _analyze_trend_quality(
        ...
        trend_consistency = (
            valid_data["sma_20"] > valid_data["sma_50"]
        ).sum() / len(valid_data)
        ...
        score = 0
        score += (
            trend_consistency / self.criteria["min_trend_consistency"]
        ) * 0.6
        ...
        passed = (
            trend_consistency >= self.criteria["min_trend_consistency"]
            and higher_highs >= self.criteria["min_higher_highs"]
        )
    ...
    def _analyze_liquidity(self, df: pd.DataFrame) -> Tuple[float, bool, str]:
        ...
        days_above_threshold = (
            df["volume"] * df["close"] > self.criteria["min_volume_usd"]
        ).sum()
        ...
        # الاجتياز
        passed = (
            avg_volume_usd >= self.criteria["min_volume_usd"]
            and volume_consistency >= self.criteria["min_volume_consistency"]
        )
    ...
    def _analyze_stability(self, df: pd.DataFrame) -> Tuple[float, bool, str]:
        ...
        score = 0
        score += (
            1 - manipulation_score / self.criteria["max_manipulation_score"]
        ) * 0.5
        ...
        # الاجتياز
        passed = (
            manipulation_score <= self.criteria["max_manipulation_score"]
            and data_quality >= self.criteria["min_data_quality"]
        )
    ...
    def _analyze_risk_reward(
        ...
        # الاجتياز
        passed = (
            win_rate >= self.criteria["min_win_rate_historical"]
            and profit_factor >= self.criteria["min_profit_factor"]
        )
