...
class DynamicUniverseSelector:
    ...
    def __init__(self):
        ...
        self.all_coins = [
            ...
            'ALGO/USDT', 'VET/USDT', 'FIL/USDT', 'TRX/USDT', 'AVAX/USDT',
            'NEAR/USDT', 'APT/USDT', 'ARB/USDT', 'OP/USDT', 'INJ/USDT'
        ]
    
    def select_universe(self, market_condition: str = 'NEUTRAL', 
                       max_coins: int = 20) -> List[Dict]:
        """
        اختيار أفضل العملات
        ...
        for symbol in self.all_coins:
            try:
                # جلب البيانات
                df = self.data_provider.get_historical_data(symbol, '1d', limit=90)
                
                if df is None or len(df) < 50:
                    continue
                
                # تسجيل العملة
                score_result = self.coin_scorer.score_coin(symbol, df, market_condition)
                
                if score_result['is_tradeable']:
                    candidates.append(score_result)
                    self.logger.info(f"  ✅ {symbol}: {score_result['total_score']:.1f}/100 ({score_result['rank']})")
                else:
                    self.logger.debug(f"  ❌ {symbol}: رفض - {score_result['details'].get('rejection_reason', 'نقاط منخفضة')}")
