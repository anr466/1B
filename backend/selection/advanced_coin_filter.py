...
class AdvancedCoinFilter:
    ...
    def filter_coins(self, coin_data_list: List[Dict]) -> List[Dict]:
        ...
        for coin in coin_data_list:
            ...
            if scores['total'] >= self.criteria['min_total_score']:
                ...
                self.logger.debug(f"✅ {symbol} اجتاز ({scores['total']:.1f}/100)")
            else:
                self.logger.debug(f"❌ {symbol} لم يجتاز ({scores['total']:.1f}/100)")
        
        # ترتيب حسب النقاط
        filtered_coins.sort(key=lambda x: x['quality_score'], reverse=True)
        
        self.logger.info(f"📊 تم فلترة {len(filtered_coins)} عملة من {len(coin_data_list)}")
    ...
    def _score_trend(self, df: pd.DataFrame) -> float:
        ...
        above_sma = (df['close'].iloc[-len(valid):] > valid).sum() / len(valid)
    ...
    def _score_performance(self, df: pd.DataFrame) -> float:
        ...
        positive_days = (returns > 0).sum() / len(returns) if len(returns) > 0 else 0.5
    ...
    def get_top_coins(self, coin_data_list: List[Dict], top_n: int = 10) -> List[Dict]:
