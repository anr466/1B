...
class UnifiedExitSystem:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "trailing": {
                ...
                # أقصى هبوط مسموح في الاتجاه 1.5%
                "max_drawdown_in_trend": 0.015,
    ...
    def register_position(
        ...
        if self.config["stop_loss"]["use_profile"]:
            ...
            sl_pct = (
                atr * self.config["stop_loss"]["atr_multiplier"]
            ) / entry_price
        else:
            sl_pct = 0.025
        ...
        # Log
        self.logger.info(
            f"📊 [{symbol}] صفقة جديدة - {profile.category.value}"
        )
    ...
    def _create_tp_levels(
        self, entry: float, profile: AssetProfile
    ) -> List[Dict]:
        """إنشاء مستويات TP من ملف تعريف العملة"""
        levels = []
        names = ["TP1", "TP2", "TP3"]

        for i, (tp_pct, close_pct) in enumerate(
            zip(profile.optimal_tp_levels, profile.tp_close_pcts)
        ):
            levels.append(
                {
                    "name": names[i],
    ...
    def check_exit(
        ...
        trailing_check = self._check_trailing_stop(
            state, current_price, candle_data
        )
        if trailing_check:
        ...
        if self.config["reversal"]["enabled"] and candle_data:
            reversal_check = self._check_reversal(
                state, current_price, candle_data, change_pct
            )
            if reversal_check:
    ...
    def _check_emergency(
        self, state: PositionState, price: float
    ) -> Optional[Dict]:
        """فحص الخروج الطارئ"""
    ...
    def _check_trailing_stop(
        self, state: PositionState, price: float, candle_data: Dict = None
    ) -> Optional[Dict]:
        ...
        if self.config["trailing"]["use_profile"] and state.asset_profile:
            ...
            trail_distance_pct = self.config["trailing"][
                "default_distance_pct"
            ]
        ...
        if not state.trailing_active:
            if profit_pct >= self.config["trailing"]["activation_pct"]:
                ...
                self.logger.info(
                    f"🎯 [{state.symbol}] Trailing مُفعّل عند +{profit_pct * 100:.2f}%"
                )
        ...
        if state.trailing_active:
            if price > state.highest_price:
                ...
                min_profit = entry * (
                    1 + self.config["trailing"]["min_profit_lock"]
                )
            ...
            if price <= state.trailing_stop:
                ...
                if self.config["trailing"]["trend_protection"] and candle_data:
                    ...
                    min_profit_for_protection = self.config["trailing"].get(
                        "min_profit_for_protection", 0.02
                    )
                    max_drawdown_in_trend = self.config["trailing"].get(
                        "max_drawdown_in_trend", 0.015
                    )

                    # إذا الاتجاه صاعد + ربح جيد + هبوط صغير = نبقى
                    if self._is_trend_still_bullish(
                        candle_data, state.asset_profile
                    ):
                        if (
                            profit_pct > min_profit_for_protection
                            and drawdown < max_drawdown_in_trend
                        ):
                            # توسيع Trailing 50%
                            expansion = self.config["trailing"].get(
                                "trend_expansion_mult", 1.5
                            )
                            state.trailing_stop = state.highest_price * (
                                1 - trail_distance_pct * expansion
                            )
                            return None  # نبقى في الصفقة

                # الخروج مع حد أدنى للربح
                actual_pnl = (price - entry) / entry
                min_lock = self.config["trailing"].get(
                    "min_profit_lock", 0.005
                )
                final_pnl = (
                    max(actual_pnl, min_lock) if actual_pnl > 0 else actual_pnl
                )
    ...
    def _check_time_exit(
        self, state: PositionState, timestamp: datetime, change_pct: float
    ) -> Optional[Dict]:
        """فحص Time-Based Exit"""
        ...
        if state.last_movement_time:
            ...
            if stagnant_hours >= self.config["time_exit"]["stagnant_hours"]:
                ...
                if (
                    abs(change_pct)
                    < self.config["time_exit"]["stagnant_threshold"]
                ):
                    return self._create_exit(
                        state,
                        state.last_price,
    ...
    def _check_reversal(
        ...
        ignore_small = self.config["reversal"].get(
            "ignore_small_corrections", True
        )
        ...
        if (
            consecutive_red
            >= self.config["reversal"]["consecutive_red_candles"]
        ):
        ...
        if (
            ...
            >= self.config["reversal"]["volume_spike"]
        ):
    ...
    def _is_trend_still_bullish(
        self, candle_data: Dict, profile: AssetProfile = None
    ) -> bool:
        """
        ...
        total_checks += 1
        if (
            not candle_data.get("is_bearish", False)
            or candle_data.get("body_pct", 0) < 0.02
        ):
            bullish_signals += 1
