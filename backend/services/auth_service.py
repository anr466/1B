...
class AuthService:
    ...
    def login(self, username: str, password: str, device_id: Optional[str] = None) -> Tuple[bool, Dict]:
        """
        تسجيل دخول المستخدم
        ...
        try:
            ...
            # توليد tokens
            tokens = self._generate_tokens(user['id'], user['username'], user['user_type'])
    ...
    def register(self, username: str, password: str, email: str, phone_number: Optional[str] = None) -> Tuple[bool, Dict]:
        """
        تسجيل مستخدم جديد
    ...
    def _create_user(self, username: str, password_hash: str, email: str, phone_number: Optional[str] = None) -> Optional[int]:
        """إنشاء مستخدم جديد"""
        try:
            with self.db.get_write_connection() as conn:
                cursor = conn.cursor()
    ...
    def _generate_tokens(self, user_id: int, username: str, user_type: str) -> Dict[str, str]:
