...
class NotificationCleanupService:
    ...
    def _enforce_notification_limit(self, user_id: int = None) -> int:
        ...
        try:
            ...
            if user_id:
                ...
                deleted_count = self._delete_excess_notifications(
                    user_id, self.MAX_NOTIFICATIONS_PER_USER
                )
            else:
                ...
                users = self.db.execute_query(
                    query, (self.MAX_NOTIFICATIONS_PER_USER,)
                )

                for user in users:
                    user_id = user["user_id"]
                    excess_deleted = self._delete_excess_notifications(
                        user_id, self.MAX_NOTIFICATIONS_PER_USER
                    )
    ...
    def _delete_excess_notifications(self, user_id: int, limit: int) -> int:
        ...
        try:
            ...
            logger.error(
                f"❌ خطأ في حذف الإشعارات الزائدة للمستخدم {user_id}: {e}"
            )
            return 0
    ...
    def get_cleanup_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats["read_notifications"] = (
                read_result[0]["count"] if read_result else 0
            )
            ...
            stats["unread_notifications"] = (
                unread_result[0]["count"] if unread_result else 0
            )
            ...
            stats["old_read_notifications"] = (
                old_read_result[0]["count"] if old_read_result else 0
            )
            ...
            stats["old_unread_notifications"] = (
                old_unread_result[0]["count"] if old_unread_result else 0
            )
            ...
            """
            excess_result = self.db.execute_query(
                excess_query, (self.MAX_NOTIFICATIONS_PER_USER,)
            )
            stats["users_with_excess"] = (
                excess_result[0]["count"] if excess_result else 0
            )
