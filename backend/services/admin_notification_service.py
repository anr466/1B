...
class AdminNotificationService:
    ...
    def __init__(self):
        ...
        if FIREBASE_AVAILABLE:
            try:
                ...
                self.logger.info(
                    "✅ Firebase Push Notifications مُفعّل للأدمن"
                )
            ...
            with self.db.get_write_connection() as conn:
                ...
                if settings:
                    ...
                    self.telegram_bot_token = (
                        settings[2] or self.telegram_bot_token
                    )
                    self.telegram_chat_id = (
                        settings[3] or self.telegram_chat_id
                    )
    ...
    def notify_admin(
        ...
        # إرسال عبر القنوات المفعّلة (في thread منفصل لعدم تأخير العملية
        # الرئيسية)
        Thread(
            target=self._send_notifications,
            args=(title, message, severity, data),
        ).start()

    def _send_notifications(
        self, title: str, message: str, severity: str, data: Dict = None
    ):
    ...
    def _send_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.warning(f"⚠️ محاولة {
                    attempt + 1}/{max_retries} فشلت لـ {description}: {e}")

                if attempt < max_retries - 1:
                    self.logger.info(
                        f"🔄 إعادة المحاولة بعد {wait_time} ثانية..."
                    )
                    time.sleep(wait_time)
                else:
                    self.logger.error(
                        f"❌ فشل نهائي في إرسال {description} بعد {max_retries} محاولات")
    ...
    def _send_telegram(self, title: str, message: str, severity: str):
        ...
        return self._send_with_retry(
            _do_send, max_retries=3, description="Telegram"
        )
    ...
    def _send_webhook(
        ...
        # ✅ استخدام retry mechanism
        return self._send_with_retry(
            _do_send, max_retries=3, description="Webhook"
        )
    ...
    def _send_fcm_push(
        ...
        try:
            ...
            success = self.firebase_service.send_to_user(
                user_id=admin_user_id, title=title, body=message, data=fcm_data
            )
    ...
    def notify_system_error(
        self, error_type: str, error_message: str, traceback: str = None
    ):
    ...
    def notify_trading_stopped(self, reason: str):
        ...
        try:
            ...
            if recent:
                self.logger.info(
                    "ℹ️ تجاهل إنذار trading_stopped مكرر خلال آخر 30 دقيقة"
                )
                return
        except Exception as e:
    ...
    def notify_ml_status_change(
        self, is_ready: bool, samples: int, required: int
    ):
    ...
    def update_settings(self, settings: Dict) -> bool:
        ...
        try:
            ...
            def _pick_str(
                key: str, current_value: Optional[str]
            ) -> Optional[str]:
            ...
            telegram_bot_token = _pick_str(
                "telegram_bot_token", self.telegram_bot_token
            )
            telegram_chat_id = _pick_str(
    ...
    def get_unread_count(self) -> int:
        ...
        try:
            with self.db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT COUNT(*) FROM system_alerts WHERE read = 0"
                )
    ...
    def mark_as_read(self, alert_id: int) -> bool:
        ...
        try:
            with self.db.get_write_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE system_alerts SET read = 1 WHERE id = %s",
                    (alert_id,),
                )
    ...
    def mark_all_as_read(self) -> bool:
        ...
        try:
            with self.db.get_write_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE system_alerts SET read = 1 WHERE read = 0"
                )
