...
class AdaptiveOptimizer:
    ...
    def get_optimal_sl(self, symbol: str) -> float:
        ...
        # تطبيق الحدود الآمنة
        return max(
            SAFE_LIMITS["sl_pct_min"], min(SAFE_LIMITS["sl_pct_max"], adjusted_sl)
        )
    ...
    def get_preferred_symbols(
        ...
        for sym, data in ranked_map.items():
            if data["trades"] >= 8 and data["win_rate"] < 0.25:
                blocked.add(sym)
                logger.info(
                    f"📈 Dropping {sym}: WR={data['win_rate']:.0%} over {data['trades']} trades"
                )
    ...
    def get_position_size_multiplier(
        ...
        return max(
            SAFE_LIMITS["size_mult_min"], min(SAFE_LIMITS["size_mult_max"], mult)
        )
    ...
    def shadow_validate(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(all_rows) < 15:
                ...
                logger.info(
                    f"🔬 Shadow Validation: {result['verdict']} ({len(all_rows)} trades)"
                )
                return result
            ...
            if len(test) < 3:
                return {
                    ...
                    "baseline": 0.5,
                    "lift": 0,
                    "verdict": "INSUFFICIENT_TEST",
                    "details": "مجموعة اختبار صغيرة جداً",
                }
    ...
    def _simulate_score(
        self, test_row: tuple, train_rows: list, weights: Dict
    ) -> float:
        ...
        if rsi is not None:
            ...
            matches = [
                t
                for t in train_rows
                if t[2] is not None and self._get_rsi_zone(t[2]) == zone
            ]
        ...
        if vol is not None:
            ...
            matches = [
                t
                for t in train_rows
                if t[5] is not None and self._vol_level(t[5]) == level
            ]
        ...
        if bb is not None:
            ...
            matches = [
                t
                for t in train_rows
                if t[4] is not None and self._bb_zone(t[4]) == zone
            ]
        ...
        if rsi is not None and trend is not None:
            ...
            matches = [
                ...
                if t[2] is not None
                and t[8] is not None
                and self._get_rsi_zone(t[2]) == rsi_zone
                and t[8] == trend
            ]
        ...
        if vol is not None and trend is not None:
            ...
            matches = [
                ...
                if t[5] is not None
                and t[8] is not None
                and self._vol_level(t[5]) == vol_level
                and t[8] == trend
            ]
    ...
    def score_signal(self, symbol: str, indicators: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            rt_indices = [
                ...
                if r[2] is not None
                and r[8] is not None
                and self._get_rsi_zone(r[2]) == rsi_zone
                and r[8] == trend_now
            ]
            if len(rt_indices) >= 3:
            ...
            vt_indices = [
                ...
                if r[5] is not None
                and r[8] is not None
                and self._vol_level(r[5]) == vol_level
                and r[8] == trend_now
            ]
            if len(vt_indices) >= 3:
    ...
    def _calculate_adaptive_weights(self, rows: list) -> Dict[str, float]:
        ...
        # Trend accuracy
        acc = _factor_acc(
            test_rows, train_rows, lambda r: r[8], lambda r: r[8] is not None
        )
        if acc is not None:
    ...
    def _decay_weighted_wr(
        self, rows: list, indices: List[int], decay_weights: List[float]
    ) -> float:
        """
