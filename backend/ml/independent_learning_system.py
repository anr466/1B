...
class IndependentLearningSystem:
    ...
    def validate_trade_source(self, trade_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if source == "unknown":
                self.logger.debug(
                    "⚠️ صفقة بدون تحديد المصدر - افتراضياً من التداول الفعلي"
                )
                return True
    ...
    def can_learn_from_trade(
        ...
        """
        try:
            result = {"can_learn": False, "reasons": [], "warnings": []}

            # 1. التحقق من المصدر
            if not self.validate_trade_source(trade_data):
                result["reasons"].append(f"مصدر غير مسموح: {
                    trade_data.get(
                        'source',
                        'unknown')}")
                return result
            ...
            missing_fields = [
                f for f in required_fields if f not in trade_data
            ]
    ...
    def extract_learning_features(
        ...
        try:
            ...
            if features["entry_price"] > 0:
                features["return_pct"] = (
                    (features["exit_price"] - features["entry_price"])
    ...
    def learn_from_real_trades(
        ...
        try:
            ...
            result["learning_summary"] = {
                ...
                "best_strategy": (
                    df["strategy"].value_counts().index[0]
                    if len(df["strategy"].value_counts()) > 0
                    else "unknown"
                ),
                "best_timeframe": (
                    df["timeframe"].value_counts().index[0]
                    if len(df["timeframe"].value_counts()) > 0
                    else "unknown"
                ),
                "best_symbol": (
                    df["symbol"].value_counts().index[0]
                    if len(df["symbol"].value_counts()) > 0
                    else "unknown"
                ),
            }
