...
class AutoHealthMonitor:
    ...
    def daily_health_check(self, recent_trades: List[Dict], learned_rules: List) -> Dict[str, Any]:
        """فحص صحة يومي"""
        ...
        logger.info(f"📊 فحص صحة: {checks['summary']['health_score']:.1%} "
    ...
    def _check_win_rate(self, recent_trades: List[Dict]) -> Dict:
        ...
        last_50 = recent_trades[-50:] if len(recent_trades) >= 50 else recent_trades
        ...
        if degradation > 0.10:  # تراجع 10%+
            return {
                ...
                'message': f'تراجع Win Rate: {win_rate:.1%} (كان {self.baseline_win_rate:.1%})'
            }
    ...
    def _correct_win_rate_issue(self, issue: Dict) -> Optional[Dict]:
        ...
        if issue['severity'] == 'high':
            ...
            self.dual_path.weights['conservative'] = min(0.70, old_weight + 0.15)
            self.dual_path.weights['balanced'] = 1.0 - self.dual_path.weights['conservative']
            
            logger.warning(f"🔧 تصحيح تلقائي: زيادة الحذر ({old_weight:.2%} → {self.dual_path.weights['conservative']:.2%})")
        ...
        if abs(c_acc - b_acc) > 0.15:
            ...
            if total > 0:
                ...
                if old_weights != self.dual_path.weights:
                    logger.info(f"⚖️ تعديل الأوزان: محافظ={self.dual_path.weights['conservative']:.2%}, "
                               f"متوازن={self.dual_path.weights['balanced']:.2%}")
    ...
    def get_health_report(self) -> Dict:
        ...
        if len(self.check_history) >= 7:
            recent_scores = [c['summary']['health_score'] for c in self.check_history[-7:]]
