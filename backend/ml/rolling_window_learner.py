...
class RollingWindowLearner:
    ...
    def __init__(self, window_days: int = 90):
        ...
        logger.info(f"✅ تهيئة Rolling Window Learner (نافذة: {window_days} يوم)")
    
    def get_recent_signals(self, all_signals: List[Dict], combination: str) -> List[Dict]:
    ...
    def should_learn(self, recent_signals: List[Dict]) -> bool:
        ...
        if len(recent_signals) < self.min_sample_size:
            logger.debug(f"⚠️ عينة صغيرة: {len(recent_signals)} < {self.min_sample_size}")
            return False
    ...
    def extract_patterns(self, recent_signals: List[Dict]) -> Optional[Dict]:
        ...
        good_signals = [s for s in recent_signals if s['signal_quality_score'] > 0.7]
        bad_signals = [s for s in recent_signals if s['signal_quality_score'] < 0.4]
        ...
        patterns = {
            ...
            # الأنماط المثلى
            'optimal_conditions': self._extract_optimal_conditions(good_signals),
            
            # الأنماط التي يجب تجنبها
            'avoid_conditions': self._extract_avoid_conditions(bad_signals) if bad_signals else [],
    ...
    def _extract_optimal_conditions(self, good_signals: List[Dict]) -> Dict:
        ...
        # RSI
        rsi_values = [s.get('entry_rsi', 50) for s in good_signals if s.get('entry_rsi')]
        if rsi_values:
            conditions['rsi'] = {
        ...
        # Volatility
        vol_values = [s.get('volatility', 0) for s in good_signals if s.get('volatility', 0) > 0]
        if vol_values:
        ...
        # Volume
        vol_values = [s.get('entry_volume', 0) for s in good_signals if s.get('entry_volume', 0) > 0]
        if vol_values:
            conditions['volume'] = {
                'min': float(np.percentile(vol_values, 10)),
                'median': float(np.median(vol_values)),
    ...
    def _extract_avoid_conditions(self, bad_signals: List[Dict]) -> List[Dict]:
        ...
        # تحليل RSI المتطرف
        extreme_rsi = [s for s in bad_signals if s.get('entry_rsi', 50) > 75 or s.get('entry_rsi', 50) < 25]
    ...
    def _calculate_data_age(self, signals: List[Dict]) -> float:
        ...
        now = datetime.now()
        ages = [(now - s['timestamp']).days for s in signals if s.get('timestamp')]
