...
class PatternSimilarityMatcher:
    ...
    def calculate_similarity(
        self, current_market: Dict[str, Any], reference_pattern: Dict[str, Any]
    ) -> float:
        """
        ...
        try:
            ...
            if (
                "price_data" in current_market
                and "price_data" in reference_pattern
            ):
                ...
                similarity_score += (
                    price_similarity * self.weights["price_action"]
                )
            ...
            if "volume" in current_market and "volume" in reference_pattern:
                volume_similarity = self._compare_volume(
                    current_market["volume"], reference_pattern["volume"]
                )
                similarity_score += (
                    volume_similarity * self.weights["volume_profile"]
                )

            # 3. Technical Indicators (المؤشرات الفنية)
            if (
                "indicators" in current_market
                and "indicators" in reference_pattern
            ):
                ...
                similarity_score += (
                    indicator_similarity * self.weights["indicators"]
                )
            ...
            if "trend" in current_market and "trend" in reference_pattern:
                structure_similarity = self._compare_structure(
                    current_market["trend"], reference_pattern["trend"]
                )
                similarity_score += (
                    structure_similarity * self.weights["market_structure"]
                )
        ...
        try:
            ...
            if ref_volatility > 0:
                volatility_ratio = min(
                    current_volatility, ref_volatility
                ) / max(current_volatility, ref_volatility)
            ...
            if ref_range > 0:
                range_ratio = min(current_range, ref_range) / max(
                    current_range, ref_range
                )
    ...
    def get_detailed_similarity_report(
        self, current_market: Dict[str, Any], reference_pattern: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        report = {
            "overall_similarity": self.calculate_similarity(
                current_market, reference_pattern
            ),
            "components": {},
        }
        ...
        if (
            ...
            report["components"]["price_action"] = self._compare_price_action(
                current_market["price_data"], reference_pattern["price_data"]
            )
        ...
        if (
            ...
            report["components"]["indicators"] = self._compare_indicators(
                current_market["indicators"], reference_pattern["indicators"]
            )
