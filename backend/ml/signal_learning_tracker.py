...
class SignalLearningTracker:
    ...
    def record_signal(
        self, signal_data: Dict[str, Any], execution_time: datetime
    ) -> str:
    ...
    def evaluate_signal(
        self, signal_id: str, trade_result: Dict[str, Any]
        ...
        signal["holding_time_minutes"] = trade_result.get(
        ...
        logger.info(f"📊 تقييم إشارة {
            signal['combination']}: جودة = {
            quality_score:.2%}")
    ...
    def _learn_from_result(self, signal: Dict):
        ...
        if quality > 0.7:  # إشارة ناجحة
            ...
            stats["optimal_conditions"]["volume"].append(
                signal["entry_volume"]
            )
            stats["optimal_conditions"]["volatility"].append(
                signal["volatility"]
            )
    ...
    def get_learned_patterns(
        self, combination: str, days: int = 90
    ) -> Optional[Dict]:
        ...
        return {
            ...
            "optimal_rsi_range": self._get_percentile_range(
                [s["entry_rsi"] for s in good_signals]
            ),
            "optimal_volume_range": self._get_percentile_range(
                [s["entry_volume"] for s in good_signals]
            ),
            "optimal_volatility_range": self._get_percentile_range(
                [s["volatility"] for s in good_signals]
            ),
        ...
        for combo, stats in self.combination_stats.items():
            if stats["total"] > 0:
                all_stats[combo] = {
                    ...
                    "avg_profit": (
                        stats["total_profit"] / stats["total"]
                        if stats["total"] > 0
                        else 0
                    ),
                    "has_learned_patterns": len(
                        stats.get("optimal_conditions", {}).get("rsi", [])
                    )
                    >= 10,
                }
