...
class HybridMLSystem:
    ...
    def should_use_backtest_data(
        ...
        if (
            self.current_market_regime != MarketRegime.UNKNOWN
            and self.backtest_market_regime != MarketRegime.UNKNOWN
            and self.current_market_regime != self.backtest_market_regime
        ):
            logger.info(f"⚠️ نظام السوق مختلف: {
                self.current_market_regime.value} vs {
                self.backtest_market_regime.value}")
            return False
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        if self.trade_dates:
            total_age = sum(
                (datetime.now() - date).days for date in self.trade_dates
            )
...
class DynamicConfidenceSystem:
    ...
    def update_after_trade(
        self, profit: float, profit_pct: float
    ) -> Dict[str, Any]:
        """
        ...
        if self.consecutive_losses >= self.MAX_CONSECUTIVE_LOSSES:
            ...
            logger.warning(f"⚠️ إيقاف مؤقت للنمط {
                self.pattern_id}: {
                self.consecutive_losses} خسائر متتالية")
            return {
                "status": "paused",
                "action": "pause_trading",
                "confidence": self.current_confidence,
                "reason": f"{
                    self.consecutive_losses} خسائر متتالية - إيقاف لمدة 7 أيام",
                "pause_until": (
                    datetime.now() +
                    timedelta(
                        days=7)).isoformat(),
    ...
    def calculate_confidence_interval(self) -> Dict[str, float]:
        ...
        confidence_level = (
            "high" if total >= self.CONFIDENCE_THRESHOLD else "medium"
        )
    ...
    def check_pattern_expiry(self) -> bool:
        ...
        if recent_win_rate < 0.30:
            self.status = "expired"
            logger.warning(f"⚠️ النمط {
                self.pattern_id} انتهت صلاحيته: Win Rate آخر 10 صفقات = {
                recent_win_rate:.1%}")
            return True
    ...
    def should_trade(self, pattern_similarity: float) -> Dict[str, Any]:
        ...
        if self.status == "paused" and self.paused_at:
            ...
            if days_paused < pause_duration:
                ...
                self.consecutive_losses = 0
                self.retry_count += 1
                logger.info(f"✅ إعادة تفعيل النمط {
                    self.pattern_id} بعد {days_paused} يوم")
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_change": self.current_confidence
...
def get_confidence_system(
    pattern_id: str, initial_confidence: float = 0.70
) -> DynamicConfidenceSystem:
    """الحصول على نظام الثقة لنمط معين"""
    global _confidence_systems
    if pattern_id not in _confidence_systems:
        _confidence_systems[pattern_id] = DynamicConfidenceSystem(
            pattern_id, initial_confidence
        )
