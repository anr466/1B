...
class SmartIncrementalLearning:
    ...
    def process_signal(self, signal_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # التركيبة
        combination = f"{
            signal_data['symbol']}_{
            signal_data['strategy']}_{
        ...
        if decision["action"] == "trade":
            ...
            signal_id = self.signal_tracker.record_signal(
                signal_data, datetime.now()
            )
            decision["signal_id"] = signal_id

            logger.info(f"✅ قبول إشارة {combination}: ثقة={
                decision['confidence']:.2%}, " f"إجماع={
                decision['consensus']}")
        else:
            logger.debug(f"⛔ رفض إشارة {combination}: {
                decision.get(
                    'explanation', '')}")
    ...
    def on_trade_closed(self, signal_id: str, trade_result: Dict[str, Any]):
        ...
        inferred_decision = {
            ...
            "conservative_decision": {
                "action": (
                    "trade"
                    if inferred_confidence
                    >= self.dual_path.conservative.confidence_threshold
                    else "skip"
                ),
                "confidence": inferred_confidence,
            ...
            "balanced_decision": {
                "action": (
                    "trade"
                    if inferred_confidence
                    >= self.dual_path.balanced.confidence_threshold
                    else "skip"
                ),
                "confidence": inferred_confidence,
                "reason": "inferred_from_recorded_signal",
        ...
        self.learned_patterns_cache.pop(
            combination, None
        )  # حذف من cache للتحديث

        logger.info(f"📚 تعلم من صفقة {signal_id}: جودة={
            signal['signal_quality_score']:.2%}")

    def _get_patterns_for_combination(
        ...
        recent_signals = self.rolling_window.get_recent_signals(
            list(self.signal_tracker.signal_history), combination
        )
        ...
        if patterns:
            ...
            logger.debug(f"📊 أنماط متعلمة لـ {combination}: {
                patterns['success_rate']:.1%} نجاح")
    ...
    def run_daily_health_check(self, recent_trades: list = None) -> Dict:
        ...
        health_report = self.health_monitor.daily_health_check(
            recent_trades, []  # learned_rules - يمكن إضافته لاحقاً
        )

        logger.info(f"🏥 فحص صحة: نتيجة={
            health_report['summary']['health_score']:.1%}")
    ...
    def get_learning_progress(self) -> Dict[str, Any]:
        ...
        overall_win_rate = (
            total_wins / total_signals if total_signals > 0 else 0
        )
        ...
        return {
            ...
            "system_readiness": self._calculate_readiness(
                total_signals, learned_combinations
            ),
        }
    ...
    def _calculate_readiness(
        self, total_signals: int, learned_combos: int
    ) -> float:
    ...
    def get_detailed_combination_report(
        self, combination: str
    ) -> Optional[Dict]:
...
def get_learning_system(
    user_id: int, db_manager=None
) -> SmartIncrementalLearning:
    """الحصول على instance التعلم للمستخدم"""

    if user_id not in _learning_instances:
        _learning_instances[user_id] = SmartIncrementalLearning(
            user_id, db_manager
        )
