...
class SimpleLearner:
    ...
    def __init__(self, name: str, learning_rate: float, min_sample: int, confidence_threshold: float):
        self.name = name
        ...
        logger.info(f"✅ تهيئة {name}: LR={learning_rate}, MinSample={min_sample}, Threshold={confidence_threshold}")
    
    def evaluate(self, signal_data: Dict, learned_patterns: Optional[Dict] = None) -> Dict[str, Any]:
        """تقييم الإشارة"""
        
        base_confidence = signal_data.get('confidence', 0.7)
        
        # إذا لا توجد أنماط متعلمة
        if not learned_patterns or learned_patterns.get('sample_size', 0) < self.min_sample:
            return {
                'action': 'trade' if base_confidence >= self.confidence_threshold else 'skip',
        ...
        adjusted_confidence = self._apply_learned_patterns(signal_data, learned_patterns, base_confidence)
        
        # القرار
        action = 'trade' if adjusted_confidence >= self.confidence_threshold else 'skip'
    ...
    def _apply_learned_patterns(self, signal: Dict, patterns: Dict, base_confidence: float) -> float:
        """تطبيق الأنماط المتعلمة"""
        ...
        if rsi_range:
            if rsi_range.get('optimal_low', 0) <= rsi <= rsi_range.get('optimal_high', 100):
                confidence += 0.1 * self.learning_rate
            elif rsi < rsi_range.get('min', 0) or rsi > rsi_range.get('max', 100):
        ...
        if vol_range:
            if vol_range.get('optimal_low', 0) <= volatility <= vol_range.get('optimal_high', 1):
        ...
        if vol_range and volume > 0:
            if vol_range.get('optimal_low', 0) <= volume <= vol_range.get('optimal_high', float('inf')):
                confidence += 0.05 * self.learning_rate
    ...
    def record_decision(self, decision: Dict, was_correct: bool):
        ...
        if len(self.recent_decisions) >= 20:
            recent_accuracy = sum(1 for d in list(self.recent_decisions)[-20:] if d['was_correct']) / 20
...
class DualPathDecision:
    ...
    def decide(self, signal_data: Dict, learned_patterns: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        decision = {
            ...
            'explanation': self._generate_explanation(c_decision, b_decision, both_agree)
        }
        
        logger.debug(f"🎯 قرار مزدوج: {action} (ثقة: {final_confidence:.2%}, إجماع: {both_agree})")
        
        return decision
    
    def _generate_explanation(self, c_dec: Dict, b_dec: Dict, consensus: bool) -> str:
        """توليد شرح للقرار"""
        
        if consensus and c_dec['action'] == 'trade':
            return f"✅ إجماع على التداول (محافظ: {c_dec['confidence']:.2%}, متوازن: {b_dec['confidence']:.2%})"
        elif consensus and c_dec['action'] == 'skip':
            return f"⛔ إجماع على التجنب (محافظ: {c_dec['confidence']:.2%}, متوازن: {b_dec['confidence']:.2%})"
        elif c_dec['action'] == 'trade' and b_dec['action'] == 'skip':
            return f"⚠️ اختلاف: محافظ يوافق ({c_dec['confidence']:.2%}) لكن متوازن يرفض ({b_dec['confidence']:.2%})"
        else:
            return f"⚠️ اختلاف: متوازن يوافق ({b_dec['confidence']:.2%}) لكن محافظ يرفض ({c_dec['confidence']:.2%})"
    ...
    def update_from_result(self, decision: Dict, trade_result: Dict):
        ...
        # تحديث كل نظام
        self.conservative.record_decision(decision['conservative_decision'], was_correct)
        self.balanced.record_decision(decision['balanced_decision'], was_correct)
    ...
    def _auto_adjust_weights(self):
        ...
        if total > 0:
            ...
            self.weights['conservative'] = (
                (1 - alpha) * self.weights['conservative'] +
                alpha * new_weights['conservative']
            )
            self.weights['balanced'] = (
                (1 - alpha) * self.weights['balanced'] +
                alpha * new_weights['balanced']
            )
            
            logger.debug(f"⚖️ تحديث الأوزان: محافظ={self.weights['conservative']:.2%}, متوازن={self.weights['balanced']:.2%}")
    ...
    def get_performance_summary(self) -> Dict:
        ...
        return {
            ...
            'total_decisions': len(self.conservative.recent_decisions) + len(self.balanced.recent_decisions)
        }
