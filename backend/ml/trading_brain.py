...
class MistakeMemory:
    ...
    def __init__(self, max_mistakes: int = 500):
        ...
        self.mistake_patterns: Dict[str, Dict] = defaultdict(lambda: {
            'count': 0,
            'total_loss': 0,
    ...
    def record_mistake(self, trade_data: Dict, market_conditions: Dict, reason: str):
        ...
        logger.warning(f"🧠 تم تسجيل خطأ: {reason} | {trade_data.get('symbol')} | خسارة: ${abs(trade_data.get('profit_loss', 0)):.2f}")
    ...
    def _analyze_pattern(self, pattern_key: str, pattern: Dict):
        ...
        if avg_loss > 3 and pattern['count'] >= 3:
            ...
            logger.info(f"🧠 اكتشاف نمط خطأ جديد: {pattern_key} | تكرار: {pattern['count']} | متوسط الخسارة: ${avg_loss:.2f}")
    ...
    def should_avoid(self, market_conditions: Dict) -> Dict[str, Any]:
        ...
        current_pattern = self._classify_mistake({'market_conditions': market_conditions})
...
class TradingBrain:
    ...
    def _init_ml_systems(self):
        ...
        try:
            ...
            from backend.ml.pattern_similarity_matcher import get_similarity_matcher
    ...
    def think(self, signal: Dict, market_data: Dict) -> Dict[str, Any]:
        ...
        if self.ml_available and self.ml_classifier:
            ...
            decision['adjusted_confidence'] *= ml_result.get('confidence_multiplier', 1.0)
        ...
        if self.ml_available and self.similarity_matcher:
            ...
            if similarity_result['similarity'] < 0.4:
                ...
                decision['reasoning'].append(f"⚠️ تشابه منخفض: {similarity_result['similarity']:.0%}")
                # لا نرفض، لكن نقلل الثقة
                decision['adjusted_confidence'] *= 0.85
            else:
                decision['checks_passed'].append('pattern_similarity')
                decision['reasoning'].append(f"✅ تشابه: {similarity_result['similarity']:.0%}")
        ...
        if rules_result['violations']:
            ...
            decision['reasoning'].append(f"⚠️ انتهاك قواعد: {rules_result['violations']}")
            decision['adjusted_confidence'] *= 0.8
        else:
            decision['checks_passed'].append('learned_rules')
        
        # ========== 6. القرار النهائي ==========
        threshold = self.confidence_thresholds.get(symbol, self.confidence_thresholds['default'])
        
        if decision['adjusted_confidence'] >= threshold:
            decision['final_decision'] = 'APPROVE'
            decision['reasoning'].append(f"✅ الثقة النهائية: {decision['adjusted_confidence']:.0%} >= {threshold:.0%}")
            self.stats['approved'] += 1
        else:
            decision['final_decision'] = 'REJECT'
            decision['rejection_reason'] = 'low_confidence'
            decision['reasoning'].append(f"❌ الثقة منخفضة: {decision['adjusted_confidence']:.0%} < {threshold:.0%}")
            self._record_rejection(decision)
    ...
    def _check_ml_classifier(self, signal: Dict, market_data: Dict) -> Dict:
        ...
        try:
            if not self.ml_classifier or not self.ml_classifier.is_ready():
                return {'approved': True, 'reason': 'ML not ready', 'confidence_multiplier': 1.0}
            ...
            return {'approved': True, 'reason': 'ML error', 'confidence_multiplier': 1.0}
    ...
    def _check_pattern_similarity(self, market_data: Dict) -> Dict:
        ...
        try:
            ...
            similarity = self.similarity_matcher.calculate_similarity(current, reference_pattern)
    ...
    def _log_decision(self, decision: Dict):
        """تسجيل القرار"""
        status = "✅ APPROVE" if decision['final_decision'] == 'APPROVE' else "❌ REJECT"
        logger.info(f"🧠 {decision['symbol']} | {status} | "
            ...
            if is_win:
                ...
                if symbol in self.confidence_thresholds:
                    self.confidence_thresholds[symbol] = min(
                        0.75,
                        self.confidence_thresholds[symbol] + 0.02
                    )
                    logger.info(f"🧠 رفع عتبة الثقة لـ {symbol}: {self.confidence_thresholds[symbol]:.0%}")
