...
class MLSignalClassifier:
    ...
    MIN_ACCURACY_FOR_READINESS = 0.65   # الحد الأدنى للدقة (65% - أفضل من العشوائية بـ 15%)
    ...
    def __init__(self, model_dir: str = None):
        ...
        self.feature_columns = [
            ...
            'plus_di', 'minus_di',    # مؤشرات الاتجاه
            'ema_cross',              # تقاطع EMAs (1=صاعد, -1=هابط, 0=لا تقاطع)
            
            # ========== مؤشرات MeanReversion ==========
            'stoch_k', 'stoch_d',     # Stochastic
            'distance_from_mean',     # المسافة من المتوسط
            'oversold', 'overbought', # تشبع بيعي/شرائي
            ...
            'rsi_divergence',         # تباعد RSI (1=إيجابي, -1=سلبي, 0=لا تباعد)
            ...
            'strategy_trend_following', 'strategy_momentum_breakout',
            'strategy_mean_reversion', 'strategy_scalping',
            'strategy_rsi_divergence', 'strategy_volume_price',
            'strategy_other',
        ...
        self.known_strategies = [
            'TrendFollowing', 'MomentumBreakout', 'MeanReversion',
            'ScalpingEMA', 'RSIDivergence', 'VolumePriceTrend'
        ]
    ...
    def extract_features_from_backtest(self, backtest_result: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            features = {
                ...
                # EMA
                'ema_trend': 1 if indicators.get('ema_trend', 'neutral') == 'up' else (
                    -1 if indicators.get('ema_trend', 'neutral') == 'down' else 0
                ),
                ...
                # Price Changes
                'price_change_1h': np.clip(indicators.get('price_change_1h', 0), -10, 10),
                'price_change_4h': np.clip(indicators.get('price_change_4h', 0), -20, 20),
            ...
            timeframe = backtest_result.get('timeframe', backtest_result.get('selected_timeframe', '1h'))
            
            # تحويل الاستراتيجية إلى One-Hot Encoding
            strategy_lower = strategy.lower().replace('_', '').replace(' ', '')
            features['strategy_trend_following'] = 1 if 'trendfollowing' in strategy_lower else 0
            features['strategy_momentum_breakout'] = 1 if 'momentumbreakout' in strategy_lower else 0
            features['strategy_mean_reversion'] = 1 if 'meanreversion' in strategy_lower else 0
            features['strategy_scalping'] = 1 if 'scalping' in strategy_lower or 'ema' in strategy_lower else 0
            features['strategy_rsi_divergence'] = 1 if 'rsi' in strategy_lower or 'divergence' in strategy_lower else 0
            features['strategy_other'] = 1 if sum([
    ...
    def train(self, force: bool = False, timestamps: Optional[np.ndarray] = None) -> Dict[str, Any]:
        """
    ...
    def _check_data_quality(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'imbalance_ratio': float(max(counts) / min(counts)) if len(unique) >= 2 else None
            }
    ...
    def _check_stability(self, current_accuracy: float):
        ...
        if len(cycles) >= self.MIN_STABLE_CYCLES:
            # فحص آخر 3 دورات
            recent_accuracies = [c['accuracy'] for c in cycles[-self.MIN_STABLE_CYCLES:]]
            
            # مستقر إذا: جميع الدقات > الحد الأدنى والتذبذب < 10%
            all_above_threshold = all(a >= self.MIN_ACCURACY_FOR_READINESS for a in recent_accuracies)
            low_variance = max(recent_accuracies) - min(recent_accuracies) < 0.10  # 10% بدلاً من 5%
            
            # عدد الدورات المستقرة
            stable_count = sum(1 for a in recent_accuracies if a >= self.MIN_ACCURACY_FOR_READINESS)
            self.training_history['stable_cycles'] = stable_count
            
            # تحديث حالة الجاهزية (استخدام test accuracy)
            is_ready = (
                total_samples >= self.MIN_SAMPLES_FOR_READINESS and
                current_accuracy >= self.MIN_ACCURACY_FOR_READINESS and
                len(self.training_history) >= self.MIN_STABLE_CYCLES and
                stable_count >= 2 and
                all_above_threshold and low_variance
            )
    ...
    def is_ready(self) -> bool:
        ...
        return (
            self.training_history.get('is_ready', False) and
            self.training_history.get('total_samples', 0) >= self.MIN_SAMPLES_FOR_READINESS and
            self.training_history.get('best_accuracy', 0) >= self.MIN_ACCURACY_FOR_READINESS
        )
    ...
    def _calculate_dynamic_threshold(self) -> float:
        ...
        try:
            ...
            avg_accuracy = sum(c.get('test_accuracy', c.get('accuracy', 0)) for c in recent) / len(recent)
    ...
    def predict(self, features: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            threshold = self._calculate_dynamic_threshold() if self.USE_DYNAMIC_THRESHOLD else self.BASE_CONFIDENCE_THRESHOLD
            ...
            return {
                ...
                'reason': 'موافق' if should_trade else f'ثقة منخفضة ({proba:.1%} < {threshold:.1%})'
            }
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'confidence_threshold': self._calculate_dynamic_threshold() if self.USE_DYNAMIC_THRESHOLD else self.BASE_CONFIDENCE_THRESHOLD,
            'threshold_type': 'dynamic' if self.USE_DYNAMIC_THRESHOLD else 'static'
        }
