...
class LiquiditySweep:
    ...
    def calculate_sweep_distance(self) -> float:
        """حساب مسافة الكنس"""
        return (
            abs(self.sweep_price - self.trigger_price)
            / self.trigger_price
            * 100
        )
    ...
    def calculate_reversal_strength(self) -> float:
        """حساب قوة الارتداد"""
        return (
            abs(self.reversal_price - self.sweep_price)
            / self.sweep_price
            * 100
        )

    def __repr__(self):
        return f"LiquiditySweep({
            self.pattern}:{
            self.sweep_type} @ {
            self.sweep_price:.6f}, strength={
                self.strength:.1f})"
...
class LiquiditySweepDetector:
    ...
    def detect_liquidity_sweeps(
        self, df: pd.DataFrame, liquidity_zones: List[LiquidityZone]
    ) -> List[LiquiditySweep]:
        """
    ...
    def _check_bullish_fakeout(
        ...
        try:
            ...
            # 1. اختراق المقاومة
            penetration = (
                current_candle["high"] - resistance_level
            ) / resistance_level
            if (
                self.fakeout_min_penetration
                <= penetration
                <= self.fakeout_max_penetration
            ):
            ...
            if (
                ...
                > avg_volume * self.volume_spike_threshold
            ):
            ...
            if index + 2 < len(df):
                ...
                reversal_strength = (
                    current_candle["close"] - lowest_after
            ...
            if len(conditions) >= 3:  # 3 من 4 شروط
                strength = (
                    len(conditions) * 20 + penetration * 1000
                )  # تسجيل القوة
    ...
    def _check_bearish_fakeout(
        ...
        try:
            ...
            # 1. اختراق الدعم
            penetration = (
                support_level - current_candle["low"]
            ) / support_level
            if (
                self.fakeout_min_penetration
                <= penetration
            ...
            if index + 2 < len(df):
                ...
                reversal_strength = (
                    highest_after - current_candle["close"]
                ) / current_candle["close"]
                if reversal_strength >= self.fakeout_reversal_min:
                    conditions.append("strong_reversal")
    ...
    def _check_zone_sweep(
        self, df: pd.DataFrame, zone: LiquidityZone
    ) -> List[LiquiditySweep]:
        ...
        try:
            ...
            for i in range(5, len(df)):
                ...
                if zone.zone_type in ["resistance", "pivot"]:
                    if (
                        current_candle["high"]
                        > zone_price * (1 + tolerance / 2)
                        and current_candle["close"] < zone_price
                    ):

                        # تأكيد بالحجم
                        avg_volume = (
                            df["volume"].iloc[max(0, i - 5): i].mean()
                        )
    ...
    def _detect_equal_level_sweeps(
        self, df: pd.DataFrame
    ) -> List[LiquiditySweep]:
        """كشف كنس القمم/القيعان المتساوية"""
        sweeps = []
        ...
        try:
            ...
            for level_price, indices in equal_highs.items():
                level_sweeps = self._check_equal_high_sweep(
                    df, level_price, indices
                )
            ...
            for level_price, indices in equal_lows.items():
                level_sweeps = self._check_equal_low_sweep(
                    df, level_price, indices
                )
                sweeps.extend(level_sweeps)
    ...
    def _find_equal_highs(
        self, df: pd.DataFrame, tolerance: float = 0.002
    ) -> Dict[float, List[int]]:
        """البحث عن القمم المتساوية"""
        equal_highs = {}
    ...
    def _check_equal_high_sweep(
        self, df: pd.DataFrame, level_price: float, indices: List[int]
    ) -> List[LiquiditySweep]:
        ...
        try:
            ...
            for i in range(last_peak_idx + 1, len(df)):
                ...
                if (
                    current_candle["high"] > level_price * 1.001  # كسر 0.1%
                    and current_candle["close"] < level_price * 0.999
                ):  # إغلاق أسفل
    ...
    def _check_equal_low_sweep(
        ...
        try:
            ...
            for i in range(last_trough_idx + 1, len(df)):
                ...
                if (
                    current_candle["low"] < level_price * 0.999  # كسر 0.1%
                    and current_candle["close"] > level_price * 1.001
                ):  # إغلاق فوق
    ...
    def _filter_and_rank_sweeps(
        ...
        try:
            ...
            unique_sweeps.sort(
                key=lambda s: (s.strength, s.timestamp), reverse=True
            )
    ...
    def analyze_sweep_patterns(self, sweeps: List[LiquiditySweep]) -> Dict:
        ...
        try:
            ...
            analysis["recent_sweeps"] = [
                ...
                and s.timestamp > recent_threshold
            ]

            # أقوى العمليات
            analysis["strongest_sweeps"] = sorted(
                sweeps, key=lambda s: s.strength, reverse=True
            )[:5]
