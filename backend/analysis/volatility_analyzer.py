...
class VolatilityAnalyzer:
    ...
    def analyze(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "position_size_adjustment": self._get_position_adjustment(
                    regime
                ),
            }
    ...
    def _calculate_historical_volatility(
        ...
        try:
            ...
            return (
                float(hist_vol.iloc[-1])
                if not pd.isna(hist_vol.iloc[-1])
                else 0
            )
    ...
    def _calculate_parkinson_volatility(
        ...
        try:
            ...
            parkinson = hl_ratio.rolling(window=period).apply(
                lambda x: np.sqrt(
                    (1 / (4 * len(x) * np.log(2))) * np.sum(x**2)
                )
            ) * np.sqrt(365)

            return (
                float(parkinson.iloc[-1])
                if not pd.isna(parkinson.iloc[-1])
                else 0
            )
