...
from .liquidity_zones_detector import LiquidityZonesDetector
from .vwap_analyzer import VWAPAnalyzer
from .liquidity_sweep_detector import LiquiditySweepDetector
from .order_blocks_detector import OrderBlocksDetector
from .fair_value_gaps_detector import FairValueGapsDetector
...
class SmartMoneySignal:
    ...
    def __repr__(self):
        return f"SmartMoneySignal({
            self.signal_type}, {
            self.confidence:.1f}%, target={
            self.price_target:.6f})"
...
class SmartMoneyOrchestrator:
    ...
    def analyze_smart_money_activity(
        ...
        try:
            ...
            risk_assessment = self._assess_risk_opportunities(
                analysis_data, df_5m
            )
            ...
            return {
                ...
                "summary": self._create_analysis_summary(
                    analysis_data, confluence_score
                ),
            }
    ...
    def _gather_component_data(
        ...
        try:
            ...
            zones_result = self.liquidity_zones.detect_all_zones(
                symbol, df_15m, df_5m
            )
            ...
            sweeps_result = self.liquidity_sweeps.detect_liquidity_sweeps(
                df_5m, liquidity_zones_list
            )
    ...
    def _score_liquidity_zones(self, zones_data: Dict) -> float:
        ...
        try:
            ...
            # متوسط قوة المناطق
            avg_strength = sum(zone.strength for zone in all_zones) / len(
                all_zones
            )
    ...
    def _score_liquidity_sweeps(self, sweeps_data: List) -> float:
        ...
        try:
            ...
            avg_strength = sum(sweep.strength for sweep in sweeps_data) / len(
                sweeps_data
            )
    ...
    def _score_order_blocks(self, blocks_data: List) -> float:
        ...
        try:
            ...
            # متوسط قوة الكتل
            avg_strength = sum(block.strength for block in blocks_data) / len(
                blocks_data
            )
    ...
    def _score_fair_value_gaps(self, fvg_data: List) -> float:
        ...
        try:
            ...
            avg_strength = sum(gap.strength for gap in active_gaps) / len(
                active_gaps
            )
    ...
    def _generate_composite_signal(
        ...
        try:
            ...
            if confluence_score >= self.min_confluence_score:
                ...
                zones_signal = self._analyze_zones_signal(
                    analysis_data.get("liquidity_zones", {}), current_price
                )
                if zones_signal["signal"] != "NEUTRAL":
                    reasons.append(f"Liquidity Zones: {
                        zones_signal['reason']}")
                    supporting_data["zones"] = zones_signal

                # إشارات VWAP
                vwap_signals = analysis_data.get("vwap_analysis", {}).get(
                    "trading_signals", {}
                )
                if vwap_signals.get("primary_signal") not in [
                    "NEUTRAL",
                    "WAIT_FOR_DIRECTION",
                ]:
                ...
                if sweeps_data:
                    recent_sweeps = [
                        s for s in sweeps_data if s.get_age_hours() < 4
                    ]
                    if recent_sweeps:
                        sweep_signal = recent_sweeps[0].sweep_type
                        reasons.append(
                            f"Recent Liquidity Sweep: {sweep_signal}"
                        )
    ...
    def _analyze_zones_signal(
        self, zones_data: Dict, current_price: float
    ) -> Dict:
        ...
        try:
            ...
            for zone in all_zones:
                distance_pct = (
                    abs(zone.price - current_price) / current_price * 100
                )
            ...
            if (
                closest_zone.zone_type == "support"
                and current_price > closest_zone.price
            ):
                signal = {
                    "signal": "BUY",
                    "reason": f"Near strong support zone ({
                        closest_zone.source})",
                    "zones": [closest_zone],
                }
            elif (
                closest_zone.zone_type == "resistance"
                and current_price < closest_zone.price
            ):
                signal = {
                    "signal": "SELL",
                    "reason": f"Near strong resistance zone ({
                        closest_zone.source})",
                    "zones": [closest_zone],
                }
    ...
    def _assess_risk_opportunities(
        self, analysis_data: Dict, df: pd.DataFrame
    ) -> Dict:
        ...
        try:
            ...
            if recent_sweeps:
                ...
                assessment["risk_factors"].append(
                    "Recent liquidity sweep activity"
                )
            ...
            nearby_gaps = [
                ...
                if abs(g.get_midpoint() - current_price) / current_price < 0.03
            ]
            if nearby_gaps:
                risk_score += 15
                assessment["risk_factors"].append(
                    f"{len(nearby_gaps)} unfilled gaps nearby"
                )
            ...
            zones_data = analysis_data.get("liquidity_zones", {})
            strong_zones = [
                z for z in zones_data.get("all_zones", []) if z.strength > 80
            ]
            if strong_zones:
                assessment["opportunities"].append(
                    f"{len(strong_zones)} high-strength liquidity zones"
                )
    ...
    def _create_analysis_summary(
        self, analysis_data: Dict, confluence_score: float
    ) -> Dict:
        """إنشاء ملخص التحليل"""
        summary = {
            "confluence_score": confluence_score,
        ...
        try:
            ...
            summary["components_summary"] = {
                "liquidity_zones": len(
                    analysis_data.get("liquidity_zones", {}).get(
                        "all_zones", []
                    )
                ),
                "liquidity_sweeps": len(
                    analysis_data.get("liquidity_sweeps", [])
                ),
                "order_blocks": len(analysis_data.get("order_blocks", [])),
                "fair_value_gaps": len(
                    analysis_data.get("fair_value_gaps", [])
                ),
                "vwap_strength": analysis_data.get("vwap_analysis", {})
                .get("vwap_strength", {})
                .get("overall_strength", 0),
            }

            # النتائج الرئيسية
            if confluence_score >= 80:
                summary["key_findings"].append(
                    "High confluence across multiple components"
                )
    ...
    def _get_error_result(self, symbol: str, error_msg: str) -> Dict:
        ...
        return {
            ...
            "smart_money_signal": SmartMoneySignal(
                "WAIT", 0, 0, ["Analysis Error"], {}, datetime.now()
            ),
        }
