...
class MarketRegimeDetector:
    ...
    def _calculate_normalized_atr(self, df: pd.DataFrame, period: int = 14) -> float:
        ...
        try:
            ...
            return float(atr_normalized.iloc[-1]) if not pd.isna(atr_normalized.iloc[-1]) else 0
    ...
    def _classify_regime(self, adx: float, atr_normalized: float, bb_width: float) -> tuple:
        """
        ...
        # Trending + Volatile
        if adx > self.adx_trending_threshold and atr_normalized > self.atr_high_threshold:
            confidence = min((adx / 40) * 100, 100)
            return 'TRENDING_VOLATILE', confidence
        
        # Trending + Calm
        elif adx > self.adx_trending_threshold and atr_normalized < self.atr_low_threshold:
        ...
        # Choppy + Volatile (سيء)
        elif adx < self.adx_ranging_threshold and atr_normalized > self.atr_high_threshold:
            confidence = 70
