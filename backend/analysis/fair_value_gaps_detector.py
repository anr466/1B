...
class FairValueGap:
    ...
    def update_fill_status(self, price: float, timestamp: datetime) -> str:
        ...
        if (
            abs(price - self.bottom_price) / self.bottom_price <= tolerance
            or abs(price - self.top_price) / self.top_price <= tolerance
        ):
            self.retest_count += 1
    ...
    def is_active(self, max_age_hours: int = 72) -> bool:
        ...
        return (
            self.status in ["unfilled", "partial"]
            and self.get_age_hours() < max_age_hours
        )

    def __repr__(self):
        return f"FVG({
            self.gap_type} @ {
            self.bottom_price:.6f}-{
            self.top_price:.6f}, {
...
class FairValueGapsDetector:
        ...
        try:
            for i in range(2, len(df) - 1):  # نحتاج 3 شموع للتحليل
                ...
                if candle_after["low"] > candle_before["high"]:
                    ...
                    if (
                        self.min_gap_size_pct
                        <= gap_size_pct
                        <= self.max_gap_size_pct
                    ):

                        # التحقق من قوة الحركة (حجم)
                        avg_volume = (
                            df["volume"].iloc[max(0, i - 10): i].mean()
                        )
                        formation_volume = (
                            candle_before["volume"]
                            + candle_current["volume"]
                            + candle_after["volume"]
                        ) / 3

                        volume_strength = (
                            formation_volume / avg_volume
                            if avg_volume > 0
                            else 1
                        )
                        ...
                        # حساب قوة الفجوة
                        price_momentum = (
                            (candle_after["close"] - candle_before["open"])
                            / candle_before["open"]
                            * 100
                        )
                        gap_strength = min(
                        ...
                        if gap_strength >= self.min_gap_strength:
                            # تحديد نمط التكوين
                            if (
                                candle_before["close"] > candle_before["open"]
                                and candle_current["close"]
                                > candle_current["open"]
                                and candle_after["close"]
                                > candle_after["open"]
                            ):
    ...
    def _update_gap_statuses(
        self, gaps: List[FairValueGap], df: pd.DataFrame
    ) -> List[FairValueGap]:
            ...
            for gap in gaps:
                ...
                if gap_formation_idx is not None:
                    ...
                    for i in range(gap_formation_idx, len(df)):
                        ...
                        # تحديث حالة الملء
                        gap.status
                        new_status = gap.update_fill_status(
                            candle["close"], candle.name
                        )

                        # فحص اختراق كامل للفجوة
                        if (
                            gap.gap_type == "bullish"
                            and candle["low"] < gap.bottom_price
                        ) or (
                            gap.gap_type == "bearish"
                            and candle["high"] > gap.top_price
                        ):
                            gap.status = "filled"
    ...
    def _filter_and_rank_gaps(
        ...
        try:
            ...
            fresh_gaps = [
                gap
                for gap in gaps
                if gap.get_age_hours() <= self.max_gap_age_hours
            ]

            # ترتيب حسب الأولوية: غير مملوءة > جزئي > قوة > حداثة
            def gap_priority(gap):
                status_priority = {"unfilled": 3, "partial": 2, "filled": 1}[
                    gap.status
                ]
                age_factor = max(
                    0, 1 - (gap.get_age_hours() / self.max_gap_age_hours)
                )
        ...
        try:
            ...
            for gap in gaps:
                ...
                if gap.status == "filled":
                    ...
                    if (
                        gap.fill_timestamp
                        and isinstance(gap.timestamp, datetime)
                        and isinstance(gap.fill_timestamp, datetime)
                    ):
                        fill_time = (
                            gap.fill_timestamp - gap.timestamp
                        ).total_seconds() / 3600
                        fill_times.append(fill_time)

            # متوسط وقت الملء
            if fill_times:
                analysis["average_fill_time"] = sum(fill_times) / len(
                    fill_times
                )
            ...
            if analysis["total_gaps"] > 0:
                analysis["fill_rate"] = (
                    analysis["by_status"]["filled"] / analysis["total_gaps"]
                ) * 100
    ...
    def get_relevant_gaps(
        ...
        try:
            for gap in gaps:
                ...
                distance = (
                    abs(current_price - gap_midpoint) / current_price * 100
                )
            ...
            relevant_gaps.sort(
                key=lambda g: (
                    abs(current_price - g.get_midpoint()),
                    -g.strength,
                )
            )
    ...
    def get_nearest_gaps(
        ...
        try:
            ...
            for gap in active_gaps:
                ...
                if gap_midpoint > current_price and direction in [
                    "both",
                    "above",
                ]:
                ...
                elif gap_midpoint < current_price and direction in [
                    "both",
                    "below",
                ]:
    ...
    def generate_fvg_signals(
        ...
        try:
            ...
            support_gaps = [
                ...
                and gap.get_midpoint() < current_price
            ]
            resistance_gaps = [
                gap
                for gap in relevant_gaps
                if gap.gap_type == "bearish"
                and gap.get_midpoint() > current_price
            ]
            ...
            if price_trend == "upward" and resistance_gaps:
                ...
                nearest_resistance = min(
                    resistance_gaps,
                    key=lambda g: abs(g.get_midpoint() - current_price),
                )
                ...
                and nearest["distance_below"] / current_price < 0.02
            ):  # أقل من 2%
                # قريب من فجوة سفلية
                ...
                and nearest["distance_above"] / current_price < 0.02
            ):  # أقل من 2%
                # قريب من فجوة علوية
