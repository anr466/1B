...
class LiquidityZone:
    ...
    def __init__(self, price: float, zone_type: str, strength: float, 
    ...
    def test_zone(self, price: float, timestamp: datetime, tolerance: float = 0.002):
    ...
    def __repr__(self):
        return f"LiquidityZone({self.source}:{self.zone_type} @ {self.price:.6f}, strength={self.strength:.1f})"
...
class LiquidityZonesDetector:
    ...
    def __init__(self):
        ...
        # إعدادات Fibonacci
        self.fib_levels = [0.236, 0.382, 0.5, 0.618, 0.786]  # مستويات فيبوناتشي
        self.fib_lookback = 50        # نافزة البحث عن القمم/القيعان للفيبوناتشي
        
        # إعدادات VWAP
        self.vwap_std_multipliers = [1.0, 1.5, 2.0, 2.5]  # مضاعفات الانحراف المعياري
        
    def detect_all_zones(self, symbol: str, df_15m: pd.DataFrame, 
                        df_5m: pd.DataFrame) -> Dict[str, List[LiquidityZone]]:
        """
    ...
    def detect_swing_zones(self, df: pd.DataFrame) -> List[LiquidityZone]:
        ...
        try:
            for i in range(self.swing_lookback, len(df) - self.swing_lookback):
                # البحث عن قمة سوينغ
                high_window = df['high'].iloc[i-self.swing_lookback:i+self.swing_lookback+1]
                ...
                # البحث عن قاع سوينغ
                low_window = df['low'].iloc[i-self.swing_lookback:i+self.swing_lookback+1]
    ...
    def detect_vwap_zones(self, df: pd.DataFrame) -> List[LiquidityZone]:
        ...
        try:
            ...
            df['price_variance'] = (df['typical_price'] - df['vwap']) ** 2 * df['volume']
            df['cumulative_price_variance'] = df['price_variance'].cumsum()
            df['variance'] = df['cumulative_price_variance'] / df['cumulative_volume']
            ...
            for multiplier in self.vwap_std_multipliers:
                ...
                upper_zone = LiquidityZone(
                    price=upper_level,
                    zone_type='resistance',
                    strength=70 - (multiplier * 10),  # كلما زاد المضاعف قلت القوة
    ...
    def _calculate_swing_strength(self, df: pd.DataFrame, index: int, 
        ...
        try:
            if swing_type == 'high':
                ...
                right_prices = df['high'].iloc[index+1:min(len(df), index+11)]
            else:
                price = df['low'].iloc[index]
                left_prices = df['low'].iloc[max(0, index-10):index]
                right_prices = df['low'].iloc[index+1:min(len(df), index+11)]
            ...
            volume_strength = df['volume'].iloc[index] / df['volume'].iloc[max(0, index-20):index+20].mean()
    ...
    def _find_equal_levels(self, swing_points: List[Dict], 
        ...
        for group in price_groups:
            if len(group) >= 2:
                ...
                strength = min(90, 50 + (len(group) * 10) + min(20, total_volume / 1000000))
    ...
    def _create_zones_summary(self, zones: Dict) -> Dict:
        ...
        # أقوى 5 مناطق
        summary['strongest_zones'] = sorted(all_zones, key=lambda z: z.strength, reverse=True)[:5]
    ...
    def get_zones_near_price(self, zones: List[LiquidityZone], 
                           current_price: float, 
                           distance_pct: float = 2.0) -> List[LiquidityZone]:
