...
class VWAPBand:
    ...
    def test_touch(
        self, price: float, timestamp: datetime, tolerance: float = 0.001
    ):
        """اختبار لمس النطاق"""
...
class VWAPAnalyzer:
    ...
    def analyze_vwap_structure(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            vwap_strength = self._evaluate_vwap_strength(
                vwap_data, price_behavior
            )
            ...
            return {
                ...
                "trading_signals": self._generate_vwap_signals(
                    vwap_data, price_behavior
                ),
            }
    ...
    def _calculate_vwap_with_std(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for date in df["date"].unique():
            ...
            day_data["cumulative_volume_price"] = day_data[
                "volume_price"
            ].cumsum()
            day_data["vwap"] = (
                day_data["cumulative_volume_price"]
                / day_data["cumulative_volume"]
            )

            # حساب الانحراف المعياري
            day_data["price_variance"] = (
                day_data["typical_price"] - day_data["vwap"]
            ) ** 2 * day_data["volume"]
            day_data["cumulative_price_variance"] = day_data[
                "price_variance"
            ].cumsum()
            day_data["variance"] = (
                day_data["cumulative_price_variance"]
                / day_data["cumulative_volume"]
            )
        ...
        for multiplier in self.std_multipliers:
            result_df[f"vwap_upper_{multiplier}"] = result_df["vwap"] + (
                result_df["vwap_std"] * multiplier
            )
            result_df[f"vwap_lower_{multiplier}"] = result_df["vwap"] - (
                result_df["vwap_std"] * multiplier
            )
    ...
    def _analyze_vwap_bands(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            bands_data["band_interactions"] = (
                self._analyze_historical_interactions(df)
            )
    ...
    def _detect_vwap_liquidity_zones(
    ...
    def _analyze_price_behavior_with_vwap(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            vwap_change = (
                (recent_vwap.iloc[-1] - recent_vwap.iloc[0])
                / recent_vwap.iloc[0]
                * 100
            )
            ...
            # تحليل الحجم
            behavior["volume_profile"] = (
                self._analyze_volume_profile_with_vwap(df)
            )

            # التفاعلات الأخيرة
            behavior["recent_interactions"] = (
                self._find_recent_vwap_interactions(df)
            )

            # إشارات السلوك
            behavior["behavioral_signals"] = self._generate_behavioral_signals(
                df, behavior
            )
    ...
    def _analyze_volume_profile_with_vwap(self, df: pd.DataFrame) -> str:
        ...
        try:
            ...
            volume_above = (
                above_vwap["volume"].sum() if len(above_vwap) > 0 else 0
            )
            volume_below = (
                below_vwap["volume"].sum() if len(below_vwap) > 0 else 0
            )
    ...
    def _find_recent_vwap_interactions(
        ...
        try:
            ...
            for i in range(1, len(recent_data)):
                ...
                for multiplier in [1.0, 2.0]:
                    ...
                    # لمس النطاق العلوي
                    if (
                        current_row["high"] >= upper_band
                        and prev_row["high"]
                        < prev_row[f"vwap_upper_{multiplier}"]
                    ):
                        interactions.append(
                            {
                    ...
                    # لمس النطاق السفلي
                    if (
                        current_row["low"] <= lower_band
                        and prev_row["low"]
                        > prev_row[f"vwap_lower_{multiplier}"]
                    ):
                        interactions.append(
                            {
    ...
    def _generate_behavioral_signals(
        self, df: pd.DataFrame, behavior: Dict
    ) -> List[str]:
        """توليد إشارات السلوك"""
        signals = []
        ...
        try:
            ...
            if (
                behavior["position_relative_to_vwap"] == "above"
                and behavior["vwap_slope"] == "rising"
            ):
                signals.append("STRONG_BULLISH_MOMENTUM")
            elif (
                behavior["position_relative_to_vwap"] == "below"
                and behavior["vwap_slope"] == "falling"
            ):
            ...
            # إشارة الحجم
            if (
                behavior["volume_profile"] == "bullish"
                and behavior["position_relative_to_vwap"] == "above"
            ):
                signals.append("VOLUME_CONFIRMS_BULLISH")
            elif (
                behavior["volume_profile"] == "bearish"
                and behavior["position_relative_to_vwap"] == "below"
            ):
    ...
    def _evaluate_vwap_strength(
        ...
        try:
            ...
            interaction_types = set(
                i["type"] for i in behavior["recent_interactions"]
            )
            diversity_score = min(20, len(interaction_types) * 4)

            # استقرار VWAP
            vwap_volatility = (
                df["vwap"].rolling(20).std().iloc[-1] / df["vwap"].iloc[-1]
            )
            # كلما قل التقلب زادت النقاط
            stability_score = max(0, 30 - (vwap_volatility * 1000))
    ...
    def _generate_vwap_signals(self, df: pd.DataFrame, behavior: Dict) -> Dict:
        ...
        try:
            ...
            if -0.5 <= distance_pct <= 0.5:
                ...
                signals["entry_zones"] = [
                    current_analysis["nearest_upper_band"]
                ]
                ...
                signals["confidence"] = 70
                signals["entry_zones"] = [
                    current_analysis["nearest_lower_band"]
                ]
    ...
    def _get_error_result(self, error_msg: str) -> Dict:
        """نتيجة خطأ"""
        return {"error": "analysis_error", "message": error_msg}
