...
class OrderBlock:
    ...
    def __repr__(self):
        return f"OrderBlock({
            self.block_type} @ {
            self.price_range[0]:.6f}-{
            self.price_range[1]:.6f}, strength={
                self.strength:.1f})"
...
class OrderBlocksDetector:
    ...
    def detect_order_blocks(self, df: pd.DataFrame) -> List[OrderBlock]:
        ...
        try:
            ...
            self.logger.info(
                f"✅ تم العثور على {len(order_blocks)} كتلة أوامر"
            )
            return order_blocks
    ...
    def _detect_high_volume_blocks(self, df: pd.DataFrame) -> List[OrderBlock]:
        ...
        try:
            ...
            df = df.copy()
            df["volume_avg"] = (
                df["volume"].rolling(window=self.volume_lookback).mean()
            )
            ...
            for i in range(self.volume_lookback, len(df)):
                ...
                if (
                    ...
                    > avg_volume * self.min_volume_multiplier
                ):
                    ...
                    strength = min(
                        100, (volume_ratio * 20) + (price_range_pct * 100)
                    )
        ...
        try:
            ...
            for i in range(10, len(df) - 5):
                ...
                if range_pct < 2.0:  # أقل من 2%
                    ...
                    avg_volume_before = (
                        df["volume"].iloc[max(0, i - 20): i - 5].mean()
                    )
                    ...
                    if total_volume > avg_volume_before * len(window) * 1.5:
                        ...
                        volume_ratio = total_volume / (
                            avg_volume_before * len(window)
                        )
                        duration_bonus = min(20, len(window) * 2)
                        strength = min(
                            95, (volume_ratio * 30) + duration_bonus
                        )
    ...
    def _detect_volume_imbalance(self, df: pd.DataFrame) -> List[OrderBlock]:
        ...
        try:
            ...
            df["selling_volume"] = np.where(
                df["price_change"] < 0,
                df["volume"]
                * (abs(df["price_change"]) / (df["high"] - df["low"])),
                0,
            )
            ...
            for i in range(window, len(df)):
                ...
                if (
                    buy_vol > buy_avg * self.imbalance_threshold
                    and buy_vol > sell_vol * 2
                ):
                    # عدم توازن شرائي قوي
                    ...
                    sell_vol > sell_avg * self.imbalance_threshold
                    and sell_vol > buy_vol * 2
                ):
                    # عدم توازن بيعي قوي
            ...
            self.logger.debug(
                f"🔍 Volume imbalance blocks: {len(blocks)} كتلة"
            )
            return blocks
        ...
        try:
            ...
            for block in blocks:
                if isinstance(block.timestamp, datetime):
                    ...
                    # الاحتفاظ بالكتل بدون timestamp
                    fresh_blocks.append(block)
            ...
            final_blocks = sorted(
                merged_blocks, key=lambda b: b.strength, reverse=True
            )[:15]
    ...
    def _blocks_overlap(
        self, block1: OrderBlock, block2: OrderBlock, tolerance: float = 0.001
    ) -> bool:
        """فحص تداخل كتلتين"""
        try:
            ...
            # فحص التداخل
            overlap = not (
                range1_high < range2_low or range2_high < range1_low
            )

            # فحص القرب (ضمن تسامح معين)
            distance = min(
                abs(range1_low - range2_high), abs(range2_low - range1_high)
            )
            close_proximity = (
                distance / max(range1_low, range2_low) <= tolerance
            )
    ...
    def analyze_block_interactions(
        self, blocks: List[OrderBlock], df: pd.DataFrame
    ) -> Dict:
        ...
        try:
            ...
            # أقوى الكتل
            analysis["strongest_blocks"] = sorted(
                blocks, key=lambda b: b.strength, reverse=True
            )[:5]

            # كفاءة الكتل
            if analysis["tested_blocks"] > 0:
                analysis["block_efficiency"] = (
                    analysis["active_blocks"] / analysis["tested_blocks"]
                ) * 100
    ...
    def get_relevant_blocks(
        ...
        try:
            for block in blocks:
                ...
                distance = (
                    abs(current_price - block_midpoint) / current_price * 100
                )
            ...
            relevant_blocks.sort(
                key=lambda b: (
                    abs(current_price - b.get_midpoint()),
                    -b.strength,
                )
            )
