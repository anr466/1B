...
from backend.core.unified_system_manager import (
...
def start_system():
    ...
    try:
        ...
        # Use unified manager (which handles StateManager internally)
        result = unified_manager.start_system(user="api_user")

        # State is managed by UnifiedSystemManager -> StateManager -> DB
        # No additional sync needed - single write path
        ...
        return jsonify(
            {"success": False, "error": str(e), "code": "ALREADY_RUNNING", "pid": pid}
        ), 409
...
def stop_system():
    ...
    try:
        ...
        logger.warning(f"⚠️ {e}")
        return jsonify({"success": False, "error": str(e), "code": "NOT_RUNNING"}), 400
...
def get_status():
    ...
    try:
        ...
        heartbeat_data = (
            subsystems.get("heartbeat", {}) or raw_state.get("heartbeat", {}) or {}
        )

        result_data = dict(canonical_state)
        trading_state = str(
            result_data.get("trading_state") or result_data.get("state") or "STOPPED"
        ).upper()
        ...
        if heartbeat_seconds_ago is not None:
            ...
            result_data["heartbeat_status"] = (
                "healthy"
                if heartbeat_seconds_ago < 30
                else "warning"
                if heartbeat_seconds_ago < 60
                else "critical"
            )
        ...
        for component in ["group_b", "ml"]:
            comp_data = subsystems.get(component) or raw_state.get("activity", {}).get(
                component, {}
            )
            if not comp_data:
                continue
            ...
            component_status = {
                ...
                if (
                    ...
                    and (
                        (component == "group_b" and seconds_since < 120)
                        or (component == "ml" and seconds_since < 300)
                    )
                )
                else "idle",
            }
