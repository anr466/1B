...
def register_mobile_settings_routes(bp, shared):
    ...
    require_idempotency = shared.get(
        "require_idempotency", lambda *a: (lambda f: f)
    )
    prevent_concurrent_duplicates = shared.get(
        "prevent_concurrent_duplicates", lambda f: f
    )
    ...
    def resolve_key_state(db, user_id: int, is_admin: bool, is_demo: bool):
        ...
        has_configured_db_keys = bool(
            db_rows and db_rows[0].get("count", 0) > 0
        )
        env_keys_enabled = (
        ...
        logical_has_keys = (
            True if not keys_required_for_current_mode else actual_has_keys
        )
        return {
    ...
    def get_user_settings(user_id):
        ...
        try:
                ...
                with db.get_write_connection() as conn:
                    conn.execute(
                        ...
                        (
                            user_id,
                            effective_is_demo,
                            "demo" if effective_is_demo else "real",
                        ),
                    )
                    conn.commit()
            ...
            data = {
                ...
                "positionSizePercentage": as_float(
                    s.get("position_size_percentage")
                ),
            ...
            response_data, status_code = success_response(
                data, "تم جلب الإعدادات بنجاح"
            )
            return jsonify(response_data), status_code
        except Exception as e:
            logger.error(f"❌ خطأ Settings {user_id}: {e}")
            response_data, status_code = error_response(
                "خطأ في جلب الإعدادات", "SETTINGS_ERROR", 500
            )
            return jsonify(response_data), status_code
    ...
    def validate_trading_settings(user_id):
        ...
        try:
            ...
            key_state = resolve_key_state(
                db_manager,
                user_id,
                getattr(g, "current_user_type", None) == "admin",
                group_b.is_demo_trading,
            )
            ...
            can_trade_gate, gate_reason = group_b._check_risk_gates(
                open_positions, risk_balance
            )

            # 3. حساب حجم الصفقة
            position_size = (
                group_b._calculate_position_size(balance) if balance > 0 else 0
            )
            ...
            if balance < 10:
                errors.append(f"الرصيد غير كافٍ: ${
                    balance:.2f} (الحد الأدنى $10)")
            ...
            can_trade = len(errors) == 0 and settings.get(
                "trading_enabled", False
            )
            reason = (
                "جاهز للتداول"
                if can_trade
                else (errors[0] if errors else "التداول معطّل")
            )
            ...
            response_data, status_code = success_response(
                data, "تم التحقق من متطلبات التداول"
            )
            return jsonify(response_data), status_code
            ...
            response_data, status_code = error_response(
                "خطأ في التحقق من الإعدادات", "VALIDATION_ERROR", 500
            )
            return jsonify(response_data), status_code
    ...
    def update_user_settings(user_id):
        ...
        try:
            ...
            if trading_enabled:
                ...
                # ملاحظة: حجم الصفقة وعدد المراكز يتحكم فيها backend تلقائياً —
                # لا نتحقق منها هنا
                user_type_query = "SELECT user_type FROM users WHERE id = %s"
                user_type_result = db.execute_query(
                    user_type_query, (user_id,)
                )
                is_admin_user = (
                    user_type_result[0]["user_type"] == "admin"
                    if user_type_result
                    else False
                )

                # ✅ جلب وضع التداول الحالي
                current_context = get_trading_context(
                    db, user_id, requested_mode=requested_mode
                )
                ...
                if not skip_binance_check:
                    ...
                    has_keys = (
                        keys_result
                        and len(keys_result) > 0
                        and keys_result[0].get("api_key")
                    )
                ...
                # trade_amount: يُقرأ من DB إذا لم يُرسَل في الطلب (backend
                # يتحكم فيه)
                if "tradeAmount" in data:
                    trade_amount = float(data["tradeAmount"])
                else:
                    _amt_q = "SELECT trade_amount FROM user_settings WHERE user_id = %s AND is_demo = %s"
                    _amt_r = db.execute_query(
                        _amt_q, (user_id, current_is_demo)
                    )
                    trade_amount = (
                        float(_amt_r[0]["trade_amount"] or 100.0)
                        if _amt_r
                        else 100.0
                    )
                ...
                if skip_binance_check:
                    ...
                    available_balance = (
                        float(demo_result[0]["available_balance"] or 0)
                        if demo_result
                        else 0.0
                    )
                    logger.info(f"✅ Demo balance for admin {user_id}: {
                        available_balance:.2f} USDT")
                else:
                    try:
                        from backend.utils.binance_balance_checker import (
                            BinanceBalanceChecker,
                        )

                        balance_checker = BinanceBalanceChecker(user_id)
                        balance_result = (
                            balance_checker.get_available_balance()
                        )

                        if balance_result.get("success"):
                            available_balance = float(
                                balance_result.get("free_usdt", 0)
                            )
                            logger.info(
                                f"✅ جلب رصيد Binance للمستخدم {user_id}: {
                                    available_balance:.2f} USDT"
                            )
                        else:
                            binance_error = balance_result.get(
                                "error", "خطأ في الاتصال بـ Binance"
                            )
                    except ImportError:
                        logger.error(
                            f"❌ BinanceBalanceChecker غير متاح — لا يمكن التحقق من الرصيد")
                        ...
                        jsonify(
                            {
                                ...
                                "message": f"💰 رصيدك الحر في Binance ({
                                    available_balance:.2f} USDT) أقل من مبلغ التداول ({
                                    trade_amount:.2f} USDT).\n\nيرجى إيداع المزيد في حساب Binance الخاص بك أو تقليل مبلغ التداول.",
                ...
                """
                positions_result = db.execute_query(
                    positions_query, (user_id, current_is_demo)
                )
                open_positions = (
                    positions_result[0]["count"] if positions_result else 0
                )
                locked_amount = (
                    positions_result[0]["locked_amount"] or 0
                    if positions_result
                    else 0
                )
                ...
                if open_positions > 0:
                    logger.info(
                        f"⚠️ المستخدم {user_id} يفعّل التداول مع {open_positions} مراكز مفتوحة (مبلغ مقفل: {
                            locked_amount:.2f} USDT)")

                logger.info(
                    f"✅ المستخدم {user_id}: تفعيل التداول - رصيد حر: {available_balance:.2f} USDT"
                )

            # ✅ Validation لحد الخسارة اليومي — لا نكتبه إلا إذا أُرسِل صراحةً في الطلب
            _raw_mdl = (
                data.get("maxDailyLoss")
                or data.get("maxDailyLossPct")
                or data.get("max_daily_loss_pct")
            )
            ...
            if max_daily_loss is not None:
                ...
                if max_daily_loss < 5.0:
                    logger.warning(
                        f"⚠️ المستخدم {user_id}: حد الخسارة اليومي أقل من 5%، تم التعديل")
                    max_daily_loss = 5.0
                elif max_daily_loss > 15.0:
                    logger.warning(
                        f"⚠️ المستخدم {user_id}: حد الخسارة اليومي أكبر من 15%، تم التعديل")
                    max_daily_loss = 15.0
            ...
            if is_admin:
                ...
                target_context = get_trading_context(
                    db, user_id, requested_mode=requested_mode
                )
            ...
            stop_loss_value = (
                data.get("stopLossPercentage")
                or data.get("stopLossPct")
                or data.get("stop_loss_pct", 3.0)
            )
            take_profit_value = (
                data.get("takeProfitPercentage")
                or data.get("takeProfitPct")
                or data.get("take_profit_pct", 6.0)
            )
            effective_mode = current_mode if is_admin else "real"
            normalized_response = {
                "tradingEnabled": bool(data.get("tradingEnabled", False)),
                "tradeAmount": float(data.get("tradeAmount", 100.0)),
                "positionSizePercentage": float(
            ...
            if existing and len(existing) > 0:
                ...
                if "positionSizePercentage" in data:
                    field_map["position_size_percentage"] = float(
                        data["positionSizePercentage"]
                    )
                ...
                if "trailingDistance" in data:
                    field_map["trailing_distance"] = float(
                        data["trailingDistance"]
                    )
                if "maxConcurrentTrades" in data:
                    field_map["max_positions"] = int(
                        data["maxConcurrentTrades"]
                    )
                if max_daily_loss is not None:
                    field_map["max_daily_loss_pct"] = float(max_daily_loss)
                # دائماً نحدّث وضع التداول
                field_map["trading_mode"] = effective_mode
    ...
    def get_trading_mode(user_id):
        ...
        try:
            ...
            key_state = resolve_key_state(
                db, user_id, is_admin, current_is_demo
            )
            ...
            return jsonify(
                {
                    ...
                    "data": {
                        ...
                        "hasBinanceKeys": key_state["has_binance_keys"],
                        "hasConfiguredDbKeys": key_state[
                            "has_configured_db_keys"
                        ],
                        "usingEnvTestKeys": key_state["using_env_test_keys"],
                        "keysRequiredForCurrentMode": key_state[
                            "keys_required_for_current_mode"
    ...
    def update_trading_mode(user_id):
        ...
        try:
            ...
            if new_mode == "real":
                ...
                keys_result = db.execute_query(keys_query, (user_id,))
                has_keys = (
                    keys_result[0]["count"] > 0 if keys_result else False
                )
            ...
            # ضمان وجود صف إعدادات للمحفظة المستهدفة قبل التبديل لتفادي دورة
            # إعدادات غير مكتملة
            target_is_demo = new_mode == "demo"
            target_row_query = "SELECT id FROM user_settings WHERE user_id = %s AND is_demo = %s LIMIT 1"
            target_row = db.execute_query(
                target_row_query, (user_id, target_is_demo)
            )
            if not target_row:
                source_query = """
                    ...
                    for limit in [10, 20, 50, 100, 200]:
                        for status in ["all", "open", "closed"]:
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"Cache delete skipped: {cache_key}"
                                )

                logger.debug(
                    f"✅ تم إبطال cache لجميع البيانات للمستخدم {user_id}"
                )

            logger.info(
                f"✅ الأدمن {user_id}: تبديل trading_mode من {old_mode} إلى {new_mode}")
                ...
                if ENCRYPTION_AVAILABLE and api_key_encrypted:
                    try:
                        ...
                        masked_key = (
                            ...
                            + "..."
                            + api_key_decrypted[-8:]
                            if len(api_key_decrypted) > 16
                            else "***"
                        )
                    except Exception as decrypt_error:
                        logger.warning(
                            f"⚠️ فشل فك التشفير، ربما مفتاح قديم غير مشفر: {decrypt_error}")
                        # إذا فشل فك التشفير، نفترض أنه غير مشفر (للتوافق مع
                        # البيانات القديمة)
                        masked_key = (
                            api_key_encrypted[:8]
                            + "..."
                            + api_key_encrypted[-8:]
                            if len(api_key_encrypted) > 16
                            else "***"
                        )
                else:
                    masked_key = (
                        api_key_encrypted[:8] + "..." + api_key_encrypted[-8:]
                        if api_key_encrypted and len(api_key_encrypted) > 16
                        else "***"
                    )
                ...
                return jsonify(
                    {
                        ...
                        "data": {
                            ...
                            "isActive": bool(key.get("is_active", True)),
                            "isConfigured": True,
                        },
                    }
                )
        ...
        try:
            ...
            if not api_key or not api_secret:
                return error_response(
                    "بيانات المفاتيح مطلوبة", "MISSING_CREDENTIALS", 400
                )
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ Could not verify key permissions before save: {perm_error}")
                return (
                    jsonify(
                        {
                            "success": False,
            ...
            if not ENCRYPTION_AVAILABLE:
                logger.critical(
                    "❌ CRITICAL: Encryption service not available - cannot save keys"
                )
                return (
                    jsonify(
                        {
            ...
            try:
                encrypted_key, encrypted_secret = encrypt_binance_keys(
                    api_key, api_secret
                )
            ...
            db.execute_query(
                "UPDATE user_binance_keys SET is_active = FALSE WHERE user_id = %s",
                (user_id,),
            )
            db.execute_query(
    ...
    def delete_binance_key(key_id):
        ...
        try:
            ...
            db.execute_query(
                "UPDATE user_binance_keys SET is_active = FALSE WHERE id = %s",
                (key_id,),
            )
        ...
        try:
            ...
            if ENCRYPTION_AVAILABLE:
                try:
                    ...
                    api_key_decrypted = (
                        decrypt_text(api_key) if len(api_key) > 50 else api_key
                    )
                    api_secret_decrypted = (
                        decrypt_text(api_secret)
                        if len(api_secret) > 50
                        else api_secret
                    )
                except Exception as e:
                    logger.warning(
                        f"⚠️ فشل فك تشفير المفاتيح، استخدام القيم الأصلية: {e}"
                    )
            ...
            try:
                ...
                try:
                    ...
                    enable_withdrawals = api_restrictions.get(
                        "enableWithdrawals", False
                    )
                    enable_spot = api_restrictions.get(
                    ...
                    logger.warning(
                        f"⚠️ تعذر جلب صلاحيات المفتاح: {perm_error}"
                    )
                    security_warnings.append(
                        {
                            "level": "info",
                ...
                logger.info(
                    f"✅ مفاتيح Binance صحيحة للمستخدم {user_id} | "
                ...
                logger.warning(
                    f"⚠️ خطأ في الاتصال بـ Binance للمستخدم {user_id}: {e}"
                )
                return (
                    jsonify(
                        {
                ...
                logger.error(
                    f"❌ خطأ في اختبار المفاتيح للمستخدم {user_id}: {e}"
                )
                return (
                    jsonify(
                        {
    ...
    def reset_account_data(user_id):
        ...
        try:
            user_row = db_manager.execute_query(
                "SELECT user_type FROM users WHERE id = %s", (user_id,)
            )
            is_admin_user = (
                user_row[0]["user_type"] == "admin" if user_row else False
            )
            ...
            if reset_is_demo:
                initial_balance = getattr(
                    db_manager, "DEMO_ACCOUNT_INITIAL_BALANCE", 1000.0
                )
                if not db_manager.reset_user_portfolio(
                    user_id, initial_balance=initial_balance
                ):
                    response_data, status_code = error_response(
                        "فشل في إعادة ضبط الحساب التجريبي", "RESET_ERROR", 500
                    )
                    return jsonify(response_data), status_code
                ...
                logger.info(
                    f"✅ تم إعادة ضبط الحساب التجريبي للمستخدم {user_id} من المصدر الموحد")
                response_data, status_code = success_response(
                    {"reset": True, "mode": "demo"},
                    "تم إعادة ضبط البيانات بنجاح",
                )
                return jsonify(response_data), status_code
            ...
            with db_manager.get_write_connection() as conn:
                # حذف المراكز النشطة
                conn.execute(
                    "DELETE FROM active_positions WHERE user_id = %s AND is_demo = FALSE",
                    (user_id,),
                )
                ...
                """,
                    (user_id, reset_is_demo),
                ).fetchone()
                initial_balance = (
                    float(portfolio_row[0] or 1000.0)
                    if portfolio_row
                    else 1000.0
                )
                conn.execute(
            ...
            response_data, status_code = success_response(
                {"reset": True}, "تم إعادة ضبط البيانات بنجاح"
            )
            return jsonify(response_data), status_code
            ...
            response_data, status_code = error_response(
                "خطأ في إعادة ضبط البيانات", "RESET_ERROR", 500
            )
            return jsonify(response_data), status_code
    ...
    def get_daily_risk_status(user_id):
        ...
        try:
            ...
            requested_mode = request.args.get("mode")
            trading_context = get_trading_context(
                db_manager, user_id, requested_mode=requested_mode
            )
            is_demo = bool(trading_context["is_demo"])
            ...
            with db_manager.get_connection() as conn:
                ...
                """,
                    (portfolio_owner_id, is_demo, today),
                ).fetchone()
            ...
            daily_loss_abs = max(0.0, -daily_pnl)
            daily_loss_used_pct = (
                round((daily_loss_abs / base_balance) * 100, 2)
                if base_balance > 0
                else 0.0
            )
            daily_limit_usdt = round(
                (max_daily_loss_pct / 100.0) * base_balance, 2
            )
            remaining_usdt = round(
                max(0.0, daily_limit_usdt - daily_loss_abs), 2
            )
            ...
            response_data, status_code = error_response(
                "خطأ في جلب البيانات", "DAILY_STATUS_ERROR", 500
            )
            return jsonify(response_data), status_code
