...
def register_admin_ml_routes(bp, shared):
    ...
    def get_system_ml_status():
        ...
        try:
            ...
            try:
                ...
                total_samples = classifier.training_history.get('total_samples', 0)
                required_samples = classifier.MIN_SAMPLES_FOR_READINESS
                is_ready = classifier.is_ready()
                best_accuracy = classifier.training_history.get('best_accuracy', 0)
                
                result['ml'] = {
                    'enabled': classifier.enabled,
                    'is_ready': is_ready,
                    'phase': hybrid_status.get('current_phase', 'initial'),
                    'phase_description': hybrid_status.get('phase_description', 'جمع البيانات'),
                    'backtest_weight': hybrid_status.get('backtest_weight', 0.7),
                    'real_weight': hybrid_status.get('real_weight', 0.3),
                    'total_samples': total_samples,
                    'required_samples': required_samples,
                    'accuracy': best_accuracy,
                    'progress_pct': min(100, round((total_samples / required_samples) * 100, 1)),
                    'status_text': f"دقة {best_accuracy*100:.1f}% ({total_samples} صفقة)" if is_ready else f"جمع البيانات ({total_samples}/{required_samples})"
                }
            except Exception as ml_err:
                logger.error(f"خطأ في تحميل نظام ML: {ml_err}")
                result['ml'] = {
                    'enabled': False, 'is_ready': False, 'phase': 'initial',
                    'phase_description': 'النظام غير متاح', 'backtest_weight': 0,
                    'real_weight': 1, 'total_samples': 0, 'required_samples': 200,
                    'accuracy': 0, 'progress_pct': 0, 'status_text': 'النظام غير نشط'
                }
            ...
            for row in cursor.fetchall():
                active_positions_data.append({
                    'symbol': row[0], 'entry_price': row[1], 'current_price': row[2],
                    'quantity': row[3], 'profit_loss': row[4],
                    'profit_loss_percentage': row[5], 'created_at': row[6]
                })
            ...
            for row in cursor.fetchall():
                recent_activity_data.append({
                    'type': 'win' if row[1] > 0 else 'loss' if row[1] < 0 else 'neutral',
                    'symbol': row[0], 'profit': row[1], 'time': row[2]
                })
    ...
    def get_ml_backtest_status():
        ...
        try:
            ...
            reliability = round((winning_trades / total_trades * 100) if total_trades > 0 else 0, 1)
            ...
            return jsonify({
                ...
                'data': {
                    ...
                    'progress_pct': min(100, round(total_trades / 100 * 100, 1)),
                    'status_text': f"موثوقية {reliability}% ({total_symbols} عملة)" if is_ready else f"جمع البيانات ({total_trades} صفقة)",
                    'best_combos': [],
                    'worst_combos': []
                }
            })
    ...
    def get_ml_reliability():
        ...
        try:
            ...
            unreliable = len([c for c in combos if not c['is_reliable'] and c['has_enough_data']])
            
            return jsonify({
                'success': True,
                'data': {
                    'status': {'enabled': True, 'is_ready': len(combos) > 0},
                    'combos': combos,
                    'summary': {
                        'total': len(combos),
                        'reliable': reliable,
    ...
    def test_notification():
        ...
        try:
            ...
            return jsonify({'success': True, 'message': 'تم إرسال الإشعار الاختباري'})
        except Exception as e:
    ...
    def get_ml_status():
        ...
        try:
            ...
            return jsonify({
                'success': False,
                'ml': {
                    'enabled': False, 'is_ready': False, 'total_samples': 0,
                    'required_samples': 200, 'progress_pct': 0,
    ...
    def get_ml_progress():
        ...
        try:
            ...
            if not classifier or not classifier.enabled:
                return jsonify({'success': False, 'error': 'ML System not available'}), 503
            
            status = classifier.get_status()
            total_samples = len(classifier.accumulated_data) if hasattr(classifier, 'accumulated_data') else 0
            ...
            progress = {
                ...
                'data': {
                    'training_progress': {
                        ...
                        'progress_percent': min((total_samples / classifier.MIN_SAMPLES_FOR_READINESS) * 100, 100),
                        'status': 'ready' if total_samples >= classifier.MIN_SAMPLES_FOR_READINESS else 'training'
                    },
                    'model_metrics': {
    ...
    def get_ml_quality_metrics():
        ...
        try:
            ...
            row = cursor.fetchone()
            total, avg_profit, max_profit, max_loss, wins = row if row else (0, 0, 0, 0, 0)
            ...
            return jsonify({
                ...
                'data': {
                    ...
                    'data_quality': {
                        'total_samples': total, 'valid_samples': total, 'completeness': 100.0
                    },
                    'model_quality': {
                        'accuracy': win_rate, 'precision': win_rate, 'win_rate': win_rate
                    },
                    'performance_metrics': {
                        'avg_profit': round(float(avg_profit) if avg_profit else 0, 2),
                        'max_profit': round(float(max_profit) if max_profit else 0, 2),
                        'max_loss': round(float(max_loss) if max_loss else 0, 2),
                        'total_trades': total,
                        'winning_trades': wins
                    },
                    'status': 'ready' if total >= 10 else 'collecting_data',
                    'recommendations': [
                        'النظام يعمل بشكل جيد' if win_rate >= 50 else 'يحتاج تحسين الاستراتيجية'
                    ]
                }
            })
        except Exception as e:
