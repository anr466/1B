...
try:
    from backend.api.cache_invalidator import (
        invalidate_cache,
        set_admin_cache,
        clear_all_cache,
    )
...
try:
    ...
    _fallback_logger.error(
        "❌ CRITICAL: admin_auth module not available - all admin endpoints will be blocked"
    )
...
def get_admin_dashboard():
    ...
    def fetch_dashboard():
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM active_positions WHERE is_active = FALSE AND is_demo = FALSE"
        )
        ...
        cursor.execute(
            "SELECT SUM(profit_loss) FROM active_positions WHERE is_active = FALSE AND is_demo = FALSE AND profit_loss > 0"
        )
        ...
        # حالة النظام
        cursor.execute(
            "SELECT trading_status, database_status FROM system_status ORDER BY id DESC LIMIT 1"
        )
...
def get_system_overview():
    ...
    try:
        # Always fetch fresh state - no caching for critical state
        result = fetch_status()
        return jsonify(result)
    except Exception as e:
...
def get_system_stats():
    ...
    try:
        # Always fetch fresh stats - no caching
        result = fetch_stats()
        return jsonify(result)
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
...
def get_errors():
    ...
    try:
        ...
        if resolved is not None and str(resolved).lower() in [
            "0",
            "1",
            "true",
            "false",
        ...
        for row in rows:
            try:
                ...
                error_dict = {
                    ...
                    or ("resolved" if bool(row_dict.get("resolved", 0)) else "new"),
                    "created_at": row_dict.get("created_at"),
                    "resolved_at": row_dict.get("resolved_at"),
                    "source": row_dict.get("source")
                    or row_dict.get("error_type")
                    or "unknown",
        ...
        return jsonify({"success": True, "data": {"errors": errors, "total": total}})
    except Exception as e:
...
def get_error_stats():
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM system_errors WHERE resolved = FALSE AND COALESCE(requires_admin, FALSE) = TRUE"
        )
...
def get_performance_metrics():
    ...
    try:
        ...
        cursor.execute(
            "SELECT COALESCE(SUM(profit_loss), 0) FROM active_positions WHERE is_active = FALSE"
        )
...
def get_group_b_performance():
    ...
    try:
        ...
        # 3️⃣ البيانات
        cursor.execute(
            "SELECT COUNT(*), AVG(profit_loss), SUM(profit_loss) FROM active_positions WHERE is_active = FALSE AND profit_loss IS NOT NULL"
        )
        ...
        cursor.execute(
            "SELECT COUNT(DISTINCT symbol) FROM successful_coins WHERE is_active = TRUE"
        )
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            effective_trading_mode = (
                (row[12] or "demo") if row[5] == "admin" else "real"
            )
...
@require_admin
@invalidate_cache(
    "admin_dashboard", "system_stats", "users_list"
)  # ✅ FIX (for PATCH/DELETE)
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "learning_validation_log": deleted_counts.get(
                        "learning_validation_log", 0
                    ),
...
def close_position(position_id):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ position #{pid} closed but balance sync failed: {bal_err}"
            )
...
def get_analytics_revenue():
    ...
    try:
        ...
        cursor.execute(
            "SELECT SUM(profit_loss) FROM active_positions WHERE is_active = FALSE AND profit_loss > 0"
        )
...
def get_backtest_results():
    ...
    try:
        ...
        if not os.path.isdir(results_dir):
            return jsonify(
                {"success": True, "data": {"available": False, "results": []}}
            )
    ...
    try:
        if getattr(db, "is_postgres", lambda: False)():
            return jsonify(
                {
                    "success": True,
                    "message": "PostgreSQL optimization is managed automatically",
                }
            )
...
def get_security_audit_log_direct():
    ...
    try:
        ...
        total = int(
            (total_row["count"] if total_row and "count" in total_row else total_row[0])
            if total_row
            else 0
        )
