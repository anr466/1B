...
def register_user_fcm_token(user_id: str, fcm_token: str, platform: str = 'android') -> bool:
...
def unregister_fcm_token():
    ...
    try:
        ...
        if success:
            return jsonify({
                'success': True,
                'message': 'تم إلغاء تسجيل FCM Token بنجاح'
            })
        else:
            return jsonify({
                'success': False,
                'error': 'فشل في إلغاء تسجيل FCM Token'
            }), 500
