...
def register_mobile_trades_routes(bp, shared):
    ...
    def _resolve_trade_context(db, user_id, requested_mode=None):
        ...
        return (
            trading_context["is_demo"],
            trading_context["portfolio_owner_id"],
        )
            ...
            with db.get_connection() as conn:
                ...
                if not row:
                    response_data, status_code = error_response(
                        "الصفقة غير موجودة", "NOT_FOUND", 404
                    )
        ...
        try:
            ...
            if date_from:
                try:
                    ...
                    response_data, status_code = error_response(
                        "صيغة date_from غير صحيحة. استخدم ISO format (YYYY-MM-DD)",
                        "INVALID_DATE",
                        400,
                    )
                    return jsonify(response_data), status_code
            ...
            if date_to:
                try:
                    ...
                    response_data, status_code = error_response(
                        "صيغة date_to غير صحيحة. استخدم ISO format (YYYY-MM-DD)",
                        "INVALID_DATE",
                        400,
                    )
                    return jsonify(response_data), status_code
            ...
            if date_from and date_to:
                try:
                    if dt.fromisoformat(date_to) < dt.fromisoformat(date_from):
                        response_data, status_code = error_response(
                            "date_to يجب أن يكون بعد date_from",
                            "INVALID_DATE_RANGE",
                            400,
                        )
                        return jsonify(response_data), status_code
            ...
            is_demo, portfolio_owner_id = _resolve_trade_context(
                db, user_id, requested_mode
            )
            ...
            if CACHE_AVAILABLE:
                ...
                if cached_data:
                    ...
                    response_data, status_code = success_response(
                        cached_data, "تم جلب الصفقات من الذاكرة المؤقتة"
                    )
                    return jsonify(response_data), status_code
            ...
            if not trades_data:
                response_data, status_code = error_response(
                    "فشل في جلب الصفقات", "TRADES_ERROR", 500
                )
                return jsonify(response_data), status_code
            ...
            if trades_list:
                for trade in trades_list:
                    ...
                    quantity = (
                        float(trade.get("quantity", 0)) if trade.get("quantity") else 0
                    )
                    position_size = (
                    ...
                    formatted_trades.append(
                        {
                            ...
                            "profitLossPercentage": (
                                float(trade.get("profit_loss_percentage", 0))
                                if trade.get("profit_loss_percentage")
                                else 0
                            ),
            ...
            logger.info(
                f"Pagination: صفحة {page}/{total_pages} - {len(formatted_trades)} صفقة"
            )

            response_data, status_code = success_response(
                result, "تم جلب الصفقات بنجاح"
            )
            return jsonify(response_data), status_code
            ...
            response_data, status_code = error_response(
                "خطأ في جلب الصفقات", "TRADES_ERROR", 500
            )
            return jsonify(response_data), status_code
    ...
    def get_active_positions(user_id):
        ...
        if not verify_user_access(user_id):
            response_data, status_code = error_response(
                "غير مصرح بالوصول", "UNAUTHORIZED_ACCESS", 403
            )
            return jsonify(response_data), status_code
        ...
        try:
            ...
            positions = db.get_user_active_positions(
                portfolio_owner_id, is_demo=is_demo_bool
            )
                ...
                formatted_positions.append(
                    {
                        ...
                        "trailingSlPrice": (
                            float(pos.get("trailing_sl_price", 0))
            ...
            response_data, status_code = success_response(
                result, "تم جلب الصفقات النشطة بنجاح"
            )
            return jsonify(response_data), status_code
            ...
            response_data, status_code = error_response(
                "خطأ في جلب الصفقات النشطة", "POSITIONS_ERROR", 500
            )
            return jsonify(response_data), status_code
    ...
    def get_daily_pnl(user_id):
        ...
        try:
            if g.current_user_id != user_id:
                logger.warning(
                    f"⚠️ محاولة وصول غير مصرح: المستخدم {
                        g.current_user_id
                    } يحاول الوصول لبيانات المستخدم {user_id}"
                )
                response_data, status_code = error_response(
                    "غير مصرح بالوصول", "UNAUTHORIZED_ACCESS", 403
                )
            ...
            requested_mode = request.args.get("trading_mode", None) or request.args.get(
                "mode", None
            )
            ...
            logger.info(
                f"📊 جلب البيانات اليومية للمستخدم {user_id} (is_demo={is_demo}, days={days})"
            )
            ...
            results = db_manager.execute_query(
                query, (portfolio_owner_id, is_demo, days)
            )
            ...
            logger.info(
                f"✅ تم جلب {len(daily_data)} يوم من البيانات للمستخدم {user_id}"
            )

            resp, code = success_response(
                response_data, "تم جلب البيانات اليومية بنجاح"
            )
            ...
            response_data, status_code = error_response(
                "خطأ في جلب البيانات اليومية", "DAILY_PNL_ERROR", 500
            )
            return jsonify(response_data), status_code
    ...
    def get_portfolio_growth(user_id):
        ...
        try:
            ...
            with db_manager.get_connection() as conn:
                try:
                    rows = conn.execute(
                        ...
                        ORDER BY date ASC
                    """,
                        (portfolio_owner_id, is_demo, days),
                    ).fetchall()
                except Exception:
                    rows = []
            ...
            response_data, status_code = success_response(
                result, "تم جلب بيانات نمو المحفظة"
            )
            return jsonify(response_data), status_code
            ...
            response_data, status_code = error_response(
                "خطأ في جلب البيانات", "GROWTH_ERROR", 500
            )
            return jsonify(response_data), status_code
    ...
    def get_admin_demo_portfolio_growth(admin_id):
        ...
        try:
            if getattr(g, "current_user_type", None) != "admin":
                response_data, status_code = error_response(
                    "غير مصرح بالوصول", "UNAUTHORIZED", 403
                )
                return jsonify(response_data), status_code
            ...
            result = {"growth": growth_data, "period": days, "mode": "demo"}

            response_data, status_code = success_response(
                result, "تم جلب بيانات نمو المحفظة التجريبية"
            )
            return jsonify(response_data), status_code
    ...
    def toggle_trade_favorite():
        ...
        try:
            ...
            if not trade_id:
                return (
                    jsonify({"success": False, "error": "معرف الصفقة مطلوب"}),
                    400,
                )
            ...
            with db_manager.get_write_connection() as conn:
                ...
                if not trade:
                    return (
                        jsonify({"success": False, "error": "الصفقة غير موجودة"}),
                        404,
                    )

                if trade[1] != g.current_user_id:
                    logger.warning(
                        f"⚠️ Unauthorized favorite toggle: user {
                            g.current_user_id
                        } tried trade {trade_id} owned by {trade[1]}"
                    )
                    return (
                        jsonify(
                            {
                                "success": False,
                                "error": "غير مصرح بتعديل صفقة مستخدم آخر",
                            }
                        ),
                        403,
                    )
    ...
    def get_favorite_trades(user_id):
        ...
        try:
            ...
            db = db_manager
            is_demo, portfolio_owner_id = _resolve_trade_context(
                db, user_id, request.args.get("mode", None)
            )
        ...
        try:
            ...
            with db_manager.get_connection() as conn:
                ...
                distribution = {
                    ...
                    "winning_percent": round(
                        (stats[1] or 0) / max(stats[0] or 1, 1) * 100
                    ),
                    "losing_percent": round(
                        (stats[2] or 0) / max(stats[0] or 1, 1) * 100
                    ),
                    "breakeven_percent": round(
                        (stats[3] or 0) / max(stats[0] or 1, 1) * 100
                    ),
                    ...
                    "by_symbol": [
                        dict(zip(["symbol", "count", "total"], row))
                        for row in by_symbol
                    ],
                    "by_strategy": [
                        dict(zip(["strategy", "count", "avg"], row))
        ...
        try:
            ...
            with db.get_connection() as conn:
                ...
                if portfolio_balance_row is not None:
                    dashboard_data["portfolio"]["balance"] = (
                        portfolio_balance_row[0] or 0
                    )
                elif not is_demo:
                ...
                dashboard_data["stats"] = {
                    ...
                    "win_rate": (
                        round(
                            (((trades_stats[1] or 0) / (trades_stats[0] or 0)) * 100),
                            2,
                        )
                        if (trades_stats[0] or 0) > 0
                        else 0
                    ),
                }
                ...
                if settings_row:
                    dashboard_data["settings"] = {
                        "trading_mode": (
                            settings_row[0]
                            if settings_row[0] in ("demo", "real")
                            else "demo"
                        ),
