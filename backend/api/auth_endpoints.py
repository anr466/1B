...
from backend.utils.user_lookup_service import (
    get_user_by_email,
...
# Import: from backend.utils.user_lookup_service import get_user_by_email,
# get_user_by_username
...
def send_otp():
    ...
    try:
        ...
        if security_audit and security_audit.is_rate_limited(
            email, "OTP_SENT", max_attempts=5, minutes=15
        ):
            return (
                jsonify(
                    {
                    ...
                    jsonify(
                        {
                            "success": False,
                            "error": (
                                result.get("error", "رمز OTP غير صحيح")
                                if isinstance(result, dict)
                                else "رمز OTP غير صحيح"
                            ),
                        }
                    ),
                    400,
...
def resend_otp():
    ...
    try:
        ...
        if not user:
            return (
                jsonify({"success": False, "error": "الإيميل غير مسجل"}),
                400,
            )
...
def send_verification_email():
    ...
    try:
        ...
        if otp_service:
            ...
            return (
                jsonify({"success": False, "error": "خدمة البريد غير متاحة"}),
                503,
            )
...
def login_user():
    ...
    try:
        ...
        # سجل تفصيلي للتشخيص
        logger.info(
            f"🔐 Login attempt - email: '{email}', username: '{username}', password_len: {len(password)}"
        )
        ...
        if email:
            ...
            user = get_user_by_email(email)
            # إذا لم يُعثر عليه، جرب البحث باليوزر نيم (في حال أدخل المستخدم
            # اليوزر في حقل الإيميل)
            if not user:
                user = get_user_by_username(email)
            # توافق إضافي: لو أُدخل بريد لحساب مسجل باليوزرنيم فقط، جرّب الجزء
            # قبل @
...
def verify_phone_token():
    ...
    try:
        ...
        if not sms_service:
            return (
                jsonify({"success": False, "error": "خدمة SMS غير متاحة"}),
                503,
            )
...
def validate_session():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "error": "خطأ في التحقق من الجلسة"}),
            500,
        )
...
def refresh_token():
    ...
    try:
        ...
        if not refresh_token_str:
            return (
                jsonify({"success": False, "error": "refresh_token مطلوب"}),
                400,
            )
        ...
        if TOKEN_SYSTEM_AVAILABLE:
            ...
            return (
                jsonify({"success": False, "error": "نظام التوكن غير متاح"}),
                500,
            )
...
def logout_user():
    ...
    try:
        ...
        return jsonify({"success": False, "error": "خطأ في تسجيل الخروج"}), 500
...
def delete_account():
    ...
    try:
        ...
        with db_manager.get_write_connection() as conn:
            ...
            cursor.execute(
                "SELECT password_hash, username, email FROM users WHERE id = %s",
                (user_id,),
            )
            ...
            # 8. حذف المستخدم (CASCADE سيحذف جميع البيانات المرتبطة -
            # PostgreSQL يدعم FK افتراضياً)
        ...
        return (
            jsonify({"success": False, "error": "حدث خطأ أثناء حذف الحساب"}),
            500,
        )
