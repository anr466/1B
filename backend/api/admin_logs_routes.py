...
def register_admin_logs_routes(bp, shared):
    ...
    def get_system_errors():
        ...
        try:
            ...
            # group_b, background, system, etc.
            source = request.args.get("source")
            # new, resolved, auto_resolved, escalated
    ...
    def get_system_error_details(error_id):
        ...
        try:
            ...
            if not row:
                conn.close()
                return (
                    jsonify({"success": False, "error": "Error not found"}),
                    404,
                )
    ...
    def generate_report():
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "report_generation_not_implemented",
                }
            ),
            501,
        )
    ...
    def export_report(report_id):
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "report_export_not_implemented",
                    "report_id": report_id,
                }
            ),
            501,
            ...
            return jsonify(
                {
                    ...
                    "data": {
                        "retention_policy": {
                            "operation_logs": {
                                "retention_days": 7,
                                "description": "سجلات العمليات (تشغيل/إيقاف المجموعات)",
                            },
                            "activity_logs": {
                                "retention_days": 7,
                                "description": "سجلات النشاط (تسجيل الدخول، التحديثات)",
                            },
                            "system_errors": {
                                "retention_days": 30,
                                "description": "أخطاء النظام والتطبيق",
                            },
                        },
                        "auto_cleanup": {
                            "enabled": True,
