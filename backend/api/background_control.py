...
try:
    ...
    try:
        ...
        logger.error(
            "❌ CRITICAL: admin_auth not available - background endpoints will be BLOCKED"
        )
...
def get_background_status():
    ...
    try:
        ...
        try:
            ...
            db_status = {
                "trading_state": tsm_state.get("trading_state", "UNKNOWN"),
                "is_running": tsm_state.get("trading_active", False),
            }
        except Exception as db_err:
            logger.warning(f"⚠️ فشل قراءة الحالة من State Machine: {db_err}")

        # ✅ الخطوة 3: لا مزامنة مباشرة - State Machine يتعامل مع المزامنة
        # هذه الدالة للقراءة فقط، المزامنة تتم عبر State Machine
...
def get_background_logs():
    ...
    try:
        ...
        if not os.path.exists(log_file):
            return jsonify(
                {"success": True, "logs": [], "message": "لا توجد سجلات بعد"}
            )
...
def get_errors():
    ...
    try:
        ...
        if not error_logger:
            return jsonify({"success": True, "count": 0, "errors": []})
...
def get_critical_errors():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "count": len(errors), "critical_errors": errors}
        )
...
def get_operations_log():
    ...
    try:
        ...
        operations = []
        if unified_logger:
            operations = unified_logger.get_recent_operations(
