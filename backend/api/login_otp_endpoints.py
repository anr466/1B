...
def send_login_otp():
    ...
    try:
        ...
        success, otp_code = otp_service.send_email_otp(
            email or phone_number, purpose="login"
        )
        ...
        if method == "sms" and phone_number:
            ...
            if not sms_sent:
                logger.info(f"📱 [DEV] OTP للمستخدم {
                    user['username']}: {otp_code}")

            masked_target = (
                phone_number[:4] + "****" + phone_number[-2:]
...
def verify_login_otp():
    ...
    try:
        ...
        verified, result = otp_service.verify_email_otp(
            user["email"], otp_code, purpose="login"
        )
...
def resend_login_otp():
    ...
    try:
        ...
        can_send, wait_seconds = otp_service.can_send_otp(
            user["email"], purpose="login", cooldown_minutes=1
        )
