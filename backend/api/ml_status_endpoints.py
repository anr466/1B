...
try:
    ...
    logger.error(
        "❌ CRITICAL: admin_auth not available - ML endpoints will be blocked"
    )
...
# إنشاء Blueprint
ml_status_bp = Blueprint("ml_status", __name__, url_prefix="/admin/ml")
...
def get_ml_status():
    ...
    try:
        ...
        try:
            ...
            payload = (
                coins_response[0].get_json()
                if isinstance(coins_response, tuple)
                else coins_response.get_json()
            )
            coins = (
                payload.get("coins", []) if isinstance(payload, dict) else []
            )
        except Exception:
            coins = []
            ...
            jsonify(
                {
                    ...
                    "summary": {
                        "total": len(coins),
                        "strong": len(
                            [c for c in coins if c["status"] == "strong"]
                        ),
                        "normal": len(
                            [c for c in coins if c["status"] == "normal"]
                        ),
                        "weak": len(
                            [c for c in coins if c["status"] == "weak"]
                        ),
                        "blocked": len(
                            [c for c in coins if c["status"] == "blocked"]
                        ),
                        "waiting": len(
                            [c for c in coins if c["status"] == "waiting"]
                        ),
                    },
                }
            ),
...
def get_ml_health():
    """فحص صحة نظام ML"""
    try:
        from backend.ml.signal_classifier import (
            get_ml_classifier,
            ML_AVAILABLE,
        )
            ...
            jsonify(
                {
                    ...
                    "classifier_enabled": (
                        classifier.enabled if classifier else False
                    ),
                    "brain_available": brain is not None,
                    "is_ready": (
                        classifier.get_status().get("is_ready", False)
                        if classifier
                        else False
                    ),
                }
        ...
        return (
            jsonify({"success": True, "ml_available": False, "error": str(e)}),
            200,
        )
...
def get_patterns_performance():
    ...
    try:
        ...
        patterns_sorted = sorted(
            patterns, key=lambda x: x["confidence"], reverse=True
        )
...
def get_pattern_details(pattern_id):
    ...
    try:
        ...
        try:
            ...
            return jsonify({"success": True, "pattern": summary}), 200

        except KeyError:
            return (
                jsonify({"success": False, "error": "Pattern not found"}),
                404,
            )
...
def reset_pattern(pattern_id):
    ...
    try:
        ...
        confidence_system.current_confidence = (
            confidence_system.initial_confidence
        )
...
def get_learning_status():
    ...
    try:
        ...
        with_indicators = (
            trade_stats["trades_with_indicators"] if trade_stats else 0
        )
...
def get_monitored_coins():
    ...
    try:
        ...
        for i, symbol in enumerate(current_symbols):
            ...
            # حساب score
            score = (
                round(wr * 0.6 + max(min(avg_pnl / 5, 0.4), -0.4), 2)
                if total >= 3
                else 0
            )
