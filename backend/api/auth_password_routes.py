...
def register_password_routes(bp, shared):
    ...
    def forgot_password():
        ...
        try:
            ...
            logger.info(f'🔍 User found: {user is not None} - User ID: {user.get("id") if user else "N/A"}')
            ...
            if security_audit and security_audit.is_rate_limited(email, 'PASSWORD_RESET_REQUEST', max_attempts=3, minutes=15):
            ...
            logger.info(f'📤 Attempting to send OTP via {method} - otp_service: {otp_service is not None}')
            if otp_service:
    ...
    def cancel_otp():
        ...
        try:
            ...
            if otp_service:
                ...
                if cancelled:
                    return jsonify({'success': True, 'message': 'تم إلغاء رمز التحقق بنجاح'})
                else:
                    return jsonify({'success': True, 'message': 'لا يوجد رمز تحقق نشط لإلغائه'})
            else:
                return jsonify({'success': False, 'error': 'خدمة OTP غير متاحة'}), 500
    ...
    def send_change_email_otp():
        ...
        try:
            ...
            if otp_service:
                success, otp_code = otp_service.send_email_otp(new_email, purpose='change_email')
                if success:
                    logger.info(f"✅ تم إرسال OTP لتغيير الإيميل إلى: {new_email}")
                    return jsonify({'success': True, 'message': 'تم إرسال رمز التحقق إلى الإيميل الجديد', 'expires_in': 600})
                else:
            ...
            try:
                with db_manager.get_connection() as conn:
                    ...
                    if not _verify_pw(old_password, user['password_hash']):
                        return jsonify({'success': False, 'error': 'كلمة المرور القديمة غير صحيحة'}), 401
            except Exception as e:
                logger.error(f"خطأ في التحقق من كلمة المرور: {e}")
                return jsonify({'success': False, 'error': 'خطأ في التحقق'}), 500
            
            if otp_service:
                success, otp_code = otp_service.send_email_otp(email, purpose='change_password')
                if success:
                    logger.info(f"✅ تم إرسال OTP لتغيير كلمة المرور إلى: {email}")
                    return jsonify({'success': True, 'message': 'تم إرسال رمز التحقق إلى إيميلك', 'expires_in': 600})
                else:
                    return jsonify({'success': False, 'error': 'فشل في إرسال رمز التحقق'}), 500
            else:
                return jsonify({'success': False, 'error': 'خدمة OTP غير متاحة'}), 500
            ...
            if otp_service:
                ...
                if verified:
                    try:
                        ...
                        return jsonify({'success': False, 'error': 'فشل في تحديث كلمة المرور'}), 500
                else:
                    return jsonify({
                        'success': False,
                        'error': result.get('error', 'رمز التحقق غير صحيح'),
        ...
        try:
            ...
            try:
                ...
                if payload.get('purpose') != 'password_reset':
                    return jsonify({'success': False, 'error': 'Token غير صالح لهذه العملية'}), 400
                
                user_id = payload.get('user_id')
                email = payload.get('email')
                
                if not user_id or not email:
                    return jsonify({'success': False, 'error': 'Token غير صحيح'}), 400
                ...
                if isinstance(jwt_import_err, jwt_mod.ExpiredSignatureError):
                    return jsonify({'success': False, 'error': 'انتهت صلاحية الرمز. يرجى طلب رمز جديد'}), 400
                elif isinstance(jwt_import_err, jwt_mod.InvalidTokenError):
                    return jsonify({'success': False, 'error': 'رمز غير صحيح'}), 400
                else:
