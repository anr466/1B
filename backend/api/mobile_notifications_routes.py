...
def register_mobile_notifications_routes(bp, shared):
    ...
    def get_notifications(user_id):
        ...
        try:
            ...
            # ✅ دعم mode للفلترة (demo/real) - للأدمن فقط
            requested_mode = request.args.get("mode", None)
            is_admin_user = getattr(g, "current_user_type", None) == "admin"

            if requested_mode in ("demo", "real") and is_admin_user:
                # للفلترة حسب mode، نضم مع active_positions للحصول على is_demo
                is_demo_filter = 1 if requested_mode == "demo" else 0
                notif_query = """
                    SELECT n.id, n.user_id, n.title, n.message, COALESCE(n.type, 'general') as type, 
                           n.is_read, n.created_at, n.data
                    FROM notifications n
                    LEFT JOIN active_positions ap ON n.data::jsonb->>'trade_id' = ap.id::text
                    WHERE n.user_id = %s 
                      AND (ap.is_demo = %s OR ap.id IS NULL)
                    ORDER BY n.created_at DESC 
                    LIMIT %s OFFSET %s
                """
                result = db.execute_query(
                    notif_query, (user_id, is_demo_filter, limit, offset)
                )
                ...
                count_query = """
                    ...
                    WHERE n.user_id = %s 
                      AND (ap.is_demo = %s OR ap.id IS NULL)
                """
                count_result = db.execute_query(count_query, (user_id, is_demo_filter))
            else:
    ...
    def mark_all_notifications_read(user_id):
        ...
        try:
            ...
            # تحديث جميع الإشعارات غير المقروءة إلى مقروءة في جدول
            # notifications
            update_query = """
            ...
            return jsonify(
                {"success": True, "message": "تم تحديد جميع الإشعارات كمقروءة"}
            )
                ...
                if cursor.rowcount == 0:
                    return (
                        jsonify(
                            {
                                "success": False,
                                "error": "الإشعار غير موجود أو لا يخصك",
                            }
                        ),
                        404,
                    )
    ...
    def mark_all_notifications_read_put(user_id):
        ...
        try:
            ...
            return jsonify(
                {
                    "success": True,
                    "message": f"تم تحديد {affected} إشعار كمقروء",
                }
            )
        except Exception as e:
    ...
    def update_user_notification_settings():
        ...
        try:
            ...
            # يجب أن يحتوي الطلب على حقل معروف واحد على الأقل من إعدادات
            # الإشعارات
            supported_fields = {
                "pushEnabled",
                "tradeNotifications",
            ...
            if not any(k in supported_fields for k in settings.keys()):
                return error_response(
                    "يجب تقديم حقل واحد على الأقل من إعدادات الإشعارات", 400
                )

            # التحقق من صحة الحدود المالية
            if "profitThreshold" in settings and (
                settings["profitThreshold"] < 0 or settings["profitThreshold"] > 10000
            ):
                return error_response("حد الربح يجب أن يكون بين 0 و 10000 دولار", 400)

            if "lossThreshold" in settings and (
                settings["lossThreshold"] < 0 or settings["lossThreshold"] > 10000
            ):
                return error_response("حد الخسارة يجب أن يكون بين 0 و 10000 دولار", 400)

            if "cumulativeLossThresholdUsd" in settings and (
                settings["cumulativeLossThresholdUsd"] < 0
                or settings["cumulativeLossThresholdUsd"] > 100000
            ):
                return error_response(
                    "حد الخسائر التراكمية يجب أن يكون بين 0 و 100000 دولار",
                    400,
                )
            ...
            if (
                settings.get("endOfDayReportEnabled")
                and "endOfDayReportTime" in settings
            ):
                report_time = str(settings["endOfDayReportTime"])
                if len(report_time) != 5 or report_time[2] != ":":
                    return error_response(
                        "صيغة وقت تقرير نهاية اليوم يجب أن تكون HH:MM", 400
                    )
    ...
    def get_notification_stats_endpoint(user_id):
        ...
        try:
            ...
            response_data, status_code = error_response(
                "خطأ في جلب الإحصائيات", "NOTIFICATION_STATS_ERROR", 500
            )
            return jsonify(response_data), status_code
    ...
    def cleanup_notifications():
        ...
        try:
            ...
            return success_response(
                {"message": "تم تنظيف الإشعارات بنجاح", "results": results}
            )
    ...
    def get_onboarding_status(user_id):
        ...
        try:
            from backend.services.user_onboarding_service import (
    ...
    def get_next_onboarding_step(user_id):
        ...
        try:
            ...
            return jsonify(
                {"success": True, "data": next_step}  # None إذا لا توجد خطوات
            )
        except Exception as e:
    ...
    def dismiss_onboarding_step(user_id):
        ...
        try:
            ...
            if not success:
                return (
                    jsonify({"success": False, "error": "dismiss_step_failed"}),
                    500,
                )
                    ...
                    notifications_list.append(
                        {
                            ...
                            "data": (
                                json.loads(notification[4]) if notification[4] else None
                            ),
