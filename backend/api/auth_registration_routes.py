...
def register_registration_routes(bp, shared):
    ...
    def check_availability():
        ...
        try:
            ...
            if not email and not username and not phone:
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": "يجب توفير البريد أو اسم المستخدم أو رقم الجوال",
                        }
                    ),
                    400,
                )
            ...
            try:
                ...
                return (
                    jsonify({"success": False, "error": "خطأ في فحص التوفر"}),
                    500,
                )
        except Exception as e:
            logger.error(f"❌ خطأ في معالجة الطلب: {e}")
            return (
                jsonify({"success": False, "error": "خطأ في معالجة الطلب"}),
                500,
            )
                    ...
                    if method == "sms" and phone:
                        ...
                        if sms_service:
                            try:
                                ...
                                sms_sent = True
                                logger.info(
                                    f"📱 تم إرسال OTP تسجيل عبر SMS إلى {phone}"
                                )
    ...
    def register_user():
        ...
        try:
            ...
            logger.info(
                f"📥 طلب تسجيل مستخدم جديد - البيانات المستلمة: {list(data.keys())}"
            )
            ...
            if not email or not password or not username:
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": "الإيميل وكلمة المرور واسم المستخدم مطلوبين",
                        }
                    ),
                    400,
                )
            ...
            if not full_name:
                ...
                return (
                    jsonify({"success": False, "error": "الاسم الكامل مطلوب"}),
                    400,
                )
            ...
            if not validate_username(username):
                logger.error(
                    f"❌ اسم مستخدم غير صحيح: {username} - الطول: {len(username)}"
                )
                return (
                    jsonify(
                        {
    ...
    def verify_registration_otp():
        ...
        try:
            ...
            if not email or not otp_code or not username or not password:
                return (
                    jsonify({"success": False, "error": "بيانات ناقصة"}),
                    400,
                )
        ...
        try:
            ...
            try:
                ...
                with db_manager.get_write_connection() as conn:
                    ...
                    try:
                        cursor.execute(
                            "SELECT id FROM users WHERE phone_number = %s OR username = %s",
                            (phone, username),
                        )
                        if cursor.fetchone():
                            conn.rollback()
                            return (
                                jsonify(
                                    {
                                        "success": False,
                                        "error": "رقم الهاتف أو اسم المستخدم مسجل مسبقاً",
                                    }
                                ),
                                409,
                            )
